...
namespace USBFS_v1_50
{
    public partial class CyUSBFSParameters
    {
        ...
        private void GetParams()
        {
            if (m_inst != null)
            {
                m_serializedDeviceDesc = m_inst.GetCommittedParam(PARAM_DEVICEDESCRIPTORS).Value;
                m_serializedStringDesc = m_inst.GetCommittedParam(PARAM_STRINGDESCRIPTORS).Value;
                m_serializedHIDReportDesc = m_inst.GetCommittedParam(PARAM_HIDREPORTDESCRIPTORS).Value;
                m_serializedAudioDesc = m_inst.GetCommittedParam(PARAM_AUDIODESCRIPTORS).Value;
                m_inst.GetCommittedParam(PARAM_MONVBUS).TryGetValueAs(out m_mon_vbus);
                m_inst.GetCommittedParam(PARAM_EXTERNCLS).TryGetValueAs(out m_extern_cls);
                m_inst.GetCommittedParam(PARAM_EXTERNVND).TryGetValueAs(out m_extern_vnd);
                m_inst.GetCommittedParam(PARAM_OUTSOF).TryGetValueAs(out m_out_sof);
        ...
        public void SetParam_rm_ep_isr()
        {
            if (m_inst != null)
            {
                for (int i = 1; i < m_usedEp.Length; i++)
                {
                    ...
                    if (m_usedEp[i] == true)
                    {
                        m_inst.SetParamExpr(param, false.ToString());
                    }
                    else
                    {
                        m_inst.SetParamExpr(param, true.ToString());
                    }
                }
                CommitParams();
            }
        ...
        public void DeserializeDescriptors()
        {
            ...
            if (!string.IsNullOrEmpty(m_serializedDeviceDesc))
            {
                ...
                catch
                {
                    ...
                    InitDescriptors(true, false, false, false);
                }
            }
            else
            {
            ...
            if (!string.IsNullOrEmpty(m_serializedStringDesc))
            {
                ...
                catch
                {
                    MessageBox.Show(String.Format(Properties.Resources.MSG_WRONG_PARAMETER, PARAM_STRINGDESCRIPTORS),
                                    MSG_TITLE_ERROR, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    InitDescriptors(false, true, false, false);
                }
            }
            else
            {
            ...
            if (!string.IsNullOrEmpty(m_serializedHIDReportDesc))
            {
                ...
                catch
                {
                    MessageBox.Show(String.Format(Properties.Resources.MSG_WRONG_PARAMETER, PARAM_HIDREPORTDESCRIPTORS),
                                    MSG_TITLE_ERROR, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    InitDescriptors(false, false, true, false);
                }
            }
            else
            {
            ...
            if (!string.IsNullOrEmpty(m_serializedAudioDesc))
            {
                ...
                catch
                {
                    MessageBox.Show(String.Format(Properties.Resources.MSG_WRONG_PARAMETER, PARAM_AUDIODESCRIPTORS),
                                    MSG_TITLE_ERROR, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    InitDescriptors(false, false, false, true);
                }

                // Check if Device tree has audio interfaces and if so, merge them with audio interfaces to make 
                // one object
        ...
        public void InitDescriptors(bool device, bool str, bool hidReport, bool audio)
        {
            if (device)
            {
                if (m_deviceTree.m_nodes.Count > 0)
                    ...
                    switch (node_child.m_value.bDescriptorType)
                    {
                        ...
                        case CyUSBDescriptorType.HID_REPORT:
                            ...
                            for (int j = 0; j < node_child.m_nodes.Count; j++)
                            {
                                ...
                                rptSize += 
                                    (ushort)(((CyHIDReportItemDescriptor)node_in_report.m_value).Item.m_size + 1);
                                // +1 - prefix
                            }
        ...
        public bool CheckBootloaderReady()
        {
            ...
            // Hid class and at least two endpoints: one In(EP2, MaxPktSize=64) and one Out(EP1, MaxPktSize=64).
            for (int i = 0; i < m_deviceTree.m_nodes[0].m_nodes.Count; i++)//device level
            {
                for (int j = 0; j < m_deviceTree.m_nodes[0].m_nodes[i].m_nodes.Count; j++) // configuration level
                {
                    ...
                    for (int k = 0; k < m_deviceTree.m_nodes[0].m_nodes[i].m_nodes[j].m_nodes.Count; k++) 
                    {
                        ...
                        for (int m = 0; m < m_deviceTree.m_nodes[0].m_nodes[i].m_nodes[j].m_nodes[k].m_nodes.Count; m++)
                        {
                            ...
                            if (desc.bInterfaceClass == CLASS_HID)
                            {
                                bool foundIN = false;
                                bool foundOUT = false;
        ...
        {
            // "USBFS_v1_50" should be replaced with other version after upgrade 
            DirectoryInfo di = new DirectoryInfo(
                    "..\\psoc\\content\\cycomponentlibrary\\CyComponentLibrary.cylib\\USBFS_v1_50\\Custom\\template");
            return di.FullName;
        }
