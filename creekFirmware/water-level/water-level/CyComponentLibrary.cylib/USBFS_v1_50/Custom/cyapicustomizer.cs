...
namespace USBFS_v1_50
{
    ...
    {
        private const string CFILE_NAME = "USBFS.c";
        private string m_paramOut;
                                                          ...
                                                          IEnumerable<CyAPICustomizer> apis)
        {
            ...
            paramDict.TryGetValue(CyUSBFSParameters.PARAM_DEVICEDESCRIPTORS, out m_paramOut);
            parameters.m_serializedDeviceDesc = m_paramOut;
            paramDict.TryGetValue(CyUSBFSParameters.PARAM_STRINGDESCRIPTORS, out m_paramOut);
            parameters.m_serializedStringDesc = m_paramOut;
            paramDict.TryGetValue(CyUSBFSParameters.PARAM_HIDREPORTDESCRIPTORS, out m_paramOut);
            parameters.m_serializedHIDReportDesc = m_paramOut;
            ...
            CyUSBCodeGenerator g = new CyUSBCodeGenerator(query.InstanceName, parameters);
        ...
        private string GenReportTable(int rptIndex, string instName, string qualName)
        {
            ...
            if ((rptIndex > 0) && (rptIndex <= m_parameters.m_hidReportTree.m_nodes[0].m_nodes.Count))
            {
                ...
                if (ReportCount(ReportTop, CyUSBFSParameters.RPTITEM_INPUT) > 0)
                {
                    ...
                    code.AppendLine(GenerateTDEntry(qualName + "_IN_BUF_SIZE", qualName + "_IN_BUF[0]", NULL_PTR));
                ...
                if (ReportCount(ReportTop, CyUSBFSParameters.RPTITEM_OUTPUT) > 0)
                {
                    ...
                    code.AppendLine(GenerateTDEntry(qualName + "_OUT_BUF_SIZE", qualName + "_OUT_BUF[0]", NULL_PTR));
                    code.AppendFormat("{0};{1}", GenerateFooter(), NEWLINE);
                ...
                {
                    code.Append(GenerateLUTEntry(qualName + "_IN_RPT_TABLE", 1));
                }
                else
                {
                ...
                {
                    code.Append(GenerateLUTEntry(qualName + "_OUT_RPT_TABLE", 1));
                }
                else
                {
                    code.Append(GenerateNullLUTEntry());
                ...
                {
                    code.Append(GenerateLUTEntry(qualName + "_FEATURE_RPT_TABLE", 1));
                }
                else
                {
                    code.Append(GenerateNullLUTEntry());
                ...
                code.Append(COMMA_NEWLINE);
                code.Append(GenerateLUTEntry(string.Format("{0}_HIDREPORT_DESCRIPTOR{1}[0]", instName, rptIndex), 1));
                code.Append(COMMA_NEWLINE);
                // Find appropriate reference
                int ind = m_hidClassQualList.IndexOf(qualName.Replace("_HID", ""));
                code.Append(
                    GenerateLUTEntry(
                        string.Format("{0}_DESCR[{1}]", qualName.Remove(qualName.IndexOf("_INTERFACE")),
    ...
    public class CyUsbCodePrimitives
    {
        ...
        {
            string code = String.Format("uint8 {0}[{1}];{2}", n, s, Environment.NewLine);
            return code;
        }
        ...
        {
            string s = String.Format("T_{0}_LUT {1} {2} = {{{3}", INSTANCE_NAME, CODE, n, Environment.NewLine);
            return s;
        }
        ...
        {
            string s = String.Format("T_{0}_TD {1} {2} = {{{3}", INSTANCE_NAME, CODE, n, NEWLINE);
            return s;
        }
    ...
    internal class CyUsbDeviceDescriptorGen : CyUsbCodeGen
    {
        ...
        public override string GenBody(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code.AppendLine(" *********************************************************************/");
            code.AppendFormat("uint8 {0} {1}_DESCR[] = {{{2}", CODE, qualName, NEWLINE);
    ...
    internal class CyUsbInterfaceDescriptorGen : CyUsbCodeGen
    {
        ...
        public override string GenBody(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            string isAudio = (ud is CyAudioInterfaceDescriptor) ? "Audio " : String.Empty;
            // Header Comment
            code.AppendLine(",");
            code.AppendLine("/*********************************************************************");
            code.AppendFormat(" {0}Interface Descriptor", isAudio).AppendLine();
            code.AppendLine(" *********************************************************************/");
    ...
    internal class CyUsbAudioDescriptorGen : CyUsbCodeGen
    {
        ...
        public override string GenBody(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            else if (ud is CyACFeatureUnitDescriptor)
            {
                ...
                code.AppendFormat("{0}{1}", GenerateDescrByteArray(" bmaControls", d.bmaControls.ToArray()),
                                  d.bmaControls.Count > 0 ? COMMA_NEWLINE : NEWLINE);
            ...
            else if (ud is CyACProcessingUnitDescriptor)
            {
                ...
                code.AppendFormat("{0}{1}", GenerateDescrByte(" bDescriptorSubtype", (byte) d.bDescriptorSubtype),
                                  COMMA_NEWLINE);
                code.AppendFormat("{0}{1}", GenerateDescrByte(" bUnitID", d.bUnitID), COMMA_NEWLINE);
                code.AppendFormat("{0}{1}", GenerateDescrByte(" wProcessType", d.bNrInPins), COMMA_NEWLINE);
                code.AppendFormat("{0}{1}", GenerateDescrByte(" bNrInPins", d.bNrInPins), COMMA_NEWLINE);
                code.AppendFormat("{0}{1}", GenerateDescrByteArray(" baSourceID", d.baSourceID),
                                  d.baSourceID.Length > 0 ? COMMA_NEWLINE : NEWLINE);
                code.AppendFormat("{0}{1}", GenerateDescrByte(" bNrChannels", d.bNrChannels), COMMA_NEWLINE);
            ...
            else if (ud is CyACExtensionDescriptor)
            {
                ...
                code.AppendFormat("{0}{1}", GenerateDescrByteArray(" bmControls", d.bmControls),
                                  d.bmControls.Length > 0 ? COMMA_NEWLINE : NEWLINE);
                code.Append(GenerateDescrByte(" iMixer", d.iExtension));
    ...
    internal class CyUsbStringZeroDescriptorGen : CyUsbCodeGen
    {
        ...
        public override string GenHeader(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code.AppendLine(" *********************************************************************/");
            code.AppendFormat("uint8 {0} {1}_STRING_DESCRIPTORS[] = {{{2}", CODE, qualName, NEWLINE);
            code.AppendLine("/*********************************************************************");
    ...
    internal class CyUsbStringSNDescriptorGen : CyUsbCodeGen
    {
        ...
        public override string GenBody(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            CyStringDescriptor d = (CyStringDescriptor) ud;
            StringBuilder code = new StringBuilder();
            code.AppendFormat("uint8 {0} {1}_SN_STRING_DESCRIPTOR[] = {{{2}", CODE, qualName, NEWLINE);
            if (d.bString != null)
            {
                code.AppendFormat("{0}{1} ", GenerateDescrByte("Descriptor Length", d.bLength), COMMA_NEWLINE);
                code.AppendFormat("{0}{1} ", GenerateDescrByte("DescriptorType: STRING", (byte) d.bDescriptorType),
    ...
    internal class CyUsbStringMSOSDescriptorGen : CyUsbCodeGen
    {
        ...
        public override string GenHeader(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code.AppendLine(" *********************************************************************/");
            code.AppendFormat("uint8 {0} {1}_MSOS_DESCRIPTOR[] = {{{2}", CODE, qualName, NEWLINE);
            return code.ToString();
        }
    ...
    internal class CyUsbHIDReportDescriptorGen : CyUsbCodeGen
    {
        ...
        public override string GenHeader(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code.AppendLine(" *********************************************************************/");
            code.AppendFormat("uint8 {0} {1} = {{{2}", CODE, qualName, NEWLINE);
            return code.ToString();
        }
    ...
    internal class CyUsbConfigTableGen : CyUsbCodeGen
    {
        ...
        public override string GenHeader(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code.AppendLine("                          and endpoint setup table and to each interface ");
            code.AppendLine("                          table if it specifies a USB Class ");
            code.AppendLine(" *********************************************************************/");
            code.Append(GenerateLUTHeader(qualName + "_TABLE[]"));
    ...
    internal class CyUsbEndportTableGen : CyUsbCodeGen
    {
                        ...
                        for (int k = 0; k < id.m_nodes.Count; k++)
                        {
                            ...
                            if (en.m_value.bDescriptorType == CyUSBDescriptorType.ENDPOINT)
                            {
                                code.Append(FormatEndpointSetting(CurInterface, CurAltSetting, 
                                    (CyEndpointDescriptor) en.m_value));
                                noEndpoints = false;
                            }
                        }
                    }
        ...
        public override string GenHeader(CyUSBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code.AppendLine("  *********************************************************************/");
            code.AppendFormat("T_{0}_EP_SETTINGS_BLOCK {1} {2}_EP_SETTINGS_TABLE[] = {{{3}", INSTANCE_NAME, CODE,
                              qualName, NEWLINE);
            code.AppendLine("/* IFC  ALT    EPAddr bmAttr MaxPktSize BufferType ********************/");
            return code.ToString();
        }
        ...
        public byte GetNumEPDescriptors(CyDescriptorNode node)
        {
            ...
            if (node.m_nodes.Count == 0)
            {
                if (node.m_value.bDescriptorType == CyUSBDescriptorType.ENDPOINT)
                {
                    count = 1;
                }
            }
        ...
        private string FormatEndpointSetting(byte ifc, byte alt, CyEndpointDescriptor d)
        {
            StringBuilder code = new StringBuilder();
            code.Append(m_nl);
            code.Append("{");
            ...
            code.Append(GenerateByte(d.DoubleBuffer ? (byte) 1 : (byte) 0));
            code.Append("}");
            m_nl = COMMA_NEWLINE;
            return code.ToString();
        }
    ...
    internal class CyUsbCodeGenMaker
    {
        ...
        public CyUsbCodeGen MakeDescriptorGenerator(CyUSBDescriptor d, string instName)
        {
            ...
            else if (d_t == typeof (CyEndpointDescriptor))
            {
