    ...
    public partial class CyCustomizer : ICyAPICustomize_v1, ICyParamEditHook_v1, ICyDRCProvider_v1
    {
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            if ((parameters.SdaTimeoutEnabled == false) && (parameters.SclTimeoutEnabled == false))
            {
                ...
                for (int i = 0; i < customizers.Count; i++)
                {
                    ...
                    if (customizers[i].OutputName.Equals(query.InstanceName + "_TMOUT.c") ||
                        customizers[i].OutputName.Equals(query.InstanceName + "_TMOUT_INT.c"))
                    {
                        itemsToRemove.Add(i);
                    }
                }
        ...
        CyCustErr VerifyRevision(ICyDeviceQuery_v1 deviceQuery, ICyInstQuery_v1 instQuery)
        {
            ...
            if (param.ErrorCount == 0)
            {
                ...
                if (enableWakeup && (deviceQuery.ArchFamilyMember == PSOC5A))
                    return new CyCustErr(Resource.DRCRevisionErrorPSoC5A);
            }
            return CyCustErr.OK;
        }
        ...
        CyCustErr VerifyDataRate(ICyDeviceQuery_v1 deviceQuery, ICyInstQuery_v1 instQuery)
        {
            ...
            if (paramDataRate.ErrorCount == 0 && paramImplementation.ErrorCount == 0)
            {
                ...
                Debug.Assert(err.IsOk);
                // The speeds are 50, 100, 400 only for FF block of the PSoC5A silicon
                // revisions which did not have the full divider implemented
