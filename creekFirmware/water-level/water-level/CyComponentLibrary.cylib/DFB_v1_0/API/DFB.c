...
static void `$INSTANCE_NAME`_LoadReg32(reg32 * regAddress, const uint32 source[], uint8 count)
{
    ...
    while (count-- > 0u)
    {
        CY_SET_REG32(regAddress++, source[i++]);
    }
}
...
void `$INSTANCE_NAME`_Init(void) `=ReentrantKeil($INSTANCE_NAME . "_Init")`
{
    ...
    /* Clears all DFB RAM */
    `$INSTANCE_NAME`_ZeroReg32(`$INSTANCE_NAME`_DA_RAM_PTR, (`$INSTANCE_NAME`_DA_RAM_SIZE >> 2u));
    `$INSTANCE_NAME`_ZeroReg32(`$INSTANCE_NAME`_DB_RAM_PTR, (`$INSTANCE_NAME`_DB_RAM_SIZE >> 2u));
    `$INSTANCE_NAME`_ZeroReg32(`$INSTANCE_NAME`_CSA_RAM_PTR, (`$INSTANCE_NAME`_CSA_RAM_SIZE >> 2u));
    `$INSTANCE_NAME`_ZeroReg32(`$INSTANCE_NAME`_CSB_RAM_PTR, (`$INSTANCE_NAME`_CSB_RAM_SIZE >> 2u));
    `$INSTANCE_NAME`_ZeroReg32(`$INSTANCE_NAME`_ACU_RAM_PTR, (`$INSTANCE_NAME`_ACU_RAM_SIZE >> 2u));
    `$INSTANCE_NAME`_ZeroReg32(`$INSTANCE_NAME`_CFSM_RAM_PTR, (`$INSTANCE_NAME`_CFSM_RAM_SIZE >> 2u));
...
*
* Parameters:
*  int32 * ptr: pointer on data source for load;
     ...
     `=ReentrantKeil($INSTANCE_NAME . "_LoadDataRAMA")`
{
    ...
    if ((addr < `$INSTANCE_NAME`_DA_RAM_PTR) || (addr > `$INSTANCE_NAME`_DB_RAM_PTR - 4u))
    {
        result = `$INSTANCE_NAME`_ADDRESS_OUT_OF_RANGE;
    }
    else if ((size == 0u) || (size > (`$INSTANCE_NAME`_DB_RAM_PTR - addr) >> 2u))
    {
    ...
    else
    {
        ...
        while (size-- > 0u)
        {
            CY_SET_REG32((reg32 *) addr, (uint32)(*ptr));
            addr++;
            ptr++;
        }
        /* Takes DFB RAM off the bus */
     ...
     `=ReentrantKeil($INSTANCE_NAME . "_LoadDataRAMB")`
{
    ...
    if ((addr < `$INSTANCE_NAME`_DB_RAM_PTR) || (addr > `$INSTANCE_NAME`_CSA_RAM_PTR - 4u))
    {
        result = `$INSTANCE_NAME`_ADDRESS_OUT_OF_RANGE;
    }
    else if ((size == 0u) || (size > (`$INSTANCE_NAME`_CSA_RAM_PTR - addr) >> 2u))
    {
        result = `$INSTANCE_NAME`_DATA_OUT_OF_RANGE;
    }
    else
...
uint8 `$INSTANCE_NAME`_GetInterruptSource(void) `=ReentrantKeil($INSTANCE_NAME . "_GetInterruptSource")`
{
    ...
    return (`$INSTANCE_NAME`_SR_REG >> `$INSTANCE_NAME`_SR_EVENT_SHIFT);
}
...
{
    `$INSTANCE_NAME`_SR_REG &= (interruptMask << `$INSTANCE_NAME`_SR_EVENT_SHIFT);
}
...
{
    `$INSTANCE_NAME`_SEM_REG = ((mask & `$INSTANCE_NAME`_SEM_MASK) << `$INSTANCE_NAME`_SEM_ENABLE_SHIFT) |
