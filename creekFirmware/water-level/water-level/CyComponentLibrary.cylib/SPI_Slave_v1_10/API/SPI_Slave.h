 ...
 **************************************/
#define `$INSTANCE_NAME`_INIT_INTERRUPTS_MASK       (`$IntOnSPIDone` << `$INSTANCE_NAME`_STS_SPI_DONE_SHIFT ) | (`$IntOnTXNotFull` << `$INSTANCE_NAME`_STS_TX_FIFO_NOT_FULL_SHIFT) | (`$IntOnTXFull` << `$INSTANCE_NAME`_STS_TX_FIFO_FULL_SHIFT ) | (`$IntOnRXNotEmpty` << `$INSTANCE_NAME`_STS_RX_FIFO_NOT_EMPTY_SHIFT ) | (`$IntOnRXEmpty` << `$INSTANCE_NAME`_STS_RX_FIFO_EMPTY_SHIFT ) | (`$IntOnRXOver` << `$INSTANCE_NAME`_STS_RX_FIFO_OVERRUN_SHIFT ) | (`$IntOnByteComp` << `$INSTANCE_NAME`_STS_BYTE_COMPLETE_SHIFT) 
