...
namespace I2C_v2_0
{
    ...
    public enum CyEModeType { Slave = 1, Master = 2, Multi_Master = 6, Multi_Master_Slave = 7 }
    ...
    public class CyI2CParameters
    {
        ...
        {
            this.m_inst = inst;
            m_modesList = (List<string>)inst.GetPossibleEnumValues(CyParamNames.I2C_MODE);
        ...
        public CyEModeType Mode
        {
            ...
            set
            {
                if (value == CyEModeType.Slave || value == CyEModeType.Master
                    || value == CyEModeType.Multi_Master || value == CyEModeType.Multi_Master_Slave)
                    m_mode = value;
            }
        }
        ...
        public void GetParams(ICyInstEdit_v1 inst)
        {
            ...
            // BusSpeed_kHz
            int previousBusSpeed = 0;
            try
            {
                previousBusSpeed = this.BusSpeed;
                this.BusSpeed = int.Parse(inst.GetCommittedParam(CyParamNames.BUS_SPEED_KHZ).Value);
            }
            catch (Exception)
            { this.BusSpeed = previousBusSpeed; }
            ...
            // Slave_Address
            int previousSlaveAddress = 0;
            try
            {
                previousSlaveAddress = this.SlaveAddress;
                this.SlaveAddress = int.Parse(inst.GetCommittedParam(CyParamNames.SLAVE_ADDRESS).Value);
            }
            catch (Exception)
            { this.SlaveAddress = previousSlaveAddress; }
