    ...
    public partial class CyCustomizer : ICyParamEditHook_v1, ICyShapeCustomize_v1, ICyAPICustomize_v1, ICyDRCProvider_v1
    {
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            for (int i = 0; i < parameters.VoltagesTable.Count; i++)
            {
                ...
                element = ((UInt16)(parameters.VoltagesTable[i].m_uvWarningTreshold * 1000)).ToString();
                uvWarnThreshold += (i < parameters.VoltagesTable.Count - 1) ? element + ", " : element + " };";

                element = ((UInt16)(parameters.VoltagesTable[i].m_ovWarningTreshold * 1000)).ToString();
                ovWarnThreshold += (i < parameters.VoltagesTable.Count - 1) ? element + ", " : element + " };";

                element = ((UInt16)(parameters.VoltagesTable[i].m_uvFaultTreshold * 1000)).ToString();
                uvFaultThreshold += (i < parameters.VoltagesTable.Count - 1) ? element + ", " : element + " };";

                element = ((UInt16)(parameters.VoltagesTable[i].m_ovFaultTreshold * 1000)).ToString();
                ovFaultThreshold += (i < parameters.VoltagesTable.Count - 1) ? element + ", " : element + " };";

                element = parameters.VoltagesTable[i].m_inputScalingFactor.ToString();
            ...
            for (int i = 0; i < parameters.CurrentsTable.Count; i++)
            {
                ...
                element = (parameters.CurrentsTable[i].m_ocWarningThreshold).ToString();
                ocWarnThreshold += (i < parameters.CurrentsTable.Count - 1) ? element + ", " : element + " };";

                element = (parameters.CurrentsTable[i].m_ocFaulthTreshold).ToString();
                ocFaultThrehsold += (i < parameters.CurrentsTable.Count - 1) ? element + ", " : element + " };";

                element = (parameters.CurrentsTable[i].m_shuntResistorValue).ToString();
                rShunt += (i < parameters.CurrentsTable.Count - 1) ? element + ", " : element + " };";

                element = parameters.CurrentsTable[i].m_csaGain.ToString();
        ...
        public IEnumerable<CyDRCInfo_v1> GetDRCs(ICyDRCProviderArgs_v1 args)
        {
            ...
            {
                if (CyParameters.VoltageTableRowHasEmptyCells(prms.VoltagesTable[i])) // this means that empty row was saved
                {
            ...
            {
                if (CyParameters.CurrentTableRowHasEmptyCells(prms.CurrentsTable[i]))
