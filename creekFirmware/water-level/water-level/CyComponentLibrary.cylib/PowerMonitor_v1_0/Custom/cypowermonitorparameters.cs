...
namespace PowerMonitor_v1_0
{
    ...
    public class CyParamRanges
    {
        ...
        public const byte NUM_CONVERTERS_MIN = 0;
    ...
    public class CyParameters
    {
            ...
            if (parent is CyVoltagesTab)
            {
                ...
                if (importedTable != null)
                {
                    ...
                    if (pasteMode)
                    {
                        ...
                        {
                            string converterNumber = VoltagesTable[i].m_powerConverterNumber;
                            VoltagesTable[i] = importedTable[j];
                            VoltagesTable[i].m_powerConverterNumber = converterNumber;
                            j++;
                        }
                    }
                    else
            ...
            else if (parent is CyCurrentsTab)
            {
                ...
                if (importedTable != null)
                {
                    ...
                    if (pasteMode)
                    {
                        ...
                        for (int i = firstPasteRow; i < lastPasteRow; i++)
                        {
                            string converterNumber = CurrentsTable[i].m_powerConverterNumber;
                            CurrentsTable[i] = importedTable[j];
                            CurrentsTable[i].m_powerConverterNumber = converterNumber;
                            j++;
                        }
                    }
                    else
                    ...
                    for (int i = 0; i < CurrentsTable.Count; i++)
                    {
                        CurrentsTable[i].m_converterName = VoltagesTable[i].m_converterName;
                        CurrentsTable[i].m_nominalOutputVoltage = VoltagesTable[i].m_nominalOutputVoltage;
                    }
                    m_currentsTab.UpdateUIFromTable();
                    SetCurrentsTable();
            ...
            else if (parent is CyAuxiliaryTab)
            {
                ...
                if (importedTable != null)
                {
                    ...
                    if (pasteMode)
                    {
                        ...
                        {
                            string auxInputNumber = AuxTable[i].m_auxInputNumber;
                            AuxTable[i] = importedTable[j];
                            m_generalTab.SyncAuxTableItem(i);
                            AuxTable[i].m_auxInputNumber = auxInputNumber;
                            j++;
                        }
                    }
                    else
        ...
        public static void SetRowEmptyValues(object[] row)
        {
            for (int k = 0; k < row.Length; k++)
                if (row[k].GetType() != typeof(string) && row[k].ToString() == "0")
                    row[k] = string.Empty;
        }
        ...
        public static bool CurrentTableRowHasEmptyCells(CyCurrentsTableRow row)
        {
            return row.m_currentMeasurementType != CyECurrentMeasurementInternalType.None &&
                    (row.m_ocFaulthTreshold == 0 || row.m_ocWarningThreshold == 0 || row.m_shuntResistorValue == 0 ||
                    (row.m_currentMeasurementType == CyECurrentMeasurementInternalType.CSA && row.m_csaGain == 0)
                    );
        }
        public static bool VoltageTableRowHasEmptyCells(CyVoltagesTableRow row)
        {
            return row.m_inputScalingFactor == 0 ||
                row.m_nominalOutputVoltage == 0 ||
                row.m_ovFaultTreshold == 0 ||
                row.m_ovWarningTreshold == 0 ||
                row.m_uvFaultTreshold == 0 ||
                row.m_uvWarningTreshold == 0;
        }
