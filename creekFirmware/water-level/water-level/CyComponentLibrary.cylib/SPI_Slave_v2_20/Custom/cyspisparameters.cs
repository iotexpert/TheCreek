...
namespace SPI_Slave_v2_20
{
    ...
    public class CySPISParameters
    {
        ...
        private void SetControlFileName()
        {
            ...
            {
                if (FixedPlacementEnabled == false)
                {
                    fileName = "SPI_Slave_Default.ctl";
                }
                else if (BidirectMode == false)
                {
                    if (NumberOfDataBits < 9)
                    {
                        if (Mode == E_SPI_MODES.Mode_00 || Mode == E_SPI_MODES.Mode_01)
                            fileName = "SPI_Slave_8_FD_CPHA_0.ctl";
                        else
                            fileName = "SPI_Slave_8_FD_CPHA_1.ctl";
                    }
                    else
                    {
                        if (Mode == E_SPI_MODES.Mode_00 || Mode == E_SPI_MODES.Mode_01)
                            fileName = "SPI_Slave_16_FD_CPHA_0.ctl";
                        else
                            fileName = "SPI_Slave_16_FD_CPHA_1.ctl";
                    }
                }
                else
                {
                    ...
                    {
                        if (Mode == E_SPI_MODES.Mode_00 || Mode == E_SPI_MODES.Mode_01)
                            fileName = "SPI_Slave_8_BD_CPHA_0.ctl";
                        else
                            fileName = "SPI_Slave_8_BD_CPHA_1.ctl";
                    }
                    else
                    {
                        if (Mode == E_SPI_MODES.Mode_00 || Mode == E_SPI_MODES.Mode_01)
                            fileName = "SPI_Slave_16_BD_CPHA_0.ctl";
                        else
                            fileName = "SPI_Slave_16_BD_CPHA_1.ctl";
                    }
                }
            }
            else if (m_inst.DeviceQuery.IsPSoC5)
            {
                ...
                else if (BidirectMode == false)
                {
                    if (NumberOfDataBits < 9)
                    {
                        fileName = "SPI_Slave_8_FD_ARM.ctl";
                    }
                    else
                    {
                        fileName = "SPI_Slave_16_FD_ARM.ctl";
                    }
                }
                else
                {
                    if (NumberOfDataBits < 9)
                    {
                        fileName = "SPI_Slave_8_BD_ARM.ctl";
                    }
                    else
                    {
                        fileName = "SPI_Slave_16_BD_ARM.ctl";
                    }
                }
            }
