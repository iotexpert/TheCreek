...
/* CyBtldr_Checksum and CyBtldr_SizeBytes are forcefully set to 0 in code. We then post process
 * the hex file from the linker and inject their values then. When the hex file is loaded
 * onto the device these two variables should have valid values.
*/
const uint8 CYCODE CyBtldr_Checksum = 0;
const uint32 CYCODE CyBtldr_SizeBytes = 0;
...
void CyBtldr_Start(void)
{
	...
	resetSR0 = CYDEV_RESET_SR0;
	
	if (CyBtldr_SizeBytes == 0)
	{
		CYBtldr_BadBootloader();
	}
		
	/* the bootloader always starts at 0 in the  */
	for (idx = 0; idx < CyBtldr_SizeBytes; ++idx)
	{
		calcedChecksum += CY_GETCODEDATA(idx);
	}
	calcedChecksum -= CyBtldr_Checksum; /* we actually included the checksum, so remove it */
	calcedChecksum = CHECKSUM_INVERTER - calcedChecksum;
	
	if (calcedChecksum != CyBtldr_Checksum)
	{
		CYBtldr_BadBootloader();
	}
...
#if defined (WORKAROUND_OPT_XRES)
	if (status != CYRET_SUCCESS || (*CY_BTLDR_P_APP_RUN_ADDR) == 0x40)
#else
...
{
#if defined (WORKAROUND_OPT_XRES)
	CyBtldr_SetFlashRunType(0x80);
#else
    (*(uint8 XDATA*)CYREG_RESET_SR0) |= 0x80; /* set bit to indicate we want to start application */
#endif
...
{
#if defined (WORKAROUND_OPT_XRES)
	if ((*CY_BTLDR_P_APP_RUN_ADDR) == 0x80)
	{
...
#else
	if ((*(uint8 XDATA*)CYREG_RESET_SR0 & 0x80) == 0x80)
	{
...
void CyBtLdrHostLink(uint16 TimeOut)
{
...
#if (defined(__C51__))
	BtLdrFlashSize.FirstRowNumber = CYSWAP_ENDIAN16((CyBtldr_SizeBytes / CYDEV_FLS_ROW_SIZE));
#else
    BtLdrFlashSize.FirstRowNumber = (CyBtldr_SizeBytes / CYDEV_FLS_ROW_SIZE);
#endif
    ...
    while(LinkWaitState || TimeOut--)
    {
        ...
        /* Get packet header. */
        if(CyBtldrCommRead(TData, 1, &NumberRead, 100) || NumberRead != 1 || *TData != SOP)
        {
            continue;
        }
        ...
        else if(PktCommand == CYBTLDR_COMMAND_PROGRAM)
        {
            ...
            memmove(&CommandDataBuffer[CommandDataSize], &CommandDataBuffer[3 + CommandDataSize], PktSize);
        ...
        else if(PktCommand == CYBTLDR_COMMAND_VERIFY)
        {
            ...
            Offset = CYDEV_FLS_BASE + ((uint32) Index * CYDEV_FLS_ROW_SIZE);
            for(TData[0] = 0, Index = 0; Index < CYDEV_FLS_ROW_SIZE; Index++)
            {
                TData[0] += CY_GET_XTND_REG8((void far *) Offset + Index);
            }
