...
void `$INSTANCE_NAME`_Enable(void) `=ReentrantKeil($INSTANCE_NAME . "_Enable")`
{
    /* If a boost clock is required  */
        ...
        `$INSTANCE_NAME`_BST_REG |= `$INSTANCE_NAME`_BST_CLK_EN | CyScBoostClk__INDEX;
...
void `$INSTANCE_NAME`_Stop(void) `=ReentrantKeil($INSTANCE_NAME . "_Stop")`
{
    ...
    #if(`$INSTANCE_NAME`_CYDEV_VDDA_MV < `$INSTANCE_NAME`_MINIMUM_VDDA_THRESHOLD_MV)
        `$INSTANCE_NAME`_BST_REG &= ~`$INSTANCE_NAME`_BST_CLK_EN;
    #endif /* `$INSTANCE_NAME`_ACLK_ENABLE */
    
    /* This sets PGA Inv in zero current mode and output routes are valid */
    #if (CY_PSOC5A)
