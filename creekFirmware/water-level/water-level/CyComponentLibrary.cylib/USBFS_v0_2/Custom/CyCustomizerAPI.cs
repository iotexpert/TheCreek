...
namespace USBFS_v0_2
{
    ...
    {
        string CFile_Name = "USBFS.c";
        string HFile_Name = "USBFS.h";
        string instanceNameParam = "INSTANCE_NAME";


        private string ParamOut;

        CyAPICustomizer CFile;
        CyAPICustomizer HFile;
...
//            Debugger.Break();
    ...
    // CyAPICustomizer
    class USBCodeGenerator : UsbCodePrimitives
    {
        const string APIGEN_DEFINES = "APIGEN_DEFINES";

        const string APIGEN_DEVICE_DESCRIPTORS = "APIGEN_DEVICE_DESCRIPTORS";
        const string APIGEN_DEVICE_TABLES = "APIGEN_DEVICE_TABLES";
        const string DEF_NUM_DEVICES = "NUM_DEVICES";

        const string APIGEN_STRING_DESCRIPTORS = "APIGEN_STRING_DESCRIPTORS";
        const string APIGEN_EE_DESCRIPTOR = "APIGEN_EE_DESCRIPTOR";
        const string APIGEN_SN_DESCRIPTOR = "APIGEN_SN_DESCRIPTOR";

        const string APIGEN_DEFINE_ENABLE_STRINGS = "APIGEN_ENABLE_STRINGS";
        const string APIGEN_DEFINE_EE_STRING = "APIGEN_ENABLE_EE_STRING";
        const string APIGEN_DEFINE_SN_STRING = "APIGEN_ENABLE_SN_STRING";

        const string DEF_ENABLE_STRINGS = "ENABLE_STRINGS";
        const string DEF_EE_STRING = "ENABLE_EE_STRING";
        const string DEF_SN_STRING = "ENABLE_SN_STRING";

        const string APIGEN_HIDREPORT_DESCRIPTORS = "APIGEN_HIDREPORT_DESCRIPTORS";
        const string APIGEN_HIDREPORT_TABLES = "APIGEN_HIDREPORT_TABLES";
        const string DEF_ENABLE_HID_CLASS = "ENABLE_HID_CLASS";


        const int SN_INDEX = 0;
        const int EE_INDEX = 1;

        string m_InstanceName;

        USBFSParameters m_Parameters;

        Dictionary<string, string> m_CodeParameters = new Dictionary<string, string>();
        
        public USBCodeGenerator(string InstanceName, USBFSParameters Parameters) : base (InstanceName)
        {
        ...
        private void GenerateDevices()
        {
            ...
            string deviceDescr = gtDescr.Generate(m_Parameters.DeviceTree.Nodes[0].Nodes, m_InstanceName, m_InstanceName);
            ...
            string deviceTables = genTable.Generate(m_Parameters.DeviceTree.Nodes[0].Nodes, m_InstanceName, m_InstanceName);
        ...
        private void GenerateStrings()
        {
            ...
            string stringDescr = gtStrings.Generate(m_Parameters.StringTree.Nodes[0].Nodes, m_InstanceName, m_InstanceName);
            ...
            if (s.bString != null)
            {
                if (s.bString.Length > 0)
                {
                    enableStrings = true;
                    UsbStringMSOSDescriptorGen cg = new UsbStringMSOSDescriptorGen(m_InstanceName);
                    string EE_Code = cg.GenHeader(s, m_InstanceName, m_InstanceName);
                    EE_Code += cg.GenBody(s, m_InstanceName, m_InstanceName);
                    EE_Code += cg.GenFooter(s, m_InstanceName, m_InstanceName);

                    m_CodeParameters.Add(APIGEN_EE_DESCRIPTOR, EE_Code);
                    AddDefine(DEF_SN_STRING);
                }
                
            }            // Special String support SN String (EE)
            ...
            if (s.bString != null)
            {
                ...
                {
                    enableStrings = true;
                    string SN_Code = "/* We gotta SN String */\n";
        ...
        private void GenerateHIDReports()
        {
            ...
            string HIDReportDescr = gtHIDReport.Generate(m_Parameters.HIDReportTree.Nodes[0], m_InstanceName, m_InstanceName);
            ...
            if (HIDReportDescr.Length > 0)
            {
                ...
                {
                    int rptIndex = i + 1;
                    AddDefine("HID_RPT_" + rptIndex + "_SIZE_LSB", String.Format("{0}", HexByteString((byte)(ReportDescriptorSize(m_Parameters.HIDReportTree.Nodes[i].Nodes[0].Nodes) & 0xFF))));
                    AddDefine("HID_RPT_" + rptIndex + "_SIZE_MSB", String.Format("{0}", HexByteString((byte)((ReportDescriptorSize(m_Parameters.HIDReportTree.Nodes[i].Nodes[0].Nodes) >> 8) & 0xFF))));
                }
        ...
        private string GenReportTable(int rptIndex, string instName, string qualName)
        {
            ...
            if ((rptIndex > 0) && (rptIndex <= m_Parameters.HIDReportTree.Nodes.Count))
            {
                ...
                if (ReportCount(ReportTop, "FEATURE") > 0)
                {
                    ...
                    code += GenerateTDEntry(qualName + "_NUM_FEATURE_RPTS", qualName + "_FEATURE_BUF[0]", NULL_PTR) + NEWLINE;
                    code += GenerateFooter() + ";\n";
    ...
    {
        const int commentWidth = 40;
        const string m_COMMA_NEWLINE = ",\n";
        ...
        public string CODE
        {
            get
            {
                return m_InstanceName + "_" + m_Code;
            }
            set
            {
                m_Code = value;
        ...
        public string NULL_PTR
        {
            get
            {
                return m_InstanceName + "_NULL";
            }
        }

    }
    #region Descriptor Code Generators
    abstract public class UsbCodeGen : UsbCodePrimitives
    {
        ...
        virtual public string Generate(List<DescriptorNode> nodes, string instName, string qualName)
        {
            string code = "";
        ...
        virtual public string Generate(DescriptorNode node, string instName, string qualName)
        {
            throw new NotImplementedException();
        }
        abstract public string GenHeader(USBDescriptor ud, string instanceName, string qualName);
        abstract public string GenBody(USBDescriptor ud, string instanceName, string qualName);
        abstract public string GenFooter(USBDescriptor ud, string instanceName, string qualName);
    ...
    class UsbDeviceDescriptorGen : UsbCodeGen
    {
        ...
        public override string GenBody(USBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code += GenerateDescrByte("bDeviceClass", d.bDeviceSubClass) + COMMA_NEWLINE;
...
//            code += GenerateDescrByte("iSerialNumber", d.iSerialNumber) + COMMA_NEWLINE;
    ...
    class UsbConfigDescriptorGen : UsbCodeGen
    {
    ...
    class UsbInterfaceDescriptorGen : UsbCodeGen
    {
    ...
    class UsbEndpointDescriptorGen : UsbCodeGen
    {
    ...
    class UsbHIDClassDescriptorGen : UsbCodeGen
    {
        ...
        public override string GenBody(USBDescriptor ud, string instanceName, string qualName)
        {
            ...
            code += GenerateSymbolicByte(" wDescriptorLength (LSB)", instanceName + "_HID_RPT_" + d.ReportIndex + "_SIZE_LSB") + COMMA_NEWLINE;
            code += GenerateSymbolicByte(" wDescriptorLength (MSB)", instanceName + "_HID_RPT_" + d.ReportIndex + "_SIZE_MSB");
    ...
    class UsbStringDescriptorGen : UsbCodeGen
    {
    ...
    class UsbStringZeroDescriptorGen : UsbCodeGen
    {
    ...
    class UsbStringMSOSDescriptorGen : UsbCodeGen
    {
    ...
    class UsbHIDReportDescriptorGen : UsbCodeGen
    {
    ...
    class UsbHIDReportItemGen : UsbCodeGen
    {
    ...
    class UsbUnknownDescriptorGen : UsbCodeGen
    {
    ...
    class UsbDeviceRootTableGen : UsbCodeGen
    {
    ...
    class UsbDeviceTableGen : UsbCodeGen
    {
    ...
    class UsbConfigTableGen : UsbCodeGen
    {
        ...
        public override string Generate(DescriptorNode node, string instName, string qualName)
        {
            ...
            {
                string qual = qualName + "_" + node.Nodes[i].Value.bDescriptorType + i;//+ i.ToString;
                code += GenNumInterfaceTblPtr(node.Nodes[i], instName, qual);
            }
            code += GenFooter(node.Value, instName, qualName);
        ...
        private string GenNumInterfaceTblPtr(DescriptorNode node, string instanceName, string qualName)
        {
            string code = "";
            InterfaceDescriptor d = (InterfaceDescriptor)node.Value;
            if ((d.bInterfaceClass == USBFSParameters.CLASS_NONE) || (d.bInterfaceClass == USBFSParameters.CLASS_VENDORSPEC))
            {
    ...
    class UsbInterfaceTableGen : UsbCodeGen
    {
    ...
    class UsbEndportTableGen : UsbCodeGen
    {
            ...
            if (node.Value.bDescriptorType == USBDescriptorType.CONFIGURATION)
            {
                ...
                for (int i = 0; i < node.Nodes.Count; i++)
                {
                    ...
                    for (int j = 0; j < node.Nodes[i].Nodes.Count; j++)
                    {
                        ...
                        {
                            code += FormatEndpointSetting(CurInterface, CurAltSetting, (EndpointDescriptor)node.Nodes[i].Nodes[j].Value);
                        }
        ...
        public byte GetNumEPDescriptors(DescriptorNode node)
        {
            ...
            foreach (DescriptorNode node_i in node.Nodes)
            {
                foreach (DescriptorNode node_j in node_i.Nodes)
                {
                    if (node_j.Value.bDescriptorType == USBDescriptorType.ENDPOINT)
                    {
                        count++;
                    }
                }
    ...
    abstract class GenTree
    {
        abstract public string Generate(List<DescriptorNode> nodes, string instName, string qualName);
        abstract public string Generate(DescriptorNode node, string instName, string qualName);
    }
    class UsbDescriptorGenTree : GenTree
    {
    ...
    class UsbStringDescriptorGenTree : GenTree
    {
    ...
    class UsbHIDReportDescriptorGenTree : GenTree
    {
        ...
        public override string Generate(List<DescriptorNode> nodes, string instName, string qualName)
        {
            ...
            if (nodes.Count > 0)
            {
                ...
                {
                    code += cg.GenBody(nodes[i].Value, instName, qualName);
                    code += prim.GenerateDescrDWLS("Descriptor Size (Not part of descriptor)", ReportSize(nodes[0].Nodes, prim)) + prim.COMMA_NEWLINE;
    ...
    class UsbHIDReportItemGenTree : GenTree
    {
    ...
    class UsbCodeGenMaker
    {
        ...
        public UsbCodeGen MakeDescriptorGenerator(USBDescriptor d, string instName)
        {
            ...
            if (d_t == Type.GetType("USBFS_v0_2.DeviceDescriptor"))
            {
                c = new UsbDeviceDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.ConfigDescriptor"))
            {
                c = new UsbConfigDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.InterfaceDescriptor"))
            {
                c = new UsbInterfaceDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.EndpointDescriptor"))
            {
                c = new UsbEndpointDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.HIDDescriptor"))
            {
                c = new UsbHIDClassDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.StringDescriptor"))
            {
                c = new UsbStringDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.StringZeroDescriptor"))
            {
                c = new UsbStringZeroDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.HIDReportDescriptor"))
            {
                c = new UsbHIDReportDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v0_2.HIDReportItemDescriptor"))
            {
