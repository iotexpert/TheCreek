...
namespace USBFS_v0_2
{
    public partial class CyDetailsEndpoint : UserControl
    {
        ...
        public CyDetailsEndpoint(EndpointDescriptor descriptor, USBFSParameters parameters)
        {
            InitializeComponent();
        ...
        private void InitFields()
        {
            ...
            switch (Descriptor.bmAttributes & 0x03)
            {
                ...
                case USBFSParameters.TRANSFERTYPE_ISOCHRONOUS:
                    ...
                    {
                        case USBFSParameters.USAGETYPE_DATAEP:
                            comboBoxUsageType.SelectedIndex = USAGETYPE_DATAEP_ITEM;
                            break;
                        case USBFSParameters.USAGETYPE_FEEDBACKEP:
                            comboBoxUsageType.SelectedIndex = USAGETYPE_FEEDBACKEP_ITEM;
                            break;
                        case USBFSParameters.USAGETYPE_IMPLICITEEP:
                    ...
                    break;
                case USBFSParameters.TRANSFERTYPE_BULK:
                    comboBoxTransferType.SelectedIndex = TRANSFERTYPE_BULK_ITEM;
                    break;
                case USBFSParameters.TRANSFERTYPE_INTERRUPT:
        ...
        private void comboBoxTransferType_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboBoxTransferType.SelectedIndex == TRANSFERTYPE_ISOCHRONOUS_ITEM)
            {
                ...
                comboBoxSynchType.SelectedIndex = 0;
                comboBoxUsageType.SelectedIndex = 0;
                groupBoxParams.Height = comboBoxUsageType.Top + comboBoxUsageType.Height +
                    groupBoxParams.Height - (numUpDownMaxPacketSize.Top + numUpDownMaxPacketSize.Height) +
                    (numUpDownMaxPacketSize.Top - numUpDownInterval.Top) * 2;
                checkBoxDoubleBuffer.Top = groupBoxParams.Top + groupBoxParams.Height + 7;
                buttonApply.Top = checkBoxDoubleBuffer.Top;
            }
            else
            {
        ...
        private void CyDetailsEndpoint_SizeChanged(object sender, EventArgs e)
        {
        ...
        private void textBoxInterval_Validated(object sender, EventArgs e)
        {
            ...
            {
                //if ((textBoxInterval.Text == "") && (Descriptor.bInterval != 0))
                //{
        ...
        private void textBoxMaxPacketSize_Validated(object sender, EventArgs e)
        {
            //try
            //{
            //    if (Descriptor.wMaxPacketSize != Convert.ToUInt16(textBoxMaxPacketSize.Text))
            //    {
            //        Descriptor.wMaxPacketSize = Convert.ToUInt16(textBoxMaxPacketSize.Text);
            //        if (!InternalChanges)
            //            Parameters.ParamsChanged = true;
            //    }
            //}
            //catch (Exception)
            //{
            //    if ((textBoxMaxPacketSize.Text == "") && (Descriptor.wMaxPacketSize != 0))
            //    {
            //        Descriptor.wMaxPacketSize = 0;
            //        if (!InternalChanges)
            //            Parameters.ParamsChanged = true;
            //    }
            //}
        ...
        private void comboBoxTransferType_Validated(object sender, EventArgs e)
        {
            ...
            switch (comboBoxTransferType.SelectedIndex)
            {
                case TRANSFERTYPE_CONTROL_ITEM:
                    Descriptor.bmAttributes = USBFSParameters.TRANSFERTYPE_CONTROL;
                    break;
                case TRANSFERTYPE_ISOCHRONOUS_ITEM:
                    switch (comboBoxSynchType.SelectedIndex)
                    {
                        case SYNCHTYPE_NOSYNCH_ITEM:
                            synchType = USBFSParameters.SYNCHTYPE_NOSYNCH;
                            break;
                        case SYNCHTYPE_ASYNCH_ITEM:
                            synchType = USBFSParameters.SYNCHTYPE_ASYNCH;
                            break;
                        case SYNCHTYPE_ADAPTIVE_ITEM:
                            synchType = USBFSParameters.SYNCHTYPE_ADAPTIVE;
                            break;
                        case SYNCHTYPE_SYNCH_ITEM:
                            synchType = USBFSParameters.SYNCHTYPE_SYNCH;
                            break;
                        default:
                            synchType = 0;
                            break;
                    ...
                    switch (comboBoxUsageType.SelectedIndex)
                    {
                        case USAGETYPE_DATAEP_ITEM:
                            usageType = USBFSParameters.USAGETYPE_DATAEP;
                            break;
                        case USAGETYPE_FEEDBACKEP_ITEM:
                            usageType = USBFSParameters.USAGETYPE_FEEDBACKEP;
                            break;
                        case USAGETYPE_IMPLICITEEP_ITEM:
                            usageType = USBFSParameters.USAGETYPE_IMPLICITEEP;
                            break;
                        default:
                            usageType = 0;
                            break;
                    }
                    Descriptor.bmAttributes = (byte)(USBFSParameters.TRANSFERTYPE_ISOCHRONOUS | (synchType << 2) | (usageType << 4));
                    break;
                case TRANSFERTYPE_BULK_ITEM:
                    Descriptor.bmAttributes = USBFSParameters.TRANSFERTYPE_BULK;
                    break;
                case TRANSFERTYPE_INTERRUPT_ITEM:
                    Descriptor.bmAttributes = USBFSParameters.TRANSFERTYPE_INTERRUPT;
                    break;
                default:
                    break;
        ...
        private void textBoxInterval_Validating(object sender, CancelEventArgs e)
        {
            ...
            //    {
            //        byte res = Convert.ToByte(textBoxInterval.Text);
            //        if (res > 255) throw new Exception();
            //    }
            //}
            //catch (Exception)
            //{
            //    MessageBox.Show("Invalid Interval Value (must be 0-255)", "USBFS", MessageBoxButtons.OK,
            //                    MessageBoxIcon.Warning);
            //    e.Cancel = true;
            //}
        }

        private void textBoxMaxPacketSize_Validating(object sender, CancelEventArgs e)
        {
            //try
            //{
            //    if (((TextBox)sender).Text != "")
            //    {
            //        byte res = Convert.ToByte(textBoxMaxPacketSize.Text);
            //        if (res > 64) throw new Exception();
            //    }
            //}
            //catch (Exception)
            //{
            //    MessageBox.Show("Invalid Max Packet Size Value (must be 0-64)", "USBFS", MessageBoxButtons.OK,
            //                    MessageBoxIcon.Warning);
            //    e.Cancel = true;
            //}
        }
