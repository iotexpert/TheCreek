...
void CyPmRestoreClocks(void) `=ReentrantKeil("CyPmRestoreClocks")`
{
    ...
    if(cyPmClockBackup.imoFreq != (CY_PM_FASTCLK_IMO_CR_REG & CY_PM_FASTCLK_IMO_CR_FREQ_MASK))
    {
        /* IMO was 24 MHz and USB clock is enabled */
...
*
*  PSoC 5:
*  Neither of the parameters to this function are used for PSoC 5.
*  The device will go into Sleep mode until it is woken by an interrupt from
*  one of three wakeup sources: Central Time Wheel (CTW), Once per second, or
*  Port Interrupt Controller (PICU). These wakeup sources must already be
*  configured to generate an interrupt. The CTW is configured using the
*  SleepTimer component and the Once per second interrupt using the Real Time
*  Clock component.
*
* Parameters:
*  wakeupTime:      Specifies a timer wakeup source and the frequency of that
...
*
*  PSoC 5:
*  Before calling this function a PICU interrupt must be configured to wake the
...
static void CyPmHibSlpSaveSet(void)
{
    ...
    */
    ...
    /* Device is PSoC 3 and the revision is ES2 or earlier */
        ...
        /* Buzz cannot be disabled. Increase buzz rate for digital and analog
        *  LDOs during sleep mode the maximum value to decrease consumption.
        */
