...
extern volatile uint8 `$INSTANCE_NAME`_interfaceClass[];
...
void `$INSTANCE_NAME`_ConfigAltChanged(void);
...
/*DIE ID string descriptor for 8 bytes ID*/
    ...
    void `$INSTANCE_NAME`_Stop_DMA(uint8 epNumber);
...
void `$INSTANCE_NAME`_Config(uint8 clearAltSetting)
{
    ...
    if(`$INSTANCE_NAME`_configuration > 0u)
    {
        ...
        */
        ...
        for (i = 0u; i < ep; i++, pEP++)
        {
            /* misc containe interface class*/
            `$INSTANCE_NAME`_interfaceClass[pEP->interface] = pEP->bMisc;
        ...
        for (i = 0u; i < 0x80u; i+= 0x10u)
        {
            ...
            if(`$INSTANCE_NAME`_EP[ep].epMode != `$INSTANCE_NAME`_MODE_DISABLE)
            {
                ...
                else
                {
                    ...
                    #if(`$INSTANCE_NAME`_EP_MM == `$INSTANCE_NAME`__EP_DMAAUTO)
                        cur_ep |= 0x01u << (ep - `$INSTANCE_NAME`_EP1);
                    #endif /* End `$INSTANCE_NAME`_EP_MM == `$INSTANCE_NAME`__EP_DMAAUTO */
            ...
            `$INSTANCE_NAME`_EP_TYPE_REG = cur_ep;
...
*
*******************************************************************************/
void `$INSTANCE_NAME`_ConfigAltChanged()
{
    uint8 ep,cur_ep,i;
    uint8 iso;
    uint8 ri;
        ...
        for (i = 0u; i < ep; i++, pEP++)
        {
               ...
               (`$INSTANCE_NAME`_interfaceSetting[pEP->interface] == pEP->altSetting) )
            {
                ...
                if (pEP->addr & `$INSTANCE_NAME`_DIR_IN)
                {
                    ...
                    `$INSTANCE_NAME`_EP[cur_ep].epMode = \
                                            (iso ? `$INSTANCE_NAME`_MODE_ISO_IN : `$INSTANCE_NAME`_MODE_ACK_IN);
                    /* Change the SIE mode for the selected EP to NAK  */
                ...
                #if((`$INSTANCE_NAME`_EP_MM == `$INSTANCE_NAME`__EP_DMAAUTO) && (CY_PSOC3_ES3 || CY_PSOC5_ES2) )
...
*
*******************************************************************************/
void `$INSTANCE_NAME`_TerminateEP(uint8 ep)
{
    uint8 ri;
