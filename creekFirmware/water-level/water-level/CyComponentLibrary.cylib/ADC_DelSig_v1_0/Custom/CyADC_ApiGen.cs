...
void SetRef(uint refMode, uint inputMode)
{
    ...
    {
        DsmRef0Reg = (rc.DSM_REFMUX_VDA_4 | rc.DSM_EN_BUF_VREF_INN | rc.DSM_VCM_RES_DIV_EN);
        switch (refMode)
        {
            case rc.ADC_INT_REF_NOT_BYPASSED:
                DsmRef2Reg = rc.DSM_REF2_S3_EN | rc.DSM_REF2_S4_EN | rc.DSM_REF2_S6_EN;
...
//
//*****************************************************************************
uint SetClock(uint resolution, uint sampleRate, uint conversionMode)
{
    uint resIndex;
    uint clockFrequency;
    ...
    switch (conversionMode)
    {
        case rc.ADC_CM_SINGLE_SAMPLE:
            clocksPerSample = (((resSettings.dr1[resIndex]+1) * 4) + 1) * (resSettings.dr2[resIndex]+1);
            break;
    ...
    // Note:
    // Should check for max and min ADC clock.
    // If ADC clock out of range, flag an error
...
//*****************************************************************************
     ...
     //       desquery.
            ...
            DecCrReg    = (Conversion_Mode << 2) | (Start_of_Conversion << 1) | rc.DEC_FIR_EN;
            ...
            paramDict.Add(clocksPerSample_ParamName, "0x" + clocksPerSample.ToString("X4") + "u");

        }
    }

    public abstract class resSettings
    {
            ...
            //  Clocks     
            //public static uint[] ssModeClks =   { 67,  67,  67, 131, 131,  131, 131, 259,  259,  259,  259,  515,   515,  515 };
            //public static uint[] ffltModeClks = { 67,  67,  67, 131, 131,  131, 131, 259,  259, 1036, 4144, 32960, 65920, 65920 };
            //public static uint[] contModeClks = { 16,  16,  16,  32,  32,   32,  32,  64,   64,   64,   64,  128,   128,   128 };
            //public static uint[] ffirModeClks = { 64,  64,  64, 128, 128,  128, 128, 256,  256,  448, 1216, 8576,  16768, 16768 };
    }
