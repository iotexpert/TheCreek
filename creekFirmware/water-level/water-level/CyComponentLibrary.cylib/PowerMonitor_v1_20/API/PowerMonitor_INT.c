...
static `$INSTANCE_NAME`_ADC_RAWVOLTS_STRUCT CYXDATA `$INSTANCE_NAME`_voltFilt[`$INSTANCE_NAME`_NUMBER_OF_CONVERTERS];
...
CY_ISR(`$INSTANCE_NAME`_ISR)
{
    ...
    */
    ...
    else if (`$INSTANCE_NAME`_adcConvNow == `$INSTANCE_NAME`_lastVoltageChan)
    {
        ...
        #elif (`$INSTANCE_NAME`_NUM_AUX_INPUTS > 0u)
            `$INSTANCE_NAME`_index = 0u;
            /* Select the proper channel */
            `$INSTANCE_NAME`_PM_AMux_Voltage_FastSelect(`$INSTANCE_NAME`_AUX_IN_CHAN);
            if (`$INSTANCE_NAME`_AuxVoltageType[0u] != `$INSTANCE_NAME`_AUX_VOLTAGE_SINGLE)
            {
                `$INSTANCE_NAME`_PM_AMux_Current_FastSelect(`$INSTANCE_NAME`_AUX_IN_CHAN);
                /* Set the proper ADC configuration */
                if ((`$INSTANCE_NAME`_AuxVoltageType[0u] == `$INSTANCE_NAME`_AUX_VOLTAGE_64MV_DIFF) ||
                    (`$INSTANCE_NAME`_AuxVoltageType[0u] == `$INSTANCE_NAME`_AUX_VOLTAGE_128MV_DIFF))
                {
                    `$INSTANCE_NAME`_ADC_CONFIG_PUSH_NEW(`$INSTANCE_NAME`_RANGE_LOW);
                }
                else
            ...
            `$INSTANCE_NAME`_adcConvNext = `$INSTANCE_NAME`_ISR_STATE_0 + 1u;
             /* Set the proper AMux Channel */
            `$INSTANCE_NAME`_PM_AMux_Voltage_FastSelect(`$INSTANCE_NAME`_adcConvNow);
            /* Update AmuxChan flag */
            `$INSTANCE_NAME`_amuxChan = `$INSTANCE_NAME`_adcConvNow;
            if (`$INSTANCE_NAME`_VoltageType[0u] == `$INSTANCE_NAME`_VOLTAGE_TYPE_DIFF)
            {
                `$INSTANCE_NAME`_PM_AMux_Current_FastSelect(`$INSTANCE_NAME`_amuxChan);
                /* Set the proper ADC configuration */
                `$INSTANCE_NAME`_ADC_CONFIG_PUSH_NEW(`$INSTANCE_NAME`_RANGE_2048);
        ...
        else if (`$INSTANCE_NAME`_adcConvNow == `$INSTANCE_NAME`_lastCurrentChan)
        {
                ...
                if (`$INSTANCE_NAME`_VoltageType[0u] == `$INSTANCE_NAME`_VOLTAGE_TYPE_SINGLE)
                {
                    `$INSTANCE_NAME`_PM_AMux_Current_FastSelect(`$INSTANCE_NAME`_PGA_OUT_CHAN);
                    `$INSTANCE_NAME`_ADC_CONFIG_PUSH_NEW(`$INSTANCE_NAME`_SE_VOLTAGE_RANGE);
                }
    ...
    #endif /* (`$INSTANCE_NAME`_NUM_CURRENT_SOURCE > 0u) */
    #if (`$INSTANCE_NAME`_NUM_AUX_INPUTS > 0u)
        ...
        switch (`$INSTANCE_NAME`_adcConvCalType)
        {
            case `$INSTANCE_NAME`_CAL_START:
                ...
                *******************************************************************/
                if (`$INSTANCE_NAME`_VoltageType[0u] == `$INSTANCE_NAME`_VOLTAGE_TYPE_SINGLE)
                {
                    `$INSTANCE_NAME`_ADC_SelectConfiguration(`$INSTANCE_NAME`_SE_VOLTAGE_RANGE, 0u);
                    /* Initialize local selection memory    */
                    `$INSTANCE_NAME`_adcConfig = `$INSTANCE_NAME`_SE_VOLTAGE_RANGE; 
                    `$INSTANCE_NAME`_adcConvDoneConfig = `$INSTANCE_NAME`_SE_VOLTAGE_RANGE;
                }
                else
                {
                    `$INSTANCE_NAME`_ADC_SelectConfiguration(`$INSTANCE_NAME`_RANGE_2048, 0u);
                    /* Initialize local selection memory    */
        ...
        if (convDone < `$INSTANCE_NAME`_lastVoltageChan)
        {
            ...
            else
            {
                ...
                pAdcFilt = &`$INSTANCE_NAME`_auxVoltFilt[convDone - `$INSTANCE_NAME`_lastCurrentChan].rawVoltage[Idx];
                    ...
                    psCtl->idx = 0u;
                #else
                    psCtl->idx = (Idx < `$INSTANCE_NAME`_AUX_VOLTAGE_FILTER_SIZE-1) ? (Idx+1u) : 0u;
    ...
    ************************************************************************/
    ...
    if(`$INSTANCE_NAME`_PMoutEn == `$INSTANCE_NAME`_CYTRUE)
    {
        ...
        /* Generate EOC output */
            ...
            if (convDone == (`$INSTANCE_NAME`_MAX_CHANNELS - 1u))
            {
