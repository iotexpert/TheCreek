    ...
    public partial class CyPWMControlAdv : UserControl
    {
        ...
        public void UpdateFormFromParams(ICyInstEdit_v1 inst)
        {
            ...
            foreach (string str in KillModeEnums)
            {
                if (!Killfound)
                {
                    string paramcompare = m_Component.ResolveEnumIdToDisplay("KillMode", prms.KillMode.Expr);
                    if (paramcompare == str)
                    {
            ...
            {
                case "true": m_rbFixedFunction.Checked = true; SetFixedFunction(); break;
                case "false": m_rbUDB.Checked = true; ClearFixedFunction(); break;
            }

            m_chbxIntOnTC.Checked = (prms.InterruptOnTC.Value == "true");
            if (prms.KillMode.Value != "0")
            {
        ...
        private void SetFixedFunction()
        {
            ...
            IEnumerable<string> PWMModesEnums = m_Component.GetPossibleEnumValues("PWMMode");
            ...
            string prm = m_Component.ResolveEnumDisplayToId("PWMMode", pwmmodeff);
            SetAParameter("PWMMode", prm); //Set to One Output

            string trigmodeff = null;
            IEnumerable<string> TrigModesEnums = m_Component.GetPossibleEnumValues("TriggerMode");
            ...
            prm = m_Component.ResolveEnumDisplayToId("TriggerMode", trigmodeff);
            SetAParameter("TriggerMode", prm); //Set Trigger Mode to none

            string DBmodeff = null;
            IEnumerable<string> DBModesEnums = m_Component.GetPossibleEnumValues("DeadBand");
            foreach (string str in DBModesEnums)
            {
                if (str.Contains("Disabled"))
                    DBmodeff = str;
            }
            prm = m_Component.ResolveEnumDisplayToId("DeadBand", DBmodeff);
            SetAParameter("DeadBand", prm);  //Set Dead Band to Disabled.


            string killmodeff = null;
            IEnumerable<string> KillModesEnums = m_Component.GetPossibleEnumValues("KillMode");
            ...
            prm = m_Component.ResolveEnumDisplayToId("KillMode", killmodeff);
            SetAParameter("KillMode", prm);
            
            string capmodeff = null;
            IEnumerable<string> CapModesEnums = m_Component.GetPossibleEnumValues("CaptureMode");
            ...
            prm = m_Component.ResolveEnumDisplayToId("CaptureMode", capmodeff);
            ...
            prm = m_Component.ResolveEnumDisplayToId("EnableMode", enablemodeff);
            SetAParameter("EnableMode", prm);

            string runmodeff = null;
            IEnumerable<string> RunModesEnums = m_Component.GetPossibleEnumValues("RunMode");
            ...
            prm = m_Component.ResolveEnumDisplayToId("RunMode", runmodeff);
        ...
        {
            string prm = m_Component.ResolveEnumDisplayToId("KillMode", m_cbKillMode.Text);
            SetAParameter("KillMode", prm);
        ...
        private void UpdateBasicForm()
        {
            if (m_control_basic != null)
                //m_control_basic.UpdateFormFromParams(m_Component);
                m_control_basic.MyUpdate();
        }
        private void UpdateDrawing()
        {
            if (m_control_basic != null)
                m_control_basic.UpdateDrawing(new PWMParameters(m_Component));
        }
