...
namespace USBFS_v2_40
{
    public partial class CyUSBFSParameters
    {
        ...
        public static bool IsHidItemUnsigned(string name)
        {
            bool res = false;
            if (name == RPTITEM_REPORT_COUNT || name == RPTITEM_REPORT_ID || name == RPTITEM_REPORT_SIZE)
            {
                res = true;
            }
            return res;
        ...
        public bool CheckSiliconRevisionForDMA(ICyDeviceQuery_v1 deviceQuery, out string selOption)
        {
            ...
            if (!supportDMA)
            {
                if (EPMemoryMgmt == (byte)CyMemoryManagement.DMA_AUTOMATIC_MEMORY) // if DMA Automatic
                {
                    res = false;
        ...
        public bool CheckEPBulkMaxPacketSize(CyDescriptorNode node)
        {
            ...
            if ((node.m_value != null) && (node.m_value.bDescriptorType == CyUSBDescriptorType.ENDPOINT))
            {
                List<ushort> possibleMaxPacketValues = new List<ushort>(new ushort[] { 8, 16, 32, 64 }); 
                CyEndpointDescriptor desc = (CyEndpointDescriptor)node.m_value;
                if ((GetEPTransferType(desc) == TRANSFERTYPE_BULK) &&
                    (possibleMaxPacketValues.Contains(desc.wMaxPacketSize) == false))
                {
                    return false;
                }
            }
        ...
        {
            // "USBFS_v2_40" should be replaced with other version after upgrade 
            DirectoryInfo di = new DirectoryInfo(
                    "..\\psoc\\content\\cycomponentlibrary\\CyComponentLibrary.cylib\\USBFS_v2_40\\Custom\\template");
            return di.FullName;
        }
    ...
    public class CyHidReportItem
    {
            ...
            : this (name, prefix, size, CyHidReportItemKind.NONE)
        {
            ...
            {
                byteList = CyUSBFSParameters.ConvertIntToByteArray(value, CyUSBFSParameters.IsHidItemUnsigned(name));
            }
            else
            {
