...
namespace USBFS_v2_40
{
    public partial class CyDetailsEndpoint : UserControl
    {
        ...
        private void InitFields()
        {
            ...
            switch (CyUSBFSParameters.GetEPTransferType(m_descriptor))
            {
        ...
        {
            List<int> possibleMaxPacketValues = new List<int>(new int[] {8, 16, 32, 64}); 
                ...
                (numUpDownMaxPacketSize.Value > 512))
            {
                errProvider.SetError(numUpDownMaxPacketSize, Properties.Resources.ERR_EP_MAXPACKETSIZE);
                     ...
                     (possibleMaxPacketValues.Contains((int)numUpDownMaxPacketSize.Value) == false))
            {
