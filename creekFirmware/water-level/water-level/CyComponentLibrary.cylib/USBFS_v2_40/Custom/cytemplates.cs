...
namespace USBFS_v2_40
{
    ...
    {
        private enum CyInterfaceType { AUDIO, MIDI, CDC, GENERAL };
        ...
        private void BuildDescriptorTemplateList(CyDescriptorNode desc, List<CyDescriptorTemplate> lst)
        {
            if (desc.m_value != null)
            {
                ...
                else if (desc.m_value is CyMSEndpointDescriptor)
                    lst.Add(new CyMSEndpointDescriptorTemplate(desc.m_value));
                else if (desc.m_value is CyMSHeaderDescriptor)
                    lst.Add(new CyMSHeaderDescriptorTemplate(desc.m_value));
                else if (desc.m_value is CyMSInJackDescriptor)
                    lst.Add(new CyMSInJackDescriptorTemplate(desc.m_value));
                else if (desc.m_value is CyMSOutJackDescriptor)
                    lst.Add(new CyMSOutJackDescriptorTemplate(desc.m_value));
                else if (desc.m_value is CyMSElementDescriptor)
                    lst.Add(new CyMSElementDescriptorTemplate(desc.m_value));
        ...
        public bool ImportDescriptorList(CyUSBDescriptorType kind)
        {
            ...
            if (lst.Count > 0)
            {
                ...
                {
                    CyInterfaceType intType = CyInterfaceType.CDC;
                    bool isAudio = lst[0] is CyAudioInterfaceDescriptor;
                    bool isMIDI = (kind == CyUSBDescriptorType.MIDI); 
                                        /*(isAudio && (((CyAudioInterfaceDescriptor)lst[0]).bInterfaceSubClass == 
                                         (byte)CyUSBOtherTypes.CyAudioSubclassCodes.MIDISTREAMING));*/
                    if (isMIDI) intType = CyInterfaceType.MIDI;
                    else if (isAudio) intType = CyInterfaceType.AUDIO;
                    ...
                    CyDescriptorNode subTree = BuildAudioCDCTreeFromDesc(lst, 0, tree.m_nodes[0].Key, intType);
                    res = true;
                }
            }
            return res;
            ...
            if (node.m_value.bDescriptorType == CyUSBDescriptorType.ALTERNATE)
            {
                ...
                if (((CyInterfaceDescriptor)lst[index]).bAlternateSetting == 0)
                {
                    ...
                    if (lst[index] is CyAudioInterfaceDescriptor)
                    {
                        ...
                        //if (((CyAudioInterfaceDescriptor)lst[index]).bInterfaceSubClass == 
                        // (byte)CyUSBOtherTypes.CyAudioSubclassCodes.MIDISTREAMING)
                        //{
                        //    // add to MIDI tree
                        //    if (!m_midiTree.m_nodes[0].m_nodes.Contains(node))
                        //        m_midiTree.m_nodes[0].m_nodes.Add(node);
                        //}
                        //else
                        //{
                        //    // add to AUDIO tree
                ...
                else
                {
                    ...
                    if (lst[index] is CyAudioInterfaceDescriptor)
                    {
                        AddAudioInfToBuffer(configNode.m_nodes[ind], parentKey);
        ...
        private void SortAudioInfFromBuffer()
        {
            foreach (List<CyDescriptorNode> infNodeList in m_importDescList) // Configurations
            {
                ...
                for (int k = 0; k < infNodeList.Count; k++) // General Interfaces
                {
                    ...
                    for (int i = 0; i < infNode.m_nodes.Count; i++) // Alternate Interfaces
                    {
                        if (((CyInterfaceDescriptor)infNode.m_nodes[i].m_value).bInterfaceSubClass == 
                            (byte)CyUSBOtherTypes.CyAudioSubclassCodes.AUDIOSTREAMING)
                        {
                            haveAS = true;
                            haveAltAS = true;
                        }
                        else if (((CyInterfaceDescriptor)infNode.m_nodes[i].m_value).bInterfaceSubClass ==
                            (byte)CyUSBOtherTypes.CyAudioSubclassCodes.MIDISTREAMING)
                        {
                            haveMS = true;
                            haveAltMS = true;
                ...
                for (int k = 0; k < infNodeList.Count; k++) // General Interfaces
                {
                    ...
                    {
                        m_audioTree.m_nodes[0].m_nodes.Add(infNodeList[k]);
                    }
                    else if (infTypes[k] == CyInterfaceType.MIDI)
                    {
                        m_midiTree.m_nodes[0].m_nodes.Add(infNodeList[k]);
                    }
                    else
                    {
                        if (!haveMS)
                                                           ...
                                                           CyInterfaceType interfaceType)
        {
            ...
            if ((parentKey == NODEKEY_AUDIO) || (parentKey == NODEKEY_CDC) || ((parentKey == NODEKEY_MIDI))) 
            {
            ...
            {
                BuildAudioCDCTreeFromDesc(lst, childIndex, tree.m_nodes[0].Key, interfaceType);
            }
            else
            {
                while ((childIndex < lst.Count) && (lst[childIndex].GetLevel() > levelParent))
                {
                    if (lst[childIndex].GetLevel() == levelParent + 1)
                    {
                        BuildAudioCDCTreeFromDesc(lst, childIndex, node.Key, interfaceType);
                    }
                    childIndex++;
                }
            }
            ...
            for (int j = 0; j < node_report.m_nodes.Count; j++)
            {
                ...
                if ((item.Kind == CyHidReportItemKind.INT) || (item.Kind == CyHidReportItemKind.CUSTOM))
                {
                    val = CyUSBFSParameters.ConvertByteArrayToInt(byteList, 
                                    CyUSBFSParameters.IsHidItemUnsigned(item.Name));
                }
                else
                {
