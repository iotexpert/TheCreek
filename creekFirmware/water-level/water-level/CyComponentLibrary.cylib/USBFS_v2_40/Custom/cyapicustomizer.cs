...
namespace USBFS_v2_40
{
    ...
    internal class CyUSBCodeGenerator : CyUsbCodePrimitives
    {
        ...
        private string GenReportTable(int rptIndex, string instName, string qualName)
        {
            ...
            if ((rptIndex > 0) && (rptIndex <= m_parameters.m_hidReportTree.m_nodes[0].m_nodes.Count))
            {
                ...
                int inputReportsCount = CyHidReportID.GetCount(reports, CyUSBFSParameters.RPTITEM_INPUT);
                int outputReportsCount = CyHidReportID.GetCount(reports, CyUSBFSParameters.RPTITEM_OUTPUT);
                int featureReportsCount = CyHidReportID.GetCount(reports, CyUSBFSParameters.RPTITEM_FEATURE);
                m_maxReportID = CyHidReportID.GetMaxReportID(reports);

                code.AppendFormat("#if !defined(USER_DEFINE_{0}_HID_RPT_STORAGE)\r\n", qualName);
                if (CyHidReportID.GetCount(reports, CyUSBFSParameters.RPTITEM_INPUT) > 0)
                {
                    code.AppendLine("/*********************************************************************");
                    ...
                    for (int i = 0; i < reports.Count; i++)
                    {
                        if (reports[i].m_inputSize > 0)
                        {
                            ...
                            code.AppendLine(GenerateAllocateArray(qualName + "_IN_BUF" + idStr, 
                                                                  qualName + "_IN_BUF_SIZE" + idStr));
                            AddDefineNoInstance(qualName + "_IN_BUF_SIZE" + idStr, reports[i].m_inputSize.ToString());
                        }
                    }
                    ...
                    for (int i = 0; i <= CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_INPUT); i++)
                    {
                        CyHidReportID rpt = CyHidReportID.ReportWithID(reports, (byte)i, 
                                                                       CyUSBFSParameters.RPTITEM_INPUT);
                        if (rpt != null)
                        {
                            string idStr = rpt.GetIDString();
                            code.Append(GenerateTDEntry(qualName + "_IN_BUF_SIZE" + idStr,
                                            qualName + "_IN_BUF" + idStr + "[0]", qualName + "_IN_RPT_SCB" + idStr));
                        }
                        else
                        {
                            code.Append(GenerateTDEntry(NULL_PTR, NULL_PTR, NULL_PTR));
                        }
                        code.Append(COMMA_NEWLINE);
                    }
                   
                    code.AppendFormat("{0};{1}", GenerateFooter(), NEWLINE);
                    AddDefineNoInstance(qualName + "_NUM_IN_RPTS", inputReportsCount.ToString());
                }

                if (CyHidReportID.GetCount(reports, CyUSBFSParameters.RPTITEM_OUTPUT) > 0)
                {
                    code.AppendLine("/*********************************************************************");
                    ...
                    for (int i = 0; i < reports.Count; i++)
                    {
                        if (reports[i].m_outputSize > 0)
                        {
                            ...
                            code.AppendLine(GenerateAllocateArray(qualName + "_OUT_BUF" + idStr, 
                                                                  qualName + "_OUT_BUF_SIZE" + idStr));
                            AddDefineNoInstance(qualName + "_OUT_BUF_SIZE" + idStr, reports[i].m_outputSize.ToString());
                        }
                    }
                    code.AppendLine("/*********************************************************************");
                    ...
                    for (int i = 0; i <= CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_OUTPUT); i++)
                    {
                        CyHidReportID rpt = CyHidReportID.ReportWithID(reports, (byte)i, 
                                                                       CyUSBFSParameters.RPTITEM_OUTPUT);
                        if (rpt != null)
                        {
                            string idStr = rpt.GetIDString();
                            code.Append(GenerateTDEntry(qualName + "_OUT_BUF_SIZE" + idStr,
                                            qualName + "_OUT_BUF" + idStr + "[0]", qualName + "_OUT_RPT_SCB" + idStr));
                        }
                        else
                        {
                            code.Append(GenerateTDEntry(NULL_PTR, NULL_PTR, NULL_PTR));
                    ...
                    code.AppendFormat("{0};{1}", GenerateFooter(), NEWLINE);
                    AddDefineNoInstance(qualName + "_NUM_OUT_RPTS", outputReportsCount.ToString());
                }

                if (CyHidReportID.GetCount(reports, CyUSBFSParameters.RPTITEM_FEATURE) > 0)
                {
                    code.AppendLine("/*********************************************************************");
                    ...
                    for (int i = 0; i < reports.Count; i++)
                    {
                        if (reports[i].m_featureSize > 0)
                        {
                            ...
                            code.AppendLine(GenerateAllocateArray(qualName + "_FEATURE_BUF" + idStr, 
                                                                  qualName + "_FEATURE_BUF_SIZE" + idStr));
                            AddDefineNoInstance(qualName + "_FEATURE_BUF_SIZE" + idStr, 
                                                reports[i].m_featureSize.ToString());
                        }
                    }
                    code.AppendLine("/*********************************************************************");
                    ...
                    for (int i = 0; i <= CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_FEATURE); i++)
                    {
                        CyHidReportID rpt = CyHidReportID.ReportWithID(reports, (byte)i, 
                                                                       CyUSBFSParameters.RPTITEM_FEATURE);
                        if (rpt != null)
                        {
                            string idStr = rpt.GetIDString();
                            code.Append(GenerateTDEntry(qualName + "_FEATURE_BUF_SIZE" + idStr,
                                     qualName + "_FEATURE_BUF" + idStr + "[0]", qualName + "_FEATURE_RPT_SCB" + idStr));
                        }
                        else
                        {
                            code.Append(GenerateTDEntry(NULL_PTR, NULL_PTR, NULL_PTR));
                    ...
                    code.AppendFormat("{0};{1}", GenerateFooter(), NEWLINE);
                    AddDefineNoInstance(qualName + "_NUM_FEATURE_RPTS", featureReportsCount.ToString());
                }
                ...
                if (reports.Count > 0 && reports[0].m_hasID)
                {
                    ...
                    {
                        maxID = CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_INPUT);
                    ...
                    {
                        maxID = CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_OUTPUT);
                    ...
                    {
                        maxID = CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_FEATURE);
                ...
                if (inputReportsCount > 0)
                {
                    code.Append(GenerateLUTEntry(qualName + "_IN_RPT_TABLE", 
                        (byte)(CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_INPUT))));
                }
                else
                {
                ...
                if (outputReportsCount > 0)
                {
                    code.Append(GenerateLUTEntry(qualName + "_OUT_RPT_TABLE", 
                        (byte)(CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_OUTPUT))));
                }
                else
                {
                    code.Append(GenerateNullLUTEntry());
                ...
                if (featureReportsCount > 0)
                {
                    code.Append(GenerateLUTEntry(qualName + "_FEATURE_RPT_TABLE", 
                        (byte)(CyHidReportID.GetMaxReportID(reports, CyUSBFSParameters.RPTITEM_FEATURE))));
                }
                else
                {
                    code.Append(GenerateNullLUTEntry());
        ...
        private static List<CyHidReportID> GetReportIDList(CyDescriptorNode nodeRpt)
        {
            ...
            for (int i = 0; i < nodeRpt.m_nodes.Count; i++)
            {
                CyDescriptorNode node = nodeRpt.m_nodes[i];
                if (node.m_value != null)
                {
                    if (node.m_value.bDescriptorType == CyUSBDescriptorType.HID_REPORT_ITEM)
                    {
                        ...
                        if (h.Item.Name == CyUSBFSParameters.RPTITEM_REPORT_ID)
                        {
                            if (h.Item.m_value.Count >= 2)
                            {
                                ...
                                currentReport.m_inputSize =
                                    (ushort)ReportSize(nodeRpt, CyUSBFSParameters.RPTITEM_INPUT, currentReport);
                                currentReport.m_outputSize =
                                    (ushort)ReportSize(nodeRpt, CyUSBFSParameters.RPTITEM_OUTPUT, currentReport);
                                currentReport.m_featureSize =
                                    (ushort)ReportSize(nodeRpt, CyUSBFSParameters.RPTITEM_FEATURE, currentReport);
    ...
    public class CyUsbCodePrimitives
    {
        ...
        public static ushort CalcHIDItemLength(CyHIDReportItemDescriptor d)
        {
    ...
    internal class CyUsbHIDReportDescriptorGenTree : CyGenTree
    {
        ...
        private ushort ReportSize(List<CyDescriptorNode> nodes)
        {
            ushort size = 0;
            for (int i = 0; i < nodes.Count; i++)
            {
                CyHIDReportItemDescriptor h = (CyHIDReportItemDescriptor) nodes[i].m_value;
                size += CyUsbCodePrimitives.CalcHIDItemLength(h);
            }
            return size;
        }
    }
    ...
    internal class CyHidReportID
    {
        ...
        public static int GetCount(List<CyHidReportID> reports, string name)
        {
            ...
            for (int i = 0; i < reports.Count; i++)
			{
                if (reports[i].m_inputSize > 0 && name == CyUSBFSParameters.RPTITEM_INPUT)
                    count++;
                else if (reports[i].m_outputSize > 0 && name == CyUSBFSParameters.RPTITEM_OUTPUT)
                    count++;
                else if (reports[i].m_featureSize > 0 && name == CyUSBFSParameters.RPTITEM_FEATURE)
                    count++;
			}
            return count;
        }
        ...
        public static byte GetMaxReportID(List<CyHidReportID> reports, string name)
        {
            ...
            for (int i = 0; i < reports.Count; i++)
            {
                if (name == CyUSBFSParameters.RPTITEM_INPUT && reports[i].m_inputSize > 0 && reports[i].m_id > max)
                    max = reports[i].m_id;
                else if (name == CyUSBFSParameters.RPTITEM_OUTPUT && reports[i].m_outputSize > 0 && reports[i].m_id > max)
                    max = reports[i].m_id;
                else if (name == CyUSBFSParameters.RPTITEM_FEATURE && reports[i].m_featureSize > 0 && reports[i].m_id > max)
                    max = reports[i].m_id;
            }
            return max;
        }
        ...
        public static CyHidReportID ReportWithID(List<CyHidReportID> reports, byte id, string name)
        {
            ...
            for (int i = 0; i < reports.Count; i++)
            {
                ...
                {
                    if ((name == CyUSBFSParameters.RPTITEM_INPUT && reports[i].m_inputSize > 0) ||
                        (name == CyUSBFSParameters.RPTITEM_OUTPUT && reports[i].m_outputSize > 0) ||
                        (name == CyUSBFSParameters.RPTITEM_FEATURE && reports[i].m_featureSize > 0))
                    {
                        report = reports[i];
                        break;
                    }
