    ...
    public partial class CyScanSlots
    {
        ...
        void ColLocationVisible()
        {
            ...
            foreach (CyColLocation item in m_massColLocation)
            {
                ...
                {
                    if (m_packParams.m_localParams.BCsHalfIsEnable(ab))
        ...
        void AssigneActions()
        {
            ...
            //Modify Splitter
            m_packParams.m_cyButtons.Load += new EventHandler(cyButtons_Load);
            m_packParams.m_cySliders.Load += new EventHandler(cySliders_Load);
            m_packParams.m_cyTouchPads.Load += new EventHandler(cyTouchPads_Load);
            m_packParams.m_cyMatrixButtons.Load += new EventHandler(cyMatrixButtons_Load);
            m_packParams.m_cyProximity.Load += new EventHandler(cyProximity_Load);
        ...
        {
            m_packParams.m_localParams.m_internalProcess = true;
            ...
            m_packParams.m_localParams.m_internalProcess = false;
        }
        ...
        public E_SENSOR_TYPE[] GetWidegetTypes(DataGridView sender)
        {
            ...
            {
                return new E_SENSOR_TYPE[] 
                {E_SENSOR_TYPE.Matrix_Buttons_Row, E_SENSOR_TYPE.Matrix_Buttons_Col};
            }
            ...
            foreach (CyElWidget item in m_packParams.m_cyWidgetsList.getListWidgets())
            {
                switch (item.m_type)
                {
                    case E_SENSOR_TYPE.Button:
                        m_packParams.m_cyButtons.dgButtons.Rows.
                    ...
                    case E_SENSOR_TYPE.Proximity:
                        m_packParams.m_cyProximity.dgProximity.Rows.Add(new object[] { item.m_Name, 
                            ((CyElUnButton)item).m_Count, item, CyColLocation.getString(item.m_side) });
                        break;
                    case E_SENSOR_TYPE.Generic:
                        m_packParams.m_cyGeneric.dgGeneric.Rows.Add(new object[] { item.m_Name, 
                            ((CyElUnButton)item).m_Count, item, CyColLocation.getString(item.m_side) });
                        break;
                    default:
                        break;
        ...
        void AddTouchPadRow(CyElWidget item)
        {
            CyElUnTouchPad wiC = (CyElUnTouchPad)m_packParams.
                        m_cyWidgetsList.FindWidget(E_SENSOR_TYPE.Touchpad_Col, item.m_Name);
            CyElUnTouchPad wiR = (CyElUnTouchPad)m_packParams.
                        m_cyWidgetsList.FindWidget(E_SENSOR_TYPE.Touchpad_Row, item.m_Name);
            m_packParams.m_cyTouchPads.dgTouchpads.Rows.Add(new object[] { wiC.m_Name, wiC.m_numElRow, 
        ...
        void AddMatrizButtonRow(CyElWidget item)
        {
            CyElUnMatrixButton wiC = (CyElUnMatrixButton)m_packParams.m_cyWidgetsList.FindWidget
                                                                (E_SENSOR_TYPE.Matrix_Buttons_Col, item.m_Name);
            CyElUnMatrixButton wiR = (CyElUnMatrixButton)m_packParams.m_cyWidgetsList.FindWidget
                                                                (E_SENSOR_TYPE.Matrix_Buttons_Row, item.m_Name);
            m_packParams.m_cyMatrixButtons.dgMatrixButtons.Rows.Add(new object[] { wiC.m_Name, wiC.m_numElRow, 
        ...
        {            
            ...
            m_packParams.m_cySliders.dgSliders.Rows.Add(new object[] { item.m_Name, 
        ...
        {
            SpliterMove(m_packParams.m_cyProximity.splitContainer1, 130);//162);
        }

        void cyMatrixButtons_Load(object sender, EventArgs e)
        {
            SpliterMove(m_packParams.m_cyMatrixButtons.splitContainer1, 130);//162);      
        }

        void cyTouchPads_Load(object sender, EventArgs e)
        {
            SpliterMove(m_packParams.m_cyTouchPads.splitContainer1, 180);
        }

        void cySliders_Load(object sender, EventArgs e)
        {
            SpliterMove(m_packParams.m_cySliders.splitContainer1, 162);
        }

        void cyButtons_Load(object sender, EventArgs e)
        {
            SpliterMove(m_packParams.m_cyButtons.splitContainer1, 130);//162);
        }
        ...
        void dgDClickButton(object sender, DataGridViewCellMouseEventArgs e, E_SENSOR_TYPE[] type)
        {
            if (e.RowIndex != ((DataGridView)sender).RowCount - 1)//not last
            {
                ...
                {
                    CyElWidget[] curWi = m_packParams.m_cyWidgetsList.GetWidget
           ...
           {
               dgDClickButton(sender, e, new E_SENSOR_TYPE[] 
               {E_SENSOR_TYPE.Touchpad_Row, E_SENSOR_TYPE.Touchpad_Col});
           }
            ...
            System.Windows.Forms.DataGridViewCellCancelEventArgs e, bool twoHalf)
        {
            ...
            if (twoHalf)
            {
                ...
                if (m_packParams.Configuration == E_MAIN_CONFIG.emParallelAsynchron)
                {
                    ...
                    {
                        if(!m_packParams.m_localParams.m_internalProcess)
                        MessageBox.Show(
        ...
        void dgEnter(object sender, System.Windows.Forms.DataGridViewCellEventArgs e)
        {
            ...
            if ((t_cRow != -1) && (t_cRow < ((DataGridView)sender).RowCount - 1))
            {
                ...
                if (sender == m_dgButtons)
                {
                    m_packParams.m_cyButtons.UnitPropertyGrid.GetProperties(m_packParams.
                        m_cyWidgetsList.GetWidget(name, E_SENSOR_TYPE.Button));
                    //lastType = sensorType.Buttons;
                }
                else if (sender == m_dgSliders)
                {
                    m_packParams.m_cySliders.UnitPropertyGrid.GetProperties(m_packParams.
                        m_cyWidgetsList.GetWidget(name, E_SENSOR_TYPE.Linear_Slider));
                    //lastType = sensorType.Linear_Slider;
                }
                else if (sender == m_dgTouchpads)
                {
                    m_packParams.m_cyTouchPads.UnitPropertyGrid.GetProperties(m_packParams.
                        m_cyWidgetsList.GetWidget(name, E_SENSOR_TYPE.Touchpad_Col));
                    //lastType = sensorType.Touchpads_Col;
                }
                else if (sender == m_dgMatrixButtons)
                {
                    m_packParams.m_cyMatrixButtons.UnitPropertyGrid.GetProperties(m_packParams.
                        m_cyWidgetsList.GetWidget(name, E_SENSOR_TYPE.Matrix_Buttons_Col));
                    //lastType = sensorType.Matrix_Buttons_Col;
                }
                else if (sender == m_dgProximity)
                {
                    m_packParams.m_cyProximity.UnitPropertyGrid.GetProperties(m_packParams.
                        m_cyWidgetsList.GetWidget(name, E_SENSOR_TYPE.Proximity));
        ...
        void FreeUnitPropertyGrid(object sender)
        {
            ...
            if (sender == m_dgButtons)
            {
                m_packParams.m_cyButtons.UnitPropertyGrid.GetProperties(null);
            }
            else if (sender == m_dgSliders)
            {
                m_packParams.m_cySliders.UnitPropertyGrid.GetProperties(null);
            }
            else if (sender == m_dgTouchpads)
            {
                m_packParams.m_cyTouchPads.UnitPropertyGrid.GetProperties(null);
            }
            else if (sender == m_dgMatrixButtons)
            {
                m_packParams.m_cyMatrixButtons.UnitPropertyGrid.GetProperties(null);
            }
            else if (sender == m_dgProximity)
            {
                m_packParams.m_cyButtons.UnitPropertyGrid.GetProperties(null);
