...
namespace CapSense_v1_10.API
{
    public partial class CyAPIGenerator
    {
        ...
        void ApiCollectCHLVariables(ref CyMyStringWriter writerResult, CyGeneralParams packParams)
        {
            ...
            int ArrayCaseWidgetsCount = packParams.m_cyWidgetsList.GetCountDoubleFullWidgets();
            ...
            foreach (CyElWidget wi in packParams.m_cyWidgetsList.GetListWithFullWidgetsHL())
            {
                for (int i = 0; i < packParams.m_cyWidgetsList.GetBothParts(wi).Count; i++)
                {
                    ...
                    {
                        writer.Write(((CyElUnTouchPad)part).GetSeparateProps().FingerThreshold);//FigerThresholds;
                        writer.Write(", ");
                        writer.Write(((CyElUnTouchPad)part).GetSeparateProps().NoiseThreshold);// NoiseThresholds;
                        writer.Write(", ");
                    ...
                    if ((props.GetFilterMask() != 0) || (!CySensorType.IsButtonsStrc(part.GetType())))
                        writer.Write("&" + m_instanceName + "_" + CyElWidget.GetPrefixFull(part.GetType()) + "SettingsTable[" + packParams.m_cyWidgetsList.GetIndexInTypedListHL(part) + "]");
                    else
                        writer.Write("0x00");
                    writer.Write("}");
                    //Write to Array
            ...
            foreach (Type itemType in CyWidgetsList.GetListTypes())
            {
                ...
                if ((listWi.Count > 0) && ((!CySensorType.IsButtonsStrc(itemType)) || (packParams.m_cyWidgetsList.GetCountFilterOnHL(itemType) != 0)))
                {
                    writer.WriteLine(m_instanceName + "_" + CyElWidget.GetPrefixFull(itemType) + "Settings " + m_instanceName + "_" + CyElWidget.GetPrefixFull(itemType) +
                    ...
                    foreach (CyElWidget item in listWi)
                    {
                        ...
                        else //Not ElUnButton type
                        {
                            ...
                            E_ENE_DIS isJitterRD = (E_ENE_DIS)Enum.Parse(typeof(E_ENE_DIS), props.GetPropertyByName(typeof(CyHAFilterPropertiesRwDt), props.m_FilterPropertiesRwDt.JitterFilterRwDt));
                            E_ENE_DIS isMedianRD = (E_ENE_DIS)Enum.Parse(typeof(E_ENE_DIS), props.GetPropertyByName(typeof(CyHAFilterPropertiesRwDt), props.m_FilterPropertiesRwDt.MedianFilterRwDt));
                            E_ENE_DIS isAveragingRD = (E_ENE_DIS)Enum.Parse(typeof(E_ENE_DIS), props.GetPropertyByName(typeof(CyHAFilterPropertiesRwDt), props.m_FilterPropertiesRwDt.AveragingFilterRwDt));
                            ...
                            {
                                E_ENE_DIS isJitterPos = (E_ENE_DIS)Enum.Parse(typeof(E_ENE_DIS), props.GetPropertyByName(typeof(CyHAFilterPropertiesPos), props.m_FilterPropertiesPos.JitterFilterPos));
                                E_ENE_DIS isMedianPos = (E_ENE_DIS)Enum.Parse(typeof(E_ENE_DIS), props.GetPropertyByName(typeof(CyHAFilterPropertiesPos), props.m_FilterPropertiesPos.MedianFilterPos));
                                E_ENE_DIS isAveragingPos = (E_ENE_DIS)Enum.Parse(typeof(E_ENE_DIS), props.GetPropertyByName(typeof(CyHAFilterPropertiesPos), props.m_FilterPropertiesPos.AveragingFilterPos));
                                ArrayLength = 1;
        ...
        public void ApiCollectCHLFunctionForSide(ref CyMyStringWriter writer, CyGeneralParams packParams, CyAmuxBParams sbItem)
        {
            ...
            writer.WriteLine(" /*-----------------------------------------------------------------------------");
            writer.WriteLine(" * FUNCTION NAME: void " + m_instanceName + "_CSHL_InitializeSlotBaseline" + sSide + "(uint8 slot)");
            writer.WriteLine(" *-----------------------------------------------------------------------------");
            writer.WriteLine(" * Summary: ");
            writer.WriteLine(" *  Loads the " + m_instanceName + "_CSHL_SlotBaseline[slot] array element with an initial value ");
            writer.WriteLine(" *  by scanning the selected slot. The raw count value is copied into the baseline ");
            writer.WriteLine(" *  array for each slot. The raw data filters are initialized if enabled.");
            writer.WriteLine(" *");
            writer.WriteLine(" * Parameters: ");
            writer.WriteLine(" *  slot: uint8 – Scan slot number");
            writer.WriteLine(" *");
            ...
            writer.WriteLine(" /*-----------------------------------------------------------------------------");
            writer.WriteLine(" * FUNCTION NAME: void " + m_instanceName + "_CSHL_InitializeAllBaselines" + sSide + "(void)");
            writer.WriteLine(" *-----------------------------------------------------------------------------");
            writer.WriteLine(" * Summary: ");
            writer.WriteLine(" *  Loads the " + m_instanceName + "_CSHL_SlotBaseline[ ] array with initial values by scanning"); 
            writer.WriteLine(" *  all slots. The raw count values are copied in to baseline array for each slot.");          
            writer.WriteLine(" *  The raw data filters are initialized if enabled.");
            writer.WriteLine(" *");
            writer.WriteLine(" * Parameters: ");
            writer.WriteLine(" *  None");
            writer.WriteLine(" *");
            ...
            writer.WriteLine(" /*-----------------------------------------------------------------------------");
            writer.WriteLine(" * FUNCTION NAME:  void " + m_instanceName + "_CSHL_UpdateSlotBaseline" + sSide + "(uint8 slot)");
            writer.WriteLine(" *-----------------------------------------------------------------------------");
            writer.WriteLine(" * Summary: ");
            writer.WriteLine(" *  Updates the " + m_instanceName + "_CSHL_SlotBaseline[ ] array using the LP filter with k = 256.");
            writer.WriteLine(" *  The signal calculates the difference between raw count and baseline. The baseline stops");
            writer.WriteLine(" *  updating if signal is greater that zero. Raw data filters are applied to the values if enabled.");
            writer.WriteLine(" *");
            writer.WriteLine(" * Parameters: ");
            writer.WriteLine(" *  slot: uint8 – Scan slot number");
            writer.WriteLine(" *");
            ...
            writer.WriteLine("		    		((" + m_instanceName + "_BtnSettings *) ptr)->RawJitter = filteredRawData;");
            writer.WriteLine("			    }");
            writer.WriteLine("			    break;");
            writer.WriteLine("		    ");
            writer.WriteLine("		    /* This case include LINEAR and RADIAL SL */");
            ...
            writer.WriteLine("			    	((" + m_instanceName + "_SlSettings *) ptr)->RawJitter[filterPos] = filteredRawData;");
            writer.WriteLine("			    }");
            writer.WriteLine("		    	break;");
            writer.WriteLine("		    	");
            writer.WriteLine("		    case " + m_instanceName + "_CSHL_TYPE_TOUCHPAD:");
            ...
            writer.WriteLine("			    	((" + m_instanceName + "_TPSettings *) ptr)->RawJitter[filterPos] = filteredRawData;");
            writer.WriteLine("			    }");
            writer.WriteLine("			    break;");
            writer.WriteLine("			    ");
            writer.WriteLine("		    case " + m_instanceName + "_CSHL_TYPE_MATRIX_BUTTONS:");
            ...
            writer.WriteLine("			    	((" + m_instanceName + "_MBSettings *) ptr)->RawJitter[filterPos] = filteredRawData;");
            writer.WriteLine("			    }");
            writer.WriteLine("			    break;");
            writer.WriteLine("		    			");
            writer.WriteLine("		    default:");
            ...
            writer.WriteLine("			    " + m_instanceName + "_CSHL_SlotSignal[rawIndex] = 0;");
            writer.WriteLine("		    }");
            writer.WriteLine("		    else if(temp < 0xFFu)");
            writer.WriteLine("		    {");
            ...
            writer.WriteLine("	    if ( " + m_instanceName + "_CSHL_SlotSignal[rawIndex] < " + m_instanceName + "_CSHL_WidgetTable[widget].NoiseThreshold )");
            writer.WriteLine("	    {");
            ...
            //            writer.WriteLine("		    //" + m_instanceName + "_CSHL_SlotSignal[rawIndex] -= " + m_instanceName + "_CSHL_WidgetTable[widget].NoiseThreshold;");
            //			writer.WriteLine("		    " + m_instanceName + "_CSHL_SlotSignal[rawIndex] = 80;");
            //            writer.WriteLine("	    }");
            writer.WriteLine("  }");
            writer.WriteLine("}");
            writer.WriteLine("");
            ...
            writer.WriteLine(" /*-----------------------------------------------------------------------------");
            writer.WriteLine(" * FUNCTION NAME: void " + m_instanceName + "_CSHL_UpdateAllBaselines" + sSide + "(void)");
            writer.WriteLine(" *-----------------------------------------------------------------------------");
            writer.WriteLine(" * Summary: ");
            writer.WriteLine(" *  Uses the " + m_instanceName + "_CSHL_UpdateSlotBaseline function to update the baselines");
            ...
            writer.WriteLine(" /*-----------------------------------------------------------------------------");
            writer.WriteLine(" * FUNCTION NAME: uint8 " + m_instanceName + "_CSHL_CheckIsSlotActive" + sSide + "(uint8 Slot)");
            writer.WriteLine(" *-----------------------------------------------------------------------------");
            writer.WriteLine(" * Summary: ");
            writer.WriteLine(" *  Compares the selected slot of the " + m_instanceName + "_CSHL_Signal[ ] array to its finger ");
            writer.WriteLine(" *  threshold. Hysteresis is taken into account. The Hysteresis value is added or subtracted from the ");
            writer.WriteLine(" *  finger threshold based on whether the slot is currently active. If the slot is active, the threshold ");
            writer.WriteLine(" *  is lowered by the hysteresis amount. If it is inactive, the threshold is raised by the hysteresis");
            writer.WriteLine(" *  amount. This function also updates the slot's bit in the " + m_instanceName + "_CSHL_SlotOnMask[ ] array.");
            writer.WriteLine(" *");
            writer.WriteLine(" * Parameters: ");
            writer.WriteLine(" *  slot: uint8 – Scan slot number");
            writer.WriteLine(" *");
            writer.WriteLine(" * Return: ");
            writer.WriteLine(" *  uint8:1 if active, 0 if not active");
            writer.WriteLine(" *");
            writer.WriteLine(" * Theory:");
            writer.WriteLine(" *  See summary");
            writer.WriteLine(" *");
            ...
            writer.WriteLine("		    else");
            writer.WriteLine("		    {");
            writer.WriteLine("			    /* Slot active - reset Debounce count  */");
            writer.WriteLine("		        " + m_instanceName + "_ScanSlotTable" + sSide + "[slot].DebounceCount = " + m_instanceName + "_CSHL_WidgetTable[widget].Debounce;");
            writer.WriteLine("		    }");
            writer.WriteLine("	    }");
            ...
            writer.WriteLine(" /*-----------------------------------------------------------------------------");
            writer.WriteLine(" * FUNCTION NAME: uint8 " + m_instanceName + "_CSHL_CheckIsAnySlotActive" + sSide + "(void)");
            writer.WriteLine(" *-----------------------------------------------------------------------------");
            writer.WriteLine(" * Summary: ");
            writer.WriteLine(" *  Compares all slots of the " + m_instanceName + "_CSHL_Signal[ ] array to their finger threshold. ");
            writer.WriteLine(" *  Calls " + m_instanceName + "_CSHL_CheckIsSlotActive() for each slot so the " + m_instanceName + "_CSHL_SlotOnMask[ ]");
            writer.WriteLine(" *  array is up todate after calling this function.");
            writer.WriteLine(" *");
            writer.WriteLine(" * Parameters: ");
            writer.WriteLine(" *  None");
            writer.WriteLine(" *");
        ...
        public void ApiCollectCHLFunctionBase(ref CyMyStringWriter writer, CyGeneralParams packParams)
        {
            ...
            if (packParams.Configuration != E_MAIN_CONFIG.emSerial)
            {
                ...
                writer.WriteLine(" /*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: void " + m_instanceName + "_CSHL_InitializeAllBaselines(void)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" * Loads the " + m_instanceName + "_CSHL_SlotBaseline[ ] array with initial values by scanning");
                writer.WriteLine(" * all slots. The raw count values are copied in to baseline array for each slot.");
                ...
                writer.WriteLine(" /*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: void " + m_instanceName + "_CSHL_UpdateAllBaselines(void)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  Uses the " + m_instanceName + "_CSHL_UpdateSlotBaseline function to update the baselines");
                writer.WriteLine(" * for all slots. Raw data filters are applied to the values if enabled.");
                ...
                writer.WriteLine(" /*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: void " + m_instanceName + "_CSHL_CheckIsAnySlotActive(void) ");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  Compares all slots of the " + m_instanceName + "_CSHL_Signal[ ] array to their finger threshold. ");
                writer.WriteLine(" *  Calls " + m_instanceName + "_CSHL_CheckIsSlotActive() for each slot so the " + m_instanceName + "_CSHL_SlotOnMask[ ]");
        ...
        public void ApiCollectCHLFunctionCentroid(ref CyMyStringWriter writer, CyGeneralParams packParams)
        {
            if (packParams.m_cyWidgetsList.GetCountWidgetsSameType(E_SENSOR_TYPE.Linear_Slider) > 0)
            {
                ...
                writer.WriteLine(" /*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: uint16 " + m_instanceName + "_CSHL_GetCentroidPos(uint8 widget)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  Checks the " + m_instanceName + "_CSHL_Signal[ ] array for a centroid. The centroid position");
                writer.WriteLine("    is calculated to the resolution specified in the CapSense customizer. The ");
                writer.WriteLine("    position filters are applied to the result if enabled. This function is ");
                writer.WriteLine("    available only if a linear slider is defined by the CapSense customizer.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Parameters: ");
                writer.WriteLine(" *  widget:uint8 – Widget number. For every linear slider widget there are defines in this format:");
                writer.WriteLine(" *  #define CapSense_CSHL_LS_”widget_name”			5");
                writer.WriteLine(" *");
                writer.WriteLine(" * Return: ");
                writer.WriteLine(" *  uint16: Position value of the slider");
                writer.WriteLine(" *");
                writer.WriteLine(" * Theory:");
                writer.WriteLine(" *  See summary");
                writer.WriteLine(" *");
                writer.WriteLine(" * Side Effects:");
                writer.WriteLine(" *  If any slider slot is active, the function returns values from zero to the ");
                writer.WriteLine(" *  resolution value set in the CapSense customizer. If no sensors are active,");
                writer.WriteLine(" *  the function returns -1. If an error occurs during execution of the ");
                writer.WriteLine(" *  centroid/diplexing algorithm, the function returns -1.You can use the ");
                writer.WriteLine(" *  " + m_instanceName + "_CSHL_ChecklsSlotActive() routine to determine which slider segments are touched,");
                writer.WriteLine(" *  if required.");
                writer.WriteLine(" *  Note If noise counts on the slider segments are greater than the noise threshold,");
                writer.WriteLine(" *  this subroutine may generate a false centroid result. The noise threshold should ");
                writer.WriteLine(" *  be set carefully (high enough above the noise level) so that noise will not ");
                writer.WriteLine(" *  generate a false centroid.");
                writer.WriteLine(" *");
                writer.WriteLine(" *---------------------------------------------------------------------------*/");
            ...
            if (packParams.m_cyWidgetsList.GetCountWidgetsSameType(E_SENSOR_TYPE.Radial_Slider) > 0)
            {
                ...
                writer.WriteLine("/*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: uint16 " + m_instanceName + "_CSHL_RotarySliderArcTan2(int32 X, int32 Y)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  This fiunction calculates ArcTan");
                writer.WriteLine(" *");
                writer.WriteLine(" * Parameters: ");
                writer.WriteLine(" *  int32 X, int32 Y");
                writer.WriteLine(" *");
                writer.WriteLine(" * Return: ");
                writer.WriteLine(" *  uint16: ArcTan");
                writer.WriteLine(" *");
                writer.WriteLine(" * Theory:");
                writer.WriteLine(" *  See summary");
                writer.WriteLine(" *");
                ...
                writer.WriteLine(" /*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: uint16 " + m_instanceName + "_CSHL_RotarySlider_GetValue(const uint8 ZeroOffset, ");
                ...
                writer.WriteLine("			if (tempAngle > " + m_instanceName + "_CSHL_ROTARY_SLIDER_A360) tempAngle -= " + m_instanceName + "_CSHL_ROTARY_SLIDER_A360;");
                ...
                #region CSHL_GetRadialCentroidPos
                writer.WriteLine("/*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: uint16 " + m_instanceName + "_CSHL_GetRadialCentroidPos(uint8 widget)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  Checks the " + m_instanceName + "_CSHL_Signal[ ] array for a centroid. The centroid position");
                writer.WriteLine("    is calculated to the resolution specified in the CapSense customizer. The ");
                writer.WriteLine("    position filters are applied to the result if enabled. This function is ");
                writer.WriteLine("    available only if a radial slider is defined by the CapSense customizer.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Parameters: ");
                writer.WriteLine(" *  widget:uint8 – Widget number. For every linear slider widget there are defines in this format:");
                ...
                writer.WriteLine(" *  the function returns -1. If an error occurs during execution of the ");
                writer.WriteLine(" *  centroid/diplexing algorithm, the function returns -1.You can use the ");
                writer.WriteLine(" *  " + m_instanceName + "_CSHL_ChecklsSlotActive() routine to determine which ");
            ...
            if (packParams.m_cyWidgetsList.GetCountDoubleFullWidgets() > 0)
            {
                ...
                writer.WriteLine(" /*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: uint16 " + m_instanceName + "_CSHL_CalcCentroid(uint8 startIndex, ");
                writer.WriteLine(" *						uint8 length, uint8 pos, uint16 Mult)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  Find position of defined centroid.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Parameters: ");
                writer.WriteLine(" *  startIndex: uint8 – Start index of cetroid in " + m_instanceName + "_CSHL_SlotSignal[],");
                ...
                writer.WriteLine(" *");
                writer.WriteLine(" * Return: ");
                writer.WriteLine(" *  uint8: Position of centroid.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Theory:");
                writer.WriteLine(" *  See summary");
                writer.WriteLine(" *");
                ...
                writer.WriteLine("/*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: uint8 " + m_instanceName + "_CSHL_FindLocalMax(uint8 startIndex, ");
                writer.WriteLine(" *						uint8 length, uint8 fingerthreshould)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  Find local max of defined centroid.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Parameters: ");
                writer.WriteLine(" *  startIndex: uint8 – Start index of cetroid in " + m_instanceName + "_CSHL_SlotSignal[],");
                writer.WriteLine(" *  length: uint8 - Length of centroid");
                writer.WriteLine(" *  fingerthreshould: uint8 - Fingerthreshould of centriod");
                writer.WriteLine(" *");
                writer.WriteLine(" * Return: ");
                writer.WriteLine(" *  uint8: Local max of centroid.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Theory:");
                writer.WriteLine(" *  See summary");
                writer.WriteLine(" *");
                ...
                #region CSHL_GetDoubleCentroidPos
                writer.WriteLine("/*-----------------------------------------------------------------------------");
                writer.WriteLine(" * FUNCTION NAME: uint16 " + m_instanceName + "_CSHL_GetDoubleCentroidPos(uint8 widget)");
                writer.WriteLine(" *-----------------------------------------------------------------------------");
                writer.WriteLine(" * Summary: ");
                writer.WriteLine(" *  If a finger is present, this function calculates the X and Y position of the finger");
                writer.WriteLine(" *  by calculating the centroids. The X and Y positions are calculated to the resolutions");
                writer.WriteLine(" *  set in the CapSense customizer. Returns a ‘1’ if a finger is on the touchpad. The ");
                writer.WriteLine(" *  position filters are applied to the result if enabled. This function is available only");
                writer.WriteLine(" *  if a touch pad is defined by the CapSense customizer.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Parameters: ");
                writer.WriteLine(" *  widget: uint8 – Widget number. For every touchpad widget there are defines in this format:");
                writer.WriteLine(" *  #define CapSense_CSHL_TP_”widget_name”			5");
                writer.WriteLine(" *");
                writer.WriteLine(" * Return: ");
                writer.WriteLine(" *  uint8: 1 if finger is on the touchspad, 0 if not.");
                writer.WriteLine(" *");
                writer.WriteLine(" * Theory:");
                writer.WriteLine(" *  See summary");
                writer.WriteLine(" *");
                writer.WriteLine(" * Side Effects:");
                writer.WriteLine(" *  The result of calculation of X and Y position store in global arrays. The arrays name are:");
                writer.WriteLine(" *  " + m_instanceName + "_CSHL_TPCol_”widget_name”_Results – postion of X");
