...
namespace  CapSense_v1_10
{
    ...
    public class CyWidgetsList
    {
        ...
        {
            return new List<CyElWidget>(getListWidgets());
        }
        public IEnumerable<CyElWidget> getListWidgets()
        {
        ...
        public void AddWidgetToList(CyElWidget wi)
        {
            switch (wi.m_type)
            {
                case E_SENSOR_TYPE.Button:
        ...
        #region Service work
        public CyElWidget GetWidget(string Name, E_SENSOR_TYPE type)
        {
        ...
        public CyElWidget GetWidget(CyElTerminal term)
        {
            return GetWidget(term.m_Name, term.m_type);
        }
        ...
        public CyElWidget FindWidget(E_SENSOR_TYPE stype, string sname)
        {
            CyElWidget res = null;
            foreach (CyElWidget itemIntern in getListWidgets())
                if ((itemIntern.m_type == stype) && (itemIntern.m_Name == sname))
                {
                    res = itemIntern;
                    break;
                }
       ...
       public CyHAProps GetWidgetsProperties(CyElWidget wi)
       {
           ...
           return GetWidget(wi.m_Name, type).GetProps();
       }

        public IEnumerable<CyElTerminal> GetTerminals(CyElWidget widg)
        {            
            foreach (CyElTerminal item in m_listTerminal)
                if (item.m_Name == widg.m_Name)
                {
                    if (item.m_type == widg.m_type)
                    {
                        yield return item;
                    }
                }
        }
        public List<CyElTerminal> GetTerminalsWithOutHeader(CyElWidget widg)
        {
            List<CyElTerminal> res = new List<CyElTerminal>(GetTerminals(widg));
            if (CySensorType.IsCustomCase(widg.m_type))
            {
                res.RemoveAt(0);
            }
            return res;
        ...
        CyElWidget AppentToList(CyElWidget wi)
        {
            foreach (CyElWidget item in getListWidgets())
            {
        ...
        public CyElWidget AddWidget(object[] props, E_SENSOR_TYPE type, bool mVirtual)
        {
            ...
            if (!mVirtual)// Is Real Widget
            {
                ...
                if (AppentToList(WI) == null)
                {
                    ...
                    m_Base.m_cyScanSlots.InsertNewScanSlots(WI, WI.m_side); 
                }
                else return AppentToList(WI);
            }
            return WI;
        }
        public void DeleteWidgets(CyElWidget[] Wis)
        {
            //Remove widget terminals
            foreach (CyElWidget element in Wis)
            {
            ...
            //remove Widget
            foreach (CyElWidget item in Wis)
            {
        ...
        public CyElWidget[] GetWidget(object[] NewRow, E_SENSOR_TYPE stype)
        {
            switch (stype)
            {
                case E_SENSOR_TYPE.Button:
        ...
        public IEnumerable<CyElWidget> ValidateWidgets(E_SENSOR_TYPE[] type, object[] NewRow)
        {
            ...
            CyElWidget[] oldWidget = GetWidget(NewRow, type[0]);
            string NewName;
            ...
            for (int i = 0; i < type.Length; i++)
            {
                ...
                {
                    //Re-Name
                    ReNameTerminal(new CyElTerminal(oldWidget[i].m_Name, oldWidget[i].m_type), newWidget.m_Name,newWidget.m_type);
                ...
                if ((c_res & E_COMPARER_TYPE.CountChange) != 0)
                {
                    ...
                    if (oldWidget[i].GetCount() > newWidget.GetCount())
                        //remove terminals
                    {
                        DeleteTerminal(oldWidget[i].m_Name, oldWidget[i].m_type, newWidget.GetCount(), oldWidget[i].GetCount());
                    }
                    if (oldWidget[i].GetCount() < newWidget.GetCount())
                        //append terminals
                    {
                        ...
                        for (int j = oldWidget[i].GetCount(); j < newWidget.GetCount(); j++)
                        {
                            ...
                            {                                
                                m_Base.m_cyScanSlots.InsertNewScanSlotWithTerm(term, wi.m_side);
                            }
                        }
                ...
                if ((c_res & E_COMPARER_TYPE.SideChange) != 0)
                {
                    for (int id = 0; id < m_listTerminal.Count; id++)
                        if (GetWidget(m_listTerminal[id]).IsSame(oldWidget[i]))
                        {
                            m_listTerminal[id].m_haveSide = newWidget.m_side;
                        }
                    m_Base.m_cyScanSlots.ChangeWidgetSide(oldWidget[i], newWidget);
        ...
        void DeleteTerminal(CyElTerminal term)
        {
            ...
            {
                if (m_listTerminal[pos].IsSameFull(term))
                {
            ...
            m_Base.m_cyScanSlots.EraceFreeSS();
        }
        ...
        IEnumerable<CyElTerminal> GetListTerminalsFromSideInt(E_EL_SIDE side)
        {
            SortListTerminals();
            foreach (CyElTerminal item in m_listTerminal)
        ...
        public List<CyElTerminal> GetListTerminalsFromSide(E_EL_SIDE side)
        {
            return new List<CyElTerminal>(GetListTerminalsFromSideInt(side));
        }
        ...
        IEnumerable<CyElTerminal> GetTerminalsSortedInt()
        {
            ...
            {
                foreach (CyElTerminal s_item in GetListTerminalsFromSide(item))
                    yield return s_item;
            }
        }
        //Update terminal list order
        public void SortListTerminals()
        {
            ...
            while (i < m_listTerminal.Count)
            {
                ...
                while (j < m_listTerminal.Count)
                    if (m_listTerminal[i].IsSameW(m_listTerminal[j], true))
                    {
                        i = j;
                        j++;
                    }
        ...
        public int GetCountWidgetsSameType(E_SENSOR_TYPE stype)
        {
            int res = 0;
            foreach (CyElWidget item in GetListWidgets())
        ...
        public int GetCountWidgetsSameType(E_SENSOR_TYPE stype, E_EL_SIDE _side)
        {
            int res = 0;
            foreach (CyElWidget item in GetListWidgets())
                if (stype == item.m_type)
        ...
        public List<CyElWidget> GetBothParts(CyElWidget wi)
        {
            ...
            {
                listRes.Add(GetWidget(wi.m_Name, item));
            }
            return listRes;
        }
        ...
        {
            foreach (CyElWidget item in m_listMatrixButtons)
                if (!CySensorType.IsRow(item.m_type))
                    yield return item;
            foreach (CyElWidget item in m_listTouchPads)
                if (!CySensorType.IsRow(item.m_type))
                    yield return item;
        ...
        public int GetIndexInListWithFullWidgetsHL(CyElWidget wi)
        {
            ...
            CyElWidget top_wi = GetWidget(wi.m_Name, CySensorType.GetBothParts(wi.m_type)[0]);
        ...
        {
            return GetListWidgets().Count - GetCountWidgetsSameType(E_SENSOR_TYPE.Generic);
        }
       #endregion
