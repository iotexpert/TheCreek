...
namespace SegLCD_v2_10
{
    partial class CyAPIGenerator
    {
            ...
            using (TextWriter writerCVariables = new StringWriter(), writerHPixelDef = new StringWriter())
            {
                ...
                if (Convert.ToBoolean(Convert.ToInt32(gang)))
                {
                    StringBuilder defStr =
                        new StringBuilder(string.Format("uint16 {0}_gCommons[{1}] = {{", m_instanceName, 
                                                                                         pNumCommonLines));
                    ...
                    for (int i = 0; i < pNumCommonLines; i++)
                    {
                        ...
                        writerHPixelDef.WriteLine(
                            ...
                            "{5}_GCom__LCD_COM_PIN__{6},  {7})",
                            m_instanceName, comIndex, m_instanceName, m_instanceName, comIndex, m_instanceName, 
                            comIndex, comIndex);
                    }
                }
                ...
                for (int i = 0; i < HelpersConfig.Count; i++)
                {
                    ...
                    {
                        writerCVariables.WriteLine("uint8 {0}_digitNum_{1} = {2};", m_instanceName, index,
                                                   hi.SymbolsCount);
                ...
                commons =
                    new StringBuilder(String.Format("uint16 {0}_commons[{1}] = {{{2}}};", m_instanceName,
                ...
                frameTX.Insert(0, string.Format("static uint8 {0}_frameTx[{1}] = {{", m_instanceName, frameTxLen*2));
                frameTX.Append("};");
                frameTX = StringLineBreaking(frameTX.ToString());
                writerCVariables.WriteLine("");
                writerCVariables.WriteLine(frameTX);
                // frameTD
                writerCVariables.WriteLine(string.Format("static uint8 {0}_frameTd[{1}];", m_instanceName, 
                                                         frameTxLen*2));
        ...
        private void WriteHeaderArray(TextWriter writer, CyHelperInfo hi, int index)
        {
            ...
            {
                str = string.Format("const uint16 CYCODE {0}_disp{1}[{2}][{3}] = {{{4}}};", m_instanceName, index,
                                    hi.SymbolsCount, hi.SegmentCount, str);
                writer.WriteLine(StringLineBreaking(str));
            }
            else
            {
                str = string.Format("const uint16 CYCODE {0}_disp{1}[{2}][{3}] = {{{{0u}}, {4}}};", m_instanceName,
