...
namespace I2C_v3_0
{
    public partial class CyI2CBasic : UserControl, ICyParamEditingControl
    {
            ...
            if (m_params.m_bGlobalEditMode)
            {
                ...
                else
                {
                    if (internalClock.IsFrequencyKnown)
                    {
                        ...
                        {
                            byte divider = 0;
                            string deviceFamily = m_instEdit.DeviceQuery.DeviceFamily;
                            int siliconRevision = m_instEdit.DeviceQuery.SiliconRevision; // values 2, 3 are PSoC3 ES3
                            if (deviceFamily == CyEDeviceFamilyType.PSoC3.ToString() && siliconRevision >= 2)
                            {
        ...
        {
            if (m_instEdit.DeviceQuery.IsPSoC5 || (m_instEdit.DeviceQuery.IsPSoC3 && m_instEdit.DeviceQuery.SiliconRevision < 2))
            {
        ...
        {
            if (m_instEdit.DeviceQuery.IsPSoC5)
            {
                if (chbWakeup.Checked)
                    errorProvider.SetError(chbWakeup, Resource.DRCRevisionErrorPSoC5);
                else
                    errorProvider.SetError(chbWakeup, string.Empty);
            }
            else if (m_instEdit.DeviceQuery.IsPSoC3 && m_instEdit.DeviceQuery.SiliconRevision < 2)
            {
                if (chbWakeup.Checked)
                    errorProvider.SetError(chbWakeup, Resource.DRCRevisionErrorPSoC3ES2);
                else
                    errorProvider.SetError(chbWakeup, string.Empty);
            }
            else
