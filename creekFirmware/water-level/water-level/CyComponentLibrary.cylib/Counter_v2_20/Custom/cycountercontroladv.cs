...
namespace Counter_v2_20
{
    ...
    {        
        ...
        public void UpdateFormFromParams(ICyInstEdit_v1 inst)
        {
            ...
            m_FF = (prms.FixedFunction.Value == Counter_v2_20.CounterParameters.TRUE) ? true : false;
        ...
        private void SetFixedFunction()
        {
            ...
            string captureMode = m_component.GetCommittedParam(CounterParameters.CAPTUREMODE).Value;
            ...
            string enableMode = m_component.GetCommittedParam(CounterParameters.ENABLEMODE).Value;
            ...
            string interruptOnCompare = m_component.GetCommittedParam(CounterParameters.INTERRUPT_ON_COMPARE).Value;
            if (interruptOnCompare != CounterParameters.FALSE)
            {
        ...
        void UpdateCheckBoxParameter(string paramName, bool isChecked, bool visible)
        {
            if (isChecked && visible)
                CounterParameters.SetAParameter(paramName, CounterParameters.TRUE, m_component);
            else
