...
cystatus `$INSTANCE_NAME`_EraseSector(uint16 address)
{
    ...
    if(address <= (uint16)(`$INSTANCE_NAME`_EEPROM_SIZE - SIZEOF_EEPROM_ROW))
    {
...
cystatus `$INSTANCE_NAME`_Write(uint8 * line, uint16 address)
{
    ...
    if(line != 0 &&
       !(address & 0xFu) &&
       address <= (uint16)(`$INSTANCE_NAME`_EEPROM_SIZE - SIZEOF_EEPROM_ROW))
    {
        /* Get the die temp. */
        status = CySetTemp();
        ...
        if(status == CYRET_SUCCESS && CySpcLock() == CYRET_SUCCESS)
        {
            ...
            {
                /* Write the command. */
                if(CySpcWriteCommand(line, SIZEOF_ECC_ROW) == CYRET_STARTED)
                {
                    /* Spin until completion. */
                    while(!(*SPC_STATUS & SPC_SPC_IDLE))
                    {
