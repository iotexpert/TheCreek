...
{
    public enum CyHelperKind {Segment7, Segment14, Segment16, Bar, Matrix, Empty};
    ...
    public partial class CyHelpers : UserControl
    {
        ...
        public bool m_ParametersChanged = false;
            ...
            switch (listBoxAvailHelpers.SelectedIndex)
            {
                case 0:
                    if (m_Parameters.NumCommonLines*m_Parameters.NumSegmentLines - 
                        HelperInfo.GetTotalPixelNumber(m_Parameters) < 8)
                    {
                        fitInSpace = false;
                        break;
                    }
                ...
                case 4:
                    if (m_Parameters.NumCommonLines * m_Parameters.NumSegmentLines - 
                        HelperInfo.GetTotalPixelNumber(m_Parameters) < 5*8)
                    {
                        fitInSpace = false;
                        break;
                    }
                    HelperInfo.CreateHelper(CyHelperKind.Matrix, m_Parameters);
                    break;
                default:
                    break;
            }
        ...
        private void buttonAddSymbol_Click(object sender, EventArgs e)
        {
            ...
            switch (m_Parameters.m_HelpersConfig[m_currentHelperIndex].Kind)
            {
                case CyHelperKind.Segment7:
                    if (m_Parameters.NumCommonLines * m_Parameters.NumSegmentLines - HelperInfo.GetTotalPixelNumber(m_Parameters) < 8)
                    {
                        fitInSpace = false;
                        break;
                    }
                ...
                case CyHelperKind.Matrix:
                    if (m_Parameters.NumCommonLines * m_Parameters.NumSegmentLines - HelperInfo.GetTotalPixelNumber(m_Parameters) < 5 * 8)
                    {
                        fitInSpace = false;
                        break;
                    }
        ...
        private void EnableCommon(int index)
        {
            ...
            {
                dataGridMap[m_Parameters.NumCommonLines - index, i].Style = dataGridMap.DefaultCellStyle;
            }
            //dataGridMap[m_Parameters.NumCommonLines - index, 0].Style = CellCommonStyle;
        }

        private void DisableCommon(int index)
        {
            if (!m_Parameters.DisabledCommons.Contains(index))
            {
                List<int> DisabledCommons = new List<int>(m_Parameters.DisabledCommons);
            ...
            for (int i = 1; i < dataGridMap.RowCount; i++)
            {
                ResetPixelCommonSegment(dataGridMap[m_Parameters.NumCommonLines - index, i].Value.ToString());
            }
            for (int i = 1; i < dataGridMap.RowCount; i++)
            {
                dataGridMap[m_Parameters.NumCommonLines - index, i].Style = cellDisabledStyle;
            }
        }
        ...
        {
           if (!HelperInfo.CheckPixelUniqueName(m_Parameters, textBoxSegmentTitle.Text))
           {
               MessageBox.Show("The name " + textBoxSegmentTitle.Text + " is not unique. Please, enter another name.",
        ...
        private void dataGridMap_MouseDown(object sender, MouseEventArgs e)
        {
