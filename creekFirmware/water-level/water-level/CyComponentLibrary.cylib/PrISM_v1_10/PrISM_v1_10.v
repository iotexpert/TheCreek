...
* 
*  pulse_den0   output      Output of the PulseDensity0
*  pulse_den1   output      Output of the PulseDensity1
*  bitstream    output      Output of the LFSR LSb
...
`include "cypress.v"
`ifdef PrISM_v1_10_V_ALREADY_INCLUDED
`else
`define PrISM_v1_10_V_ALREADY_INCLUDED
...
module PrISM_v1_10
  (
   ...
   output wire pulse_den0,
   output wire pulse_den1,
   output wire tc
   );
   ...
   localparam LESS_THEN_OR_EQUAL          = 8'd0;
   localparam GREATER_THAN_OR_EQUAL       = 8'd1;
   ...
   localparam PRS_CTRL_COMPARE_TYPE0 =8'h1;	 /* CompareType0 */   
   localparam PRS_CTRL_COMPARE_TYPE1 =8'h2;  /* CompareType1 */ 
   ...
   wire       reset_final;
   wire [2:0] cs_addr = {1'b0, reset_final , ctrl_enable_final};

   assign ctrl_enable_final = ctrl_enable & enable & ~reset;
   assign reset_final = ctrl_enable & reset;
   
   /**************************************************************************/
   /* Generate Pulse Density signals                                     	 */
   /**************************************************************************/
   wire ce0,cl0,ce1,cl1;
   wire Pd0a,Pd0b,Pd1a,Pd1b;
   assign Pd0a=(ce0|cl0) & ~compare_type0 & ~kill & ~reset;  /* LESS_THEN_OR_EQUAL */
   assign Pd0b=~cl0 & compare_type0 & ~kill & ~reset;		 /* GREATER_THAN_OR_EQUAL */
   assign Pd1a=(ce1|cl1) & ~compare_type1 & ~kill & ~reset;  /* LESS_THEN_OR_EQUAL */
   assign Pd1b=~cl1 & compare_type1 & ~kill & ~reset;		 /* GREATER_THAN_OR_EQUAL */
   
   assign pulse_den0=Pd0a|Pd0b;
   assign pulse_den1=Pd1a|Pd1b;
  ...
  begin : sC16
...
}), .cy_dpconfig_c(
    {
        ...
        /* output [02:00]   */ .z0(),       
        /* output [02:00]   */ .ff0({tc, chained5,chained6}),    
...
}), .cy_dpconfig_d(
    {
        ...
        /* output [03:00]   */ .z0(),        
        /* output [03:00]   */ .ff0({tc,chained21,chained22,chained23}),       
...
endmodule   /* PrISM_v1_10 */
`endif      /* PrISM_v1_10_V_ALREADY_INCLUDED */
