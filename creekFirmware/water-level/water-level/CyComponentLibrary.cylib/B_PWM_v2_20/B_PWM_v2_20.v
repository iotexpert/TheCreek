...
`include "cypress.v"
`ifdef B_PWM_V_v2_20_ALREADY_INCLUDED
`else
`define B_PWM_V_v2_20_ALREADY_INCLUDED
module B_PWM_v2_20 (
...
localparam CY_UDB_V1 = (!CY_UDB_V0);
    ...
    if((EnableMode != PWM_ENMODE_HWONLY) || (CompareType1 == PWM_CTRL_CMPMODE_SW) || 
        (CompareType2 == PWM_CTRL_CMPMODE_SW)) begin : sCTRLReg
    ...
    if(DeadBand != PWM_DBMODE_DISABLED) begin : sDBen
        ...
        /* The ph1 disable signal goes high on rising edge detect and stays high until tc */
        /* is reached on the dead band counter */
        /* The ph2 disable signal goes high on falling edge detect and stays high until */
        /* tc is reached on the dead band counter */
        always @(posedge ClockOutFromEnBlock or posedge reset) begin
            if(reset)
        ...
        /* TODO: I could remove the run signal and just pass _temp here to save resources. */
        /* This just puts the deadband output out 1 clock cycle on both edges. */
        assign ph1_i = pwm_db & !db_ph1_run; 
        /* TODO: I could remove the run signal and just pass _temp here to save resources.  */
        /* This just puts the deadband output out 1 clock cycle on both edges. */
        assign ph2_i = !pwm_db & !db_ph2_run; 
        ...
        begin :rstSts           
            ...
            /* output         */  .interrupt(interrupt)
                ...
                if((CompareStatusEdgeSense == 1) && (PWMMode !=PWM_MODE_ONE_OUTPUT) && 
                    (PWMMode != PWM_MODE_CENTER_ALIGN) && (PWMMode != PWM_MODE_DITHER)) begin : sCMP2EdgeSense
                        reg prevCompare2;
                        always @(posedge ClockOutFromEnBlock) begin
                                prevCompare2 <= cmp2;
                        end
