...
#define NVIC_IRQ_BASE	16

#define CYINT_VECTORS       ((cyisraddress *) CYDEV_NVIC_VECT_OFFSET)
#define CYINT_PRIORITY      ((reg32 *) CYDEV_NVIC_PRI_0)
...
cyisraddress CyIntSetVector(uint8 number, cyisraddress address)
{
    ...
    oldIsr = (cyisraddress) ramVectorTable[NVIC_IRQ_BASE + number];

    /* Set new Interrupt service routine. */
    ramVectorTable[NVIC_IRQ_BASE + number] = address;
...
cyisraddress CyIntGetVector(uint8 number)
{
    ...
    return (cyisraddress) ramVectorTable[NVIC_IRQ_BASE + number];
}
...
{
    CYINT_PRIORITY[NVIC_IRQ_BASE + number] = priority << 5;
}
...
uint8 CyIntGetPriority(uint8 number)
{
    ...
    priority = (uint8) CYINT_PRIORITY[NVIC_IRQ_BASE + number] >> 5;
