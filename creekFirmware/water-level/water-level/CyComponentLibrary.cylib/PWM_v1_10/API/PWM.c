...
{
    `$CyGetRegReplacementString`(`$INSTANCE_NAME`_COUNTERCAP_LSB_PTR);
...
void `$INSTANCE_NAME`_WriteDeadTime( uint8 deadtime )
{
    /* If using the Dead Band 1-255 mode then just write the register */
        ...
        `$INSTANCE_NAME`_DEADBAND_COUNT |= (deadtime << `$INSTANCE_NAME`_DEADBAND_COUNT_SHIFT) & `$INSTANCE_NAME`_DEADBAND_COUNT_MASK; /* Set new dead time */
    #endif
}
