...
#include "`$INSTANCE_NAME`.h"
    ...
    extern uint8 `$INSTANCE_NAME`_rxBufferFull;
...
CY_ISR(`$INSTANCE_NAME`_RX_ISR)
{
        ...
        `$RegSizeReplacementString` rxData = 0; 
    #endif /* `$INSTANCE_NAME`_InternalRxInterruptEnabled */
    
    /* User code required at start of ISR */
    /* `#START `$INSTANCE_NAME`_ISR_RX_START` */
    ...
    /* User code required at end of ISR (Optional) */
    /* `#START `$INSTANCE_NAME`_ISR_RX_END` */
