...
/* They are defined starting with cy_boot v2.30 */
    ...
    #error Component `$CY_COMPONENT_NAME` requires cy_boot v2.30 or later
                                     ...
                                     `$INSTANCE_NAME`_UART_ISR__ES2_PATCH))
        #include <intrins.h>
...
l_u16 l_ifc_ioctl_`$INSTANCE_NAME`(l_ioctl_op, void*);
l_u16 l_ifc_ioctl(l_ifc_handle, l_ioctl_op, void*);
...
void        l_sys_irq_restore(l_irqmask) `=ReentrantKeil("l_sys_irq_restore")`;
        ...
        l_u8 ld_read_by_id_callout(l_ifc_handle, l_u8, volatile l_u8*);

    #endif /* (1u == `$INSTANCE_NAME`_CS_ENABLED) */

    /* Transport Layer Functions: Initialization */
    void        ld_init(l_ifc_handle);

    /* Transport Layer Functions: Node Configuration Functions */
    l_u8        ld_read_configuration(l_ifc_handle, l_u8*, l_u8*);
    l_u8        ld_set_configuration(l_ifc_handle, l_u8*, l_u16);

    #if(1u == `$INSTANCE_NAME`_TL_API_FORMAT)

        /* Transport Layer Functions: Cooked Transport Layer API */
        void        ld_send_message(l_ifc_handle, l_u16, l_u8, l_u8*);
        void        ld_receive_message(l_ifc_handle, l_u16* const, l_u8* const, l_u8* const);
        l_u8        ld_tx_status(l_ifc_handle);
        l_u8        ld_rx_status(l_ifc_handle);

    #else

        /* Transport Layer Functions: Raw Transport Layer API */
        void        ld_put_raw(l_ifc_handle, const l_u8* const);
        void        ld_get_raw(l_ifc_handle, l_u8* const);
        l_u8        ld_raw_tx_status(l_ifc_handle);
        l_u8        ld_raw_rx_status(l_ifc_handle);
...
#define `$INSTANCE_NAME`_CONTROL_REG            (* (reg8 *) `$INSTANCE_NAME`_bLIN_`$CtlModeReplacementString`_CtrlReg__CONTROL_REG)
#define `$INSTANCE_NAME`_CONTROL_PTR            (  (reg8 *) `$INSTANCE_NAME`_bLIN_`$CtlModeReplacementString`_CtrlReg__CONTROL_REG)
