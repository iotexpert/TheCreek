...
namespace VoltageFaultDetector_v2_0
{
    ...
    public class CyParameters
    {
        ...
        public const double MAX_CLOCK_FREQ_V1 = 8;
        public const double MAX_CLOCK_FREQ_V4 = 3.2;
        ...
        public void UpdateClock(ICyInstQuery_v1 edit, ICyTerminalQuery_v1 termQuery)
        {
            ...
            if (clkData != null)
            {
                ...
                double glitchFiltLen = GetGlitchFilterLength(freq, NumVoltages, GFLength);
                m_basicTab.SetMaxClockValueWarning(CheckMaxClockValue(freq), DACRange);
                m_basicTab.UpdateTimeUnitsLabel(glitchFiltLen);
            }
            else
            {
                m_basicTab.UpdateTimeUnitsLabel(null);
                m_basicTab.SetMaxClockValueWarning(CheckMaxClockValue(null), DACRange);
            }
        }

        public static double GetGlitchFilterLength(double freq, int numVoltages, int glitchFilterLen)
        {
            // 1/Hz = sec * 10^6 * 16 * NumVoltages * GlitchFilterLength = us
            double len = (1 / freq * 1000000) * CyParamRanges.NUM_CYCLE * numVoltages * glitchFilterLen;
            return len;
        }

        public bool CheckMaxClockValue(double? freq)
        {
            if (ExternalRef == false && freq.HasValue)
            {
                const double EPS = 0.001;
                if ((DACRange == CyDACRangeType.V1 && freq.Value > MAX_CLOCK_FREQ_V1 * Math.Pow(10, 6) + EPS) ||
                    (DACRange == CyDACRangeType.V4 && freq.Value > MAX_CLOCK_FREQ_V4 * Math.Pow(10, 6) + EPS))
                {
                    return false;
                }
            }
        ...
        public bool CheckMaxClockValueDRC(ICyTerminalQuery_v1 termQuery)
        {
            bool res = true;
            CyClockData clkData = CyParameters.GetExternalClock(termQuery);
            if (clkData != null)
        ...
        public static string GetWarningMaxClockMsg(bool isOk, CyDACRangeType dacRange)
        {
            ...
            if (!isOk)
            {
                double maxClockFreq = (dacRange == CyDACRangeType.V1) ? MAX_CLOCK_FREQ_V1 : MAX_CLOCK_FREQ_V4;
                msg = String.Format(Resources.WarningMaxClockValue, maxClockFreq, 
                                        CyEnumConverter.GetEnumDesc(dacRange));
            }
            return msg;
        }
        #endregion
