...
namespace CapSense_v1_30.API
{
    public partial class CyAPIGenerator
    {
        ...
        void ApiCollectCHLVariables(ref CyMyStringWriter writerResult, CyGeneralParams packParams)
        {
            ...
            writer.WriteLine("");
            writer.WriteLine("uint8 " + m_instanceName + "_CSHL_SlotOnMask[(((" + m_instanceName + "_TOTAL_SCANSLOT_COUNT - 1) / 8) + 1)] = {0};");
            writer.WriteLine("uint16 " + m_instanceName + "_CSHL_SlotBaseline[" + m_instanceName + "_TOTAL_SCANSLOT_COUNT] = {0};");
            writer.WriteLine("uint8 " + m_instanceName + "_CSHL_SlotBaselineLow[" + m_instanceName + "_TOTAL_SCANSLOT_COUNT] = {0};");
            writer.WriteLine("uint8 " + m_instanceName + "_CSHL_SlotSignal[" + m_instanceName + "_TOTAL_SCANSLOT_COUNT] = {0};");
            #endregion
        ...
        public void ApiCollectCHLFunctionForSide(ref CyMyStringWriter writer, CyGeneralParams packParams, CyAmuxBParams sbItem)
        {
            ...
            writer.WriteLine("*");
            writer.WriteLine("* Summary:");
            writer.WriteLine("*  Uses the " + m_instanceName + "_CSHL_InitializeSlotBaselines" + sSide + " function to");
            writer.WriteLine("*  loads the " + m_instanceName + "_CSHL_SlotBaseline[ ] array with an initial values by");
            writer.WriteLine("*  scanning all slots. The raw count values are copied into the baseline array");
            writer.WriteLine("*  for all slots. The raw data filters are initialized if enabled.");
            writer.WriteLine("*");
            writer.WriteLine("* Parameters:");
            writer.WriteLine("*  void");
            writer.WriteLine("*");
            ...
            writer.WriteLine("*  k = 256. The signal calculates the difference between raw count and baseline.");
            writer.WriteLine("*  The baseline stops updating if signal is greater that zero.");
            ...
            writer.WriteLine("*");
            writer.WriteLine("* Summary:");
            writer.WriteLine("*  Uses the " + m_instanceName + "_CSHL_UpdateSlotBaselines" + sSide + " function to update");
            writer.WriteLine("*  baselines for all slots. Raw data filters are applied to the values if");
            ...
            writer.WriteLine("*  threshold. Hysteresis and Debounce are taken into account. The Hysteresis value");
            writer.WriteLine("*  is added or subtracted from the finger threshold based on whether the slot is");
            ...
            writer.WriteLine("*  hysteresis amount. The Debounce counter added to the slot active transition.");
            writer.WriteLine("*  This function also updates the slot's bit in the");
        ...
        public void ApiCollectCHLFunctionBase(ref CyMyStringWriter writer, CyGeneralParams packParams)
        {
            ...
            if (packParams.Configuration != E_MAIN_CONFIG.emSerial)
            {
                ...
                writer.WriteLine("*  into the baseline array for all slots. The raw data filters are initialized");
                writer.WriteLine("*  if enabled.");
                writer.WriteLine("*");
                writer.WriteLine("* Parameters:");
                writer.WriteLine("*  void");
                ...
                if (packParams.m_localParams.IsParallelFull())
                {
                    ...
                    writer.WriteLine("                " + m_instanceName + "_CSHL_SlotBaselineLow[rawIndex] = 0;");
                    writer.WriteLine("                " + m_instanceName + "_CSHL_SlotSignal[rawIndex] = 0;");
                    writer.WriteLine("                " + m_instanceName + "_ScanSlotTableRight[i].DebounceCount = " + m_instanceName + "_CSHL_WidgetTable[widget].Debounce;");
                    writer.WriteLine("                ");
                    writer.WriteLine("                switch ((" + m_instanceName + "_CSHL_WidgetTable[widget].Type & (~" + m_instanceName + "_CSHL_IS_DIPLEX)))");
                    writer.WriteLine("                {");
                    writer.WriteLine("                    /* This case include BTN and PROX */");
                ...
                writer.WriteLine("*  " + m_instanceName + "_CSHL_UpdateAllSBaselinesRight functions to update the");
                writer.WriteLine("*  baselines for all slots. Raw data filters are applied to the values if");
                writer.WriteLine("*  enabled.");
                writer.WriteLine("*");
                writer.WriteLine("* Parameters:");
                ...
                writer.WriteLine("*  threshold. Uses the " + m_instanceName + "_CSHL_CheckIsSlotActiveLeft and");
                writer.WriteLine("*  " + m_instanceName + "_CSHL_CheckIsSlotActiveRight functions to chech if any of");
                writer.WriteLine("*  all scan slots is active. The" + m_instanceName + "_CSHL_SlotOnMask[ ] array");
                writer.WriteLine("*  is up to date after calling this function.");
                writer.WriteLine("*");
                writer.WriteLine("* Parameters:");
                writer.WriteLine("*  void");
                writer.WriteLine("*");
        ...
        public void ApiCollectCHLFunctionCentroid(ref CyMyStringWriter writer, CyGeneralParams packParams)
        {
            if (packParams.m_cyWidgetsList.GetWidgetsCount(E_SENSOR_TYPE.Linear_Slider) > 0)
            {
                ...
                writer.WriteLine("*");
                writer.WriteLine("* Summary:");
                writer.WriteLine("*  Checks the " + m_instanceName + "_CSHL_Signal[ ] array for a centroid within");
                writer.WriteLine("*  slider specified range. The centroid position is calculated to the resolution");
                writer.WriteLine("*  specified in the CapSense customizer. The position filters are applied to the");
                writer.WriteLine("*  result if enabled.");
                writer.WriteLine("*");
                writer.WriteLine("* Parameters:");
                ...
                writer.WriteLine("*  You can use the _CSHL_ChecklsSlotActive() routine to determine which");
                writer.WriteLine("*  slider segments are touched, if required.");
                writer.WriteLine("*");
                writer.WriteLine("* Note:");
                writer.WriteLine("*  If noise counts on the slider segments are greater than the noise");
                ...
                writer.WriteLine("                    ptr->PosIIR = basePos;");
                writer.WriteLine("                }");
                ...
                writer.WriteLine("            ptr->FirstTime = 0u;");
                writer.WriteLine("        }");
                writer.WriteLine("    }");
                writer.WriteLine("    else");
                writer.WriteLine("    {");
            ...
            if (packParams.m_cyWidgetsList.GetWidgetsCount(E_SENSOR_TYPE.Radial_Slider) > 0)
            {
                ...
                writer.WriteLine("*");
                writer.WriteLine("* Summary:");
                writer.WriteLine("*  Calculates the angle in radians between the positive x-axis of a plane and");
                writer.WriteLine("*  the point given by the coordinates (x, y) on it.");
                writer.WriteLine("*");
                writer.WriteLine("* Parameters:");
            ...
            if (packParams.m_cyWidgetsList.GetWidgetsCount(E_SENSOR_TYPE.Touchpad_Col) > 0)
            {
                ...
                writer.WriteLine("*  pos:  local max of centriod");
                writer.WriteLine("*  Mult:  multiplier calculated accordingly centriod resolution");
                writer.WriteLine("*");
                writer.WriteLine("* Return:");
                ...
                writer.WriteLine("*  position of the finger by calculating the centroids within touch pad specified");
                writer.WriteLine("*  range. The X and Y positions are calculated to the resolutions set in the");
                writer.WriteLine("*  CapSense customizer. The position filters are applied to the result if enabled.");
                writer.WriteLine("*");
                writer.WriteLine("* Parameters:");
                writer.WriteLine("*  widget:  Widget number. ");
                ...
                writer.WriteLine("{");
                writer.WriteLine("    " + m_instanceName + "_TPSettings *ptrX;");
                writer.WriteLine("    " + m_instanceName + "_TPSettings *ptrY;");
                writer.WriteLine("    uint16 posX, posY, basePos;");
                writer.WriteLine("    uint8 i, touch;");
                writer.WriteLine("    ");
                writer.WriteLine("    touch = 0u;");
                writer.WriteLine("    ");
                ...
                writer.WriteLine("        }");
                writer.WriteLine("        ");
                writer.WriteLine("        ptrX = (" + m_instanceName + "_TPSettings *) " + m_instanceName + "_CSHL_WidgetTable[widget].AdvancedSettings;");
                writer.WriteLine("        ptrY = (" + m_instanceName + "_TPSettings *) " + m_instanceName + "_CSHL_WidgetTable[widget + " + m_instanceName + "_CSHL_NUMBER_OF_DOUBLE_STRUCTS].AdvancedSettings;");
                writer.WriteLine("        ");
                ...
                writer.WriteLine("                /* Filtering */");
                writer.WriteLine("                ");
                writer.WriteLine("                if (ptrX->FirstTime == 0u)                /* Initialize the filters */");
                writer.WriteLine("                {");
                writer.WriteLine("                    /* Median filter first time initialization */");
                writer.WriteLine("                    if(" + m_instanceName + "_CSHL_WidgetTable[widget].Filters & " + m_instanceName + "_CSHL_POS_MEDIAN_FILTER)");
                writer.WriteLine("                    {");
                ...
                writer.WriteLine("                    }");
                writer.WriteLine("                    ");
                writer.WriteLine("                    ptrX->FirstTime = 1u;");
                writer.WriteLine("                }");
                writer.WriteLine("                else                            /* Do the filtering */");
                writer.WriteLine("                {");
                writer.WriteLine("                    /* Median filter */");
                ...
                writer.WriteLine("                        ptrX->PosIIR[i] = basePos;");
                writer.WriteLine("                    }");
                writer.WriteLine("                    else if (" + m_instanceName + "_CSHL_WidgetTable[widget].Filters & " + m_instanceName + "_CSHL_POS_IIR_FILTER_1)");
                writer.WriteLine("                    {");
                writer.WriteLine("                        basePos = posX;");
                ...
                writer.WriteLine("                touch |= " + m_instanceName + "_CSHL_X_TOUCH;");
                writer.WriteLine("            }");
                writer.WriteLine("            else");
                writer.WriteLine("            {");
                writer.WriteLine("                ptrX->FirstTime = 0u;");
                writer.WriteLine("            }");
                writer.WriteLine("            ");
                ...
                writer.WriteLine("                ");
                writer.WriteLine("                if (ptrY->FirstTime == 0u)        /* Initialize the filters */");
                writer.WriteLine("                {");
                writer.WriteLine("                    /* Median filter first time initialization */");
                writer.WriteLine("                    if(" + m_instanceName + "_CSHL_WidgetTable[widget].Filters & " + m_instanceName + "_CSHL_POS_MEDIAN_FILTER)");
                writer.WriteLine("                    {");
                ...
                writer.WriteLine("                    }");
                writer.WriteLine("                    ");
                writer.WriteLine("                    ptrY->FirstTime = 1u;");
                writer.WriteLine("                }");
                writer.WriteLine("                else            /* Do the filtering */");
                writer.WriteLine("                {");
                writer.WriteLine("                    /* Median filter */");
                ...
                writer.WriteLine("                touch |= " + m_instanceName + "_CSHL_Y_TOUCH;");
                writer.WriteLine("            }");
                writer.WriteLine("            else");
                writer.WriteLine("            {");
                writer.WriteLine("                ptrY->FirstTime = 0u;");
                writer.WriteLine("            }");
                writer.WriteLine("        }");
                writer.WriteLine("    }");
                writer.WriteLine("    else");
                ...
                writer.WriteLine("    }");
                writer.WriteLine("    ");
                writer.WriteLine("    if(touch != " + m_instanceName + "_CSHL_TRUE_TOUCH)");
                writer.WriteLine("    {");
                writer.WriteLine("        ptrX->FirstTime = 0u;");
                writer.WriteLine("        ptrY->FirstTime = 0u;");
                writer.WriteLine("    }");
                writer.WriteLine("    ");
                writer.WriteLine("    return ((touch == " + m_instanceName + "_CSHL_TRUE_TOUCH) ? 1:0);");
                writer.WriteLine("}");
                #endregion
