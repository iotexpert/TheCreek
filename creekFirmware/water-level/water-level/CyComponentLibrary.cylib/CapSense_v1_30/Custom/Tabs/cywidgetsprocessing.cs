...
namespace CapSense_v1_30
{
    ...
    {
        #region Header
        const string m_strNewName = "NewName";
        const int m_defResolutions = 100;
            ...
            foreach (CyColLocation item in m_massColLocation)
            {
                ...
                foreach (CyAmuxBParams ab in m_packParams.m_localParams.m_listCsHalfs)
                {
                    if (m_packParams.m_localParams.IsAmuxBusEnable(ab))
                        item.Items.Add(CyColLocation.LIST_SIDES_DGSTR[m_id]);
        ...
        {
            dgDClickButton(sender, e, new E_SENSOR_TYPE[] { E_SENSOR_TYPE.Matrix_Buttons_Row, 
                E_SENSOR_TYPE.Matrix_Buttons_Col });
        }
            ...
            System.Windows.Forms.DataGridViewCellCancelEventArgs e, bool twoHalf)
        {
            ...
            if (CyGeneralParams.GetDGString(sender, e.RowIndex, col1) == "")
                dgInput[col1, e.RowIndex].Value =
                    CyColLocation.LIST_SIDES_DGSTR[m_packParams.m_localParams.GetFirstAvailibleSide()];
                ...
                if (m_packParams.Configuration == E_MAIN_CONFIG.emParallelAsynchron)
                {
                    ...
                    if (dgInput[col1, e.RowIndex].Value.ToString() != dgInput[col2, e.RowIndex].Value.ToString())
                    {
                        if (m_packParams.GlobalEditMode)
                            MessageBox.Show( "In Parallel Asynchronous mode Analog Mux Bus for both rows and"
        ...
        void dgSliders_RowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (e.RowIndex != ((DataGridView)sender).RowCount - 1)//not last
            {
                ...
                if (CyGeneralParams.GetDGString(sender, e.RowIndex, col) == "")
                {
                    ((DataGridView)sender)[col, e.RowIndex].Value = CyGeneralParams.STR_SLIDER_TYPE[0];
                }
                col = 2;
                int minValue = 2;
                res = dgNumberValidating(sender, e.RowIndex, col, minValue, false);
        ...
        int dgNameValidating(object sender, int row, int col)
        {
            ...
            catch
            {
                if (res != 2)//Remove message, if user hasn't define any value
                    MessageBox.Show("Please input correct Name!", "Warning", 
        ...
        int dgNumberValidating(object sender, int row, int col, int minValue, bool isResolution)
        {
            ...
            {
                if (res != 2)//Remove message, if user hasn't define any value
                    MessageBox.Show("Please input correct " + valueType + "!", "Warning", 
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            return res;
    ...
    public class CyColLocation : DataGridViewComboBoxColumn
    {
        public static string[] LIST_SIDES_DGSTR = new string[2] { "Left", "Right" };

        public static E_EL_SIDE GetSide(object side)
        {
            E_EL_SIDE res = E_EL_SIDE.None;
            if (Convert.ToString(side) == LIST_SIDES_DGSTR[0]) res = E_EL_SIDE.Left;
            if (Convert.ToString(side) == LIST_SIDES_DGSTR[1]) res = E_EL_SIDE.Right;

            return res;
        }
        public static string GetString(E_EL_SIDE side)
        {
            string res = LIST_SIDES_DGSTR[0];
            if (side == E_EL_SIDE.Right) res = LIST_SIDES_DGSTR[1];
            return res;
        }
        ...
        public CyColLocation(string name, string strHeader)
        {
            ...
            this.Width = 56;
            this.Items.AddRange(LIST_SIDES_DGSTR);

        }
        public void ReAssigneItems()
        {
            Items.Clear();
            Items.AddRange(LIST_SIDES_DGSTR);
        }
    }
    #endregion
}
