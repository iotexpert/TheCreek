/*******************************************************************************
* File Name: GraphicLCDIntf_v1_50.v
...
*  |  Bit  |   7   |  6   |  5   |  4   |  3   |  2   |    1     |      0      |
*  +=======+-------+------+------+------+------+------+----------+-------------+
*  | Desc  |                 unused                   |data_valid|f0_half_empty|
*  +=======+-------+------+------+------+------+------+----------+-------------+
*
*    f0_half_empty  =>  0 = there is not room for at least 2 bytes in FIFO 0   
*                       1 = there is room for at least 2 bytes in FIFO 0
*
...
* REGISTER USAGE:
* F0 => Command and Data FIFO
* F1 => not used
* D0 => Read Low Pulse Width Counter Period
* D1 => Read High Pulse Width Counter Period
* A0 => Command/Data reg, Low Pulse Counter internal reg
* A1 => High Pulse Counter internal reg
*
********************************************************************************
...
`include "cypress.v"
`ifdef GraphicLCDIntf_V_v1_50_ALREADY_INCLUDED
`else
`define GraphicLCDIntf_V_v1_50_ALREADY_INCLUDED

module GraphicLCDIntf_v1_50 (
    ...
    ***************************************************************************/
    ...
    parameter [7:0] ReadHiPulse = 8'd13;
    parameter [7:0] ReadLoPulse = 8'd13;
    ...
    ***************************************************************************/
    ...
    wire lsb_empty;      /* FIFO0 LSB empty status signal                 */
    wire msb_empty;      /* FIFO0 MSB empty status signal                 */
    wire f0_half_empty;  /* FIFO0 at least half emty status to system bus */ 
    ...
    assign cmd_ready = ~lsb_empty;
        
    assign data_ready = (BitWidth == BIT_WIDTH_8) ? ~lsb_empty : ~(lsb_empty | msb_empty);  
    ...
    ***************************************************************************/
    ...
    localparam FIFO_HALF_EMPTY = 3'd0; /* there is room for at least 2 bytes in the FIFO */
    localparam DATA_IS_VALID   = 3'd1; /* read data is valid */ 
    
    wire [7:0] status;
    assign status[FIFO_HALF_EMPTY] = f0_half_empty;  /* transparent */
    assign status[DATA_IS_VALID]   = data_valid;     /* sticky      */
    ...
    */
    ...
    localparam [3:0] GRAPH_LCD_INTF_STATE_WRITE           = 4'd3;
    ...
    begin
        ...
        begin
            state <= GRAPH_LCD_INTF_STATE_WRITE;
        end
        GRAPH_LCD_INTF_STATE_WRITE:
        begin
            state <= (cmd_ready)? GRAPH_LCD_INTF_STATE_LOAD_CMD : GRAPH_LCD_INTF_STATE_IDLE;
        end
    ...
    always @(posedge op_clk)
    begin
        ncs <= ~(state == GRAPH_LCD_INTF_STATE_LOAD_DATA | state == GRAPH_LCD_INTF_STATE_WRITE |
    ...
    always @(posedge op_clk)
    begin
        oe <= state == GRAPH_LCD_INTF_STATE_LOAD_DATA | state == GRAPH_LCD_INTF_STATE_WRITE;
    end
    ...
    localparam lsb_config = {
        ...
        `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
        `CS_CMP_SEL_CFGA, /*CS_REG1 Comment:LOAD_CMD */
        `CS_ALU_OP_PASS, `CS_SRCA_A0, `CS_SRCB_D0,
        `CS_SHFT_OP_PASS, `CS_A0_SRC___F0, `CS_A1_SRC_NONE,
        `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
        `CS_CMP_SEL_CFGA, /*CS_REG2 Comment:LOAD_DATA */
        `CS_ALU_OP_PASS, `CS_SRCA_A0, `CS_SRCB_D0,
        `CS_SHFT_OP_PASS, `CS_A0_SRC_NONE, `CS_A1_SRC_NONE,
        `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
        ...
        `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
        `CS_CMP_SEL_CFGA, /*CS_REG4 Comment:LOAD_LO_PULSE */
        `CS_ALU_OP_PASS, `CS_SRCA_A0, `CS_SRCB_D0,
        `CS_SHFT_OP_PASS, `CS_A0_SRC_NONE, `CS_A1_SRC___D1,
        `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
        `CS_CMP_SEL_CFGA, /*CS_REG5 Comment:LOAD_HI_PULSE */
        `CS_ALU_OP__DEC, `CS_SRCA_A0, `CS_SRCB_D0,
        `CS_SHFT_OP_PASS, `CS_A0_SRC__ALU, `CS_A1_SRC_NONE,
        `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
        `CS_CMP_SEL_CFGA, /*CS_REG6 Comment:COUNT_LO */
        `CS_ALU_OP__DEC, `CS_SRCA_A1, `CS_SRCB_D0,
        `CS_SHFT_OP_PASS, `CS_A0_SRC_NONE, `CS_A1_SRC__ALU,
        `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
        `CS_CMP_SEL_CFGA, /*CS_REG7 Comment:COUNT_HI */
        8'hFF, 8'h00,	/*SC_REG4	Comment: */
        8'h01, 8'h01,	/*SC_REG5	Comment: */
    ...
    generate
        ...
        begin: GraphLcd8
            ...
            (
                ...
                /* output */         .cmsb(), 			             
                /* output */         .so(), 			             
                /* output */         .f0_bus_stat(f0_half_empty), 	 
                /* output */         .f0_blk_stat(lsb_empty),
                /* output */         .f1_bus_stat(), 	     
                /* output */         .f1_blk_stat(), 	     
                /* input */          .ci(), 				 
                /* output */         .co(), 			     
        ...
        begin: GraphLcd16
            ...
            cy_psoc3_dp #(.cy_dpconfig(
            {
                ...
                `CS_SHFT_OP_PASS, `CS_A0_SRC___F0, `CS_A1_SRC_NONE,
                `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
                `CS_CMP_SEL_CFGA, /*CS_REG1 Comment:LOAD_DATA */
                `CS_ALU_OP_PASS, `CS_SRCA_A0, `CS_SRCB_D0,
                `CS_SHFT_OP_PASS, `CS_A0_SRC_NONE, `CS_A1_SRC_NONE,
                `CS_FEEDBACK_DSBL, `CS_CI_SEL_CFGA, `CS_SI_SEL_CFGA,
                `CS_CMP_SEL_CFGA, /*CS_REG0 Comment: IDLE */
...
`endif /* GraphicLCDIntf_V_v1_50_ALREADY_INCLUDED */
