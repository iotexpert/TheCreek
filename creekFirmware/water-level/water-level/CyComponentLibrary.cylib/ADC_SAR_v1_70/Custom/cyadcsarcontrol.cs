...
namespace ADC_SAR_v1_70
{
    public partial class CySARADCControl : UserControl, ICyParamEditingControl
    {
        ...
        private void numRefVoltage_TextChanged(object sender, EventArgs e)
        {
            ...
            if (value < CyParamRange.REF_VOLTAGE_MIN || value > m_params.m_systemVdda)
            {
                errorProvider.SetError(currentNumeric, string.Format(Resources.VoltageReferenceErrorMsg,
                    CyParamRange.REF_VOLTAGE_MIN, m_params.m_systemVdda));
            }
            else
            {
                errorProvider.SetError(currentNumeric, string.Empty);
            ...
            if (cbInputRange.Text.Contains(RANGE_VDDA) || cbInputRange.Text.Contains(RANGE_VDAC))
            {
                ...
                else if (cbInputRange.Text.Contains(RANGE_VDDA))
                {
                    if (isInternalRef)
                    {
                        ...
                        string item1 = CyDictParser.GetDictValue(m_params.m_dnDict,
                            CyEAdcInputRangeType.Vssa_to_Vdda);
                        string item2 = CyDictParser.GetDictValue(m_params.m_dnDict,
                            CyEAdcInputRangeType.Vneg_Vdda_Diff);
                        string item3 = CyDictParser.GetDictValue(m_params.m_dnDict,
                            CyEAdcInputRangeType.Vneg_Vdda_2_Diff);
                        if (cbInputRange.SelectedItem.ToString() == item2)
                        {
        ...
        {
            return (uint)Math.Ceiling(((double)CyParamRange.CLOCK_FREQ_MIN_HZ / (double)(GetResolution()
                + m_params.SamplePrecharge)));
        }

        // Returns actual resolution value or 12 for PSoC5 ES1
        private byte GetResolution()
        {
            byte defaultResolution = 12;
            byte resolution = defaultResolution;
