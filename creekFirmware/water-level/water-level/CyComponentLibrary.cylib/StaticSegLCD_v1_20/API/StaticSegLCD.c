...
{
   `$INSTANCE_NAME`_LCD_ISR_Disable();
}
...
uint8 `$INSTANCE_NAME`_WritePixel(uint8 PixelNumber, uint8 PixelState)
{
	...
	else
	{
	   ...
	   PixelState ^= 0x01;
	   `$INSTANCE_NAME`_Buffer[(Row + 1) * 16 + ByteAddr] = (`$INSTANCE_NAME`_Buffer[(Row + 1) * 16 + ByteAddr] & Mask) | (PixelState << BitAddr);  
	   status = CYRET_SUCCESS;
	}
	return(status);
}
