    ...
    public class CySARADCParameters
    {
        ...
        private void InitializeSiliconDependentVariables(string archFamilyMemberName)
        {
            ...
            else
            {
                ...
                if (archFamilyMemberName == "PSoC5A")
                {
        ...
        {
            if (m_externalClock >= 0)
                return (uint)Math.Round((m_externalClock * 1000) / ((double)m_actualResolution
                    + m_SamplePercharge));
            else return 0;
        }
        ...
        private CyCustErr SetParam<T>(string paramName, T value, bool toLowerCase)
        {
            ...
            if ((m_inst as ICyInstEdit_v1).SetParamExpr(paramName, value.ToString()))
            {
                err = CyCustErr.OK;
            }
            return err;
