...
//            code += GenerateDescrByte("iSerialNumber", d.iSerialNumber) + COMMA_NEWLINE;
    ...
    class UsbInterfaceGeneralDescriptorGen : UsbCodeGen
    {
    ...
    class UsbDeviceTableGen : UsbCodeGen
    {
        ...
        public override string Generate(DescriptorNode node, string instName, string qualName)
        {
            ...
            {
                string qual = qualName + "_" + node.Nodes[i].Value.bDescriptorType + i;//+ i.ToString;
                code += this.GenLUTEntry(qual, "_TABLE", (byte) node.Nodes[i].Nodes.Count);
            }
            code += GenFooter(node.Value, instName, qualName);
    ...
    class UsbConfigTableGen : UsbCodeGen
    {
        ...
        private string GenInterfaceTblPtr(DescriptorNode node, string instanceName, string qualName)
        {
            ...
            // for (int i = 0; i < node.Nodes.Count; i++)
            {
                if ((d.bInterfaceClass == CyUSBFSParameters.CLASS_NONE) || (d.bInterfaceClass == CyUSBFSParameters.CLASS_VENDORSPEC))
                {
                ...
                foreach (DescriptorNode igd in node.Nodes)
                {
                    foreach (DescriptorNode id in igd.Nodes)
                    {
                        ...
                        foreach (DescriptorNode en in id.Nodes)
                        {
                            ...
                            {
                                code += FormatEndpointSetting(CurInterface, CurAltSetting, (EndpointDescriptor)en.Value);
                            }
                        }
                    }
                }
    ...
    class UsbCodeGenMaker
    {
        ...
        public UsbCodeGen MakeDescriptorGenerator(USBDescriptor d, string instName)
        {
            ...
            if (d_t == Type.GetType("USBFS_v1_10.DeviceDescriptor"))
            {
                c = new UsbDeviceDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.ConfigDescriptor"))
            {
                c = new UsbConfigDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.InterfaceGeneralDescriptor"))
            {
                c = new UsbInterfaceGeneralDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.InterfaceDescriptor"))
            {
                c = new UsbInterfaceDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.EndpointDescriptor"))
            {
                c = new UsbEndpointDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.HIDDescriptor"))
            {
                c = new UsbHIDClassDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.StringDescriptor"))
            {
                c = new UsbStringDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.StringZeroDescriptor"))
            {
                c = new UsbStringZeroDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.HIDReportDescriptor"))
            {
                c = new UsbHIDReportDescriptorGen(instName);
            }
            else if (d_t == Type.GetType("USBFS_v1_10.HIDReportItemDescriptor"))
            {
