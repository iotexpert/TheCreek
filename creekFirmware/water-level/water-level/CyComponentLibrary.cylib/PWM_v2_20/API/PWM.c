...
void `$INSTANCE_NAME`_SetCompareMode(uint8 comparemode) `=ReentrantKeil($INSTANCE_NAME . "_SetCompareMode")`
{
    ...
    #elif (`$INSTANCE_NAME`_UseControl)
        uint8 comparemode1masked = (comparemode << `$INSTANCE_NAME`_CTRL_CMPMODE1_SHIFT) & 
                                    `$INSTANCE_NAME`_CTRL_CMPMODE1_MASK;
        uint8 comparemode2masked = (comparemode << `$INSTANCE_NAME`_CTRL_CMPMODE2_SHIFT) & 
                                   `$INSTANCE_NAME`_CTRL_CMPMODE2_MASK;
        /*Clear existing mode */
        `$INSTANCE_NAME`_CONTROL &= ~(`$INSTANCE_NAME`_CTRL_CMPMODE1_MASK | `$INSTANCE_NAME`_CTRL_CMPMODE2_MASK); 
...
void `$INSTANCE_NAME`_SetCompareMode2(uint8 comparemode) `=ReentrantKeil($INSTANCE_NAME . "_SetCompareMode2")`
{
    ...
    #elif (`$INSTANCE_NAME`_UseControl)
        uint8 comparemodemasked = (comparemode << `$INSTANCE_NAME`_CTRL_CMPMODE2_SHIFT) & 
                                             `$INSTANCE_NAME`_CTRL_CMPMODE2_MASK;
...
void `$INSTANCE_NAME`_WriteDeadTime(uint8 deadtime) `=ReentrantKeil($INSTANCE_NAME . "_WriteDeadTime")`
{
    /* If using the Dead Band 1-255 mode then just write the register */
        ...
        `$INSTANCE_NAME`_DEADBAND_COUNT &= ~`$INSTANCE_NAME`_DEADBAND_COUNT_MASK; 
            /* Set new dead time */
        `$INSTANCE_NAME`_DEADBAND_COUNT |= (deadtime << `$INSTANCE_NAME`_DEADBAND_COUNT_SHIFT) & 
