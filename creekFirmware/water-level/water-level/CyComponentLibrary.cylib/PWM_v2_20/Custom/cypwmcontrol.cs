...
namespace PWM_v2_20
{
    public partial class CyPWMControl : UserControl
    {
        ...
        public void UpdateFormFromParams(ICyInstEdit_v1 inst)
        {
            ...
            PeriodValue16 = PeriodDefaultValue;
            try
            {
                ...
                switch (wfnames[i])
                {
                    ...
                    case PWM1:
                       ...
                       // "Less" OR "Less than or equal" compare type 
                        ...
                        else if (prms.CompareType1.Value == Greater_Str || prms.CompareType1.Value == Greater_Equal_Str || prms.CompareType1.Value == FirmwareControl_Str)
                        {
                            StartHigh = true;
                            comparestring1 = " > " + prms.CompareValue1.Value;
                            if ((prms.CompareType1.Value == Greater_Equal_Str) || (prms.CompareType1.Value == FirmwareControl_Str))
                            {
                                Edge1 += bitwidth;
                                Edge1_2 += bitwidth;
                                comparestring1 = " >= " + prms.CompareValue1.Value;
                            }
                            if (((CompareValue32 >= PeriodValue32) && prms.CompareType1.Value == Greater_Str) ||
                              ((CompareValue32 > PeriodValue32) && ((prms.CompareType1.Value == Greater_Equal_Str) || (prms.CompareType1.Value == FirmwareControl_Str))))
                            {
                                pts[0] = new PointF(startleft, StartHigh ? LowY : HighY);
                                pts[1] = pts[0];
                                pts[2] = pts[0];
                    ...
                    case PWM2:
                        ...
                        else if (prms.CompareType2.Value == Greater_Str || prms.CompareType2.Value == Greater_Equal_Str || prms.CompareType2.Value == FirmwareControl_Str)
                        {
                            StartHigh = true;
                            comparestring1 = " > " + prms.CompareValue2.Value;
                            if ((prms.CompareType2.Value == Greater_Equal_Str) || (prms.CompareType2.Value == FirmwareControl_Str))
                            {
                                Edge1 += bitwidth;
                                Edge1_2 += bitwidth;
                                comparestring1 = " >= " + prms.CompareValue2.Value;
                            ...
                            if (((Compare2Value32  >= PeriodValue32) && prms.CompareType2.Value == Greater_Str) ||
                              ((Compare2Value32  > PeriodValue32) && ((prms.CompareType2.Value == Greater_Equal_Str) || (prms.CompareType2.Value == FirmwareControl_Str))))
                            {
                                pts[0] = new PointF(startleft, StartHigh ? LowY : HighY);
                                pts[1] = pts[0];
                                pts[2] = pts[0];
                    ...
                    case PWM:
                        ...
                        if (prms.PWMMode.Value == One_Output_Str)
                        {
                            ...
                            if (prms.CompareType1.Value == Less_Str || prms.CompareType1.Value == Less_Equal_Str)
                            {
                                ...
                                prms.CompareType1.Value == FirmwareControl_Str)
                            {
                                StartHigh = true;
                                comparestring1 = " > " + prms.CompareValue1.Value;
                                if ((prms.CompareType1.Value == Greater_Equal_Str) || (prms.CompareType1.Value == FirmwareControl_Str))
                                ...
                                if (((CompareValue32 >= PeriodValue32) && 
                                    prms.CompareType1.Value == Greater_Str) || ((CompareValue32 > PeriodValue32)
                                    && ((prms.CompareType1.Value == Greater_Equal_Str) || (prms.CompareType1.Value == FirmwareControl_Str))))
                                {
                                    pts[0] = new PointF(startleft, StartHigh ? LowY : HighY);
                                    pts[1] = pts[0];
                                    pts[2] = pts[0];
                        ...
                        if (prms.PWMMode.Value == Dual_Edge_Str)
                        {
                            ...
                            if (prms.CompareType2.Value == Less_Str || prms.CompareType2.Value == Less_Equal_Str)
                            {
                                ...
                                prms.CompareType2.Value == FirmwareControl_Str)
                            {
                                cmptype2 = "1";
                                if ((prms.CompareType2.Value == Greater_Equal_Str) || (prms.CompareType2.Value == FirmwareControl_Str))
                                {
        ...
        private void m_numDeadBandCounts_Validating(object sender, CancelEventArgs e)
        {
            if (m_cbFFDeadBandMode.Visible)
            {
                ...
                {
                    ep_Errors.SetError(m_numDeadBandCounts, 
                    "Dead Band Counts Must Be Between 0 and 3 for Fixed Function Implementation");
                    e.Cancel = true;
                }
                else
                {
            ...
            if (m_cbDeadBandMode.SelectedIndex == Clock1)
            {
                ...
                if (m_numDeadBandCounts.Value < DEADBANDCOUNTS_MINIMUM || 
                    m_numDeadBandCounts.Value > DEADBANDCOUNTS_4MAXIMUM)
                {
            ...
            else
            {
                // Verify not exceeding minimum or maximum of this mode 
                if (m_numDeadBandCounts.Value < DEADBANDCOUNTS_MINIMUM || 
                    m_numDeadBandCounts.Value > DEADBANDCOUNTS_256MAXIMUM)
                {
                    ep_Errors.SetError(m_numDeadBandCounts, 
                                       string.Format("Dead Band Counts Must Be Between 1 and 256"));
                    e.Cancel = true;
                    return;
                }
            }
    ...
    /// <summary>
    /// Custom Event Args for the UpButtonEvent Delegate needs 
    /// to allow the event to be cancelled before the value is changed
    /// </summary>
    ...
    /// <summary>
    /// Custom Event Args for the DownButtonEvent Delegate needs to allow the event 
    /// to be cancelled before the value is changed
    /// </summary>
