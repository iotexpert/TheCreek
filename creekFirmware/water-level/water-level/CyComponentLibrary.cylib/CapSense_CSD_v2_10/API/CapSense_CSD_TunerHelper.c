...
#include "`$INSTANCE_NAME`_TunerHelper.h"
...
#if ((`$INSTANCE_NAME`_TUNER_API_GENERATE) && (`$INSTANCE_NAME`_TUNING_METHOD != `$INSTANCE_NAME`_NO_TUNING))
    ...
    volatile `$INSTANCE_NAME`_MAILBOXES `$INSTANCE_NAME`_mailboxesComm;
        ...
        extern uint8 `$INSTANCE_NAME`_position[`$INSTANCE_NAME`_TOTAL_CENTROIDS_COUNT];
...
void `$INSTANCE_NAME`_TunerComm(void)
{
    ...
    *******************************************************************************/
    void `$INSTANCE_NAME`_ProcessAllWidgets(volatile `$INSTANCE_NAME`_OUTBOX *outbox)
    {
        ...
        #if (`$INSTANCE_NAME`_TOTAL_CENTROIDS_COUNT && (`$INSTANCE_NAME`_TUNING_METHOD != `$INSTANCE_NAME`_NO_TUNING))
        ...
        /* Calculate widget with centroids */
            ...
            {
                if(`$INSTANCE_NAME`_GetTouchCentroidPos(i) == 0u)
                {
                ...
                {
                    outbox->position[i] = `$INSTANCE_NAME`_SWAP_ENDIAN16( (uint16) `$INSTANCE_NAME`_position[i]);
                    outbox->position[i+1] = `$INSTANCE_NAME`_SWAP_ENDIAN16( (uint16) `$INSTANCE_NAME`_position[i+1]);
                }
            }
        #endif  /* End (`$INSTANCE_NAME`_TOTAL_TOUCH_PADS_COUNT) */
