...
/* Internal functions */
uint8   `$INSTANCE_NAME`_DmaConfigure(void) `=ReentrantKeil($INSTANCE_NAME . "_DmaConfigure")`;
void    `$INSTANCE_NAME`_DmaDispose(void) `=ReentrantKeil($INSTANCE_NAME . "_DmaDispose")`;
...
*
* Return:
*  Status one of standard status for PSoC5 Component:
...
*
*******************************************************************************/
uint8 `$INSTANCE_NAME`_Start(void) `=ReentrantKeil($INSTANCE_NAME . "")`
{
    uint8 status = CYRET_LOCKED;
