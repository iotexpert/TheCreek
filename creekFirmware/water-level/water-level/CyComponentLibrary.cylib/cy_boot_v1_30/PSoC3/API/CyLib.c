...
********************************************************************************
* Summary:
*   Alows the caller to set the power mode for the Internal Low Speed Oscillator
*   
*
* Parameters:  
...
#define CYDELAY_LOOP_CYCLES         210
...
#define CYINT_VECTORS       ((cyisraddress XDATA *) CYDEV_INTC_BASE)
#define CYINT_PRIORITY      ((reg8 *) CYDEV_INTC_PRIOR0)
#define CYINT_ENABLE        ((reg8 *) CYDEV_INTC_SET_EN0)
#define CYINT_CLEAR         ((reg8 *) CYDEV_INTC_CLR_EN0)
#define CYINT_SET_PEND      ((reg8 *) CYDEV_INTC_SET_PD0)
#define CYINT_CLR_PEND      ((reg8 *) CYDEV_INTC_CLR_PD0)
...
cyisraddress CyIntSetVector(uint8 number, cyisraddress address)
{
    ...
    oldIsr = (cyisraddress) CYINT_VECTORS[number];

    /* Set new Interrupt service routine. */
    CY_SET_REG16((uint16 XDATA *) &CYINT_VECTORS[number], (uint16) address);
...
{
    return (cyisraddress) CY_GET_REG16((uint16 XDATA *) &CYINT_VECTORS[number]);
}
...
void CyIntEnable(uint8 number)
{
    ...
    enableReg = CYINT_ENABLE + (number >> 3);
...
uint8 CyIntGetState(uint8 number)
{
    ...
    /* Get a pointer to the Interrupt enable register. */
    stateReg = CYINT_ENABLE + (number >> 3);
...
void CyIntSetPending(uint8 number)
{
    ...
    pendReg = CYINT_SET_PEND + (number >> 3);
...
void CyIntClearPending(uint8 number)
{
    ...
    pendReg = CYINT_CLR_PEND + (number >> 3);
