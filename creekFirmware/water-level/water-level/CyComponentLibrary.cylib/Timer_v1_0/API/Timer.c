...
void `$INSTANCE_NAME`_Start(void) 
{
    ...
    {
        #if (!`$INSTANCE_NAME`_UsingFixedFunction && `$INSTANCE_NAME`_SoftwareCaptureMode)
            uint8 ctrl;
        #endif
...
void `$INSTANCE_NAME`_SetTriggerMode( uint8 triggermode )
{
    ...
    triggermode &= `$INSTANCE_NAME`_CTRL_TRIG_MODE_MASK;
        ...
        /* Write The New Setting */   
        `$INSTANCE_NAME`_CONTROL |= triggermode; 
    #endif 
}
#endif
...
{
    while(`$INSTANCE_NAME`_ReadStatusRegister() & `$INSTANCE_NAME`_STATUS_FIFONEMP_SHIFT)
    {
