...
`include "cypress.v"
`ifdef CRC_v1_10_V_ALREADY_INCLUDED
`else
`define CRC_v1_10_V_ALREADY_INCLUDED
...
module CRC_v1_10
  (
   ...
   localparam [7:0] CRC_RUNMODE_CLOCKED         = 8'd0;
   localparam [7:0] CRC_RUNMODE_API_SINGLE_STEP = 8'd1;
   parameter [0:0] RunMode = CRC_RUNMODE_CLOCKED;
   ...
   localparam CRC_CTRL_RISING_EDGE = 8'h1;   // API single step
   ...
   wire       api_clock = control[CRC_CTRL_RISING_EDGE];
 ...
 // assign ctrl_enable_final = ctrl_enable & enable;
   ...
   generate
      if (RunMode == CRC_RUNMODE_CLOCKED)
	assign clk = clock;
      else
   ...
   generate
	 ...
	 // SYNC SO
	 dtrig sync(so_reg, so, 1'b0, dff_reset, clk);
	 ...
	 // CI
	 dtrig d1(ci_temp, temp_ci, 1'b0, dff_reset, nclk);
	 assign temp_ci = dcfg ? (sc_out[dpPOVal]) : (ci_temp );
   
   assign ci = temp_ci ^ di;
  
	 // SI 
	 dtrig d2(sc_temp, sc_out[dpPOVal], 1'b0, dff_reset, nclk);
...
//   assign si = temp_si;
	...
	// assign bitstream = ci_temp;
      ...
      end // else: !if(Resolution <= CRC_8_BIT)
...
`endif      /* CRC_v1_10_V_ALREADY_INCLUDED */
