...
********************************************************************************/
`include "cypress.v"
`ifdef B_SPI_Slave_v2_10__V_ALREADY_INCLUDED
`else
`define B_SPI_Slave_v2_10__V_ALREADY_INCLUDED

module B_SPI_Slave_v2_10(
...
***************************************************************************/    
    ...
    begin:es2
        ...
        B_SPI_Slave_ES2_v2_10 #(.ModePOL(ModePOL), .ModeCPHA(ModeCPHA),.ShiftDir(ShiftDir),
        ...
        B_SPI_Slave_ES3_v2_10 #(.ModePOL(ModePOL), .ModeCPHA(ModeCPHA),.ShiftDir(ShiftDir),
                          .NumberOfDataBits(NumberOfDataBits),.BidirectMode(BidirectMode))
        SPISlave(
            .reset(reset),
            .clock(clock),
	...
	`ifdef B_SPI_Slave_ES3_v2_10__V_ALREADY_INCLUDED
	`else
	`define B_SPI_Slave_ES3_v2_10__V_ALREADY_INCLUDED
	
    module B_SPI_Slave_ES3_v2_10(
        input  wire reset,     /* System Reset             */
        input  wire clock,     /* System Clk - 2x Bit rate */
        input  wire mosi,      /* SPI MOSI input           */
        input  wire sclk,      /* SPI SCLK input           */
    ...
    reg dpcounter_one_dbl_reg;
	reg mosi_buf_overrun_reg;
	reg dpMOSI_fifo_full_reg;
		
    wire tx_load           = (ModeCPHA == 1'b0) ? (load & (~dpMISO_fifo_empty)) : dpcounter_zero;
    wire dpcounter_one_inv = ~dpcounter_one_reg;    
	wire byte_complete     = dpcounter_one_fin & dpcounter_one_dbl_reg;
    ...
    always@(posedge clock_fin) begin
		...
		dpcounter_one_dbl_reg     <= dpcounter_one_reg;
        dpcounter_one_fin         <= dpcounter_one_dbl_reg;
        miso_tx_empty_reg         <= dpMISO_fifo_empty;
        miso_tx_empty_reg_fin     <= miso_tx_empty_reg;
		mosi_buf_overrun_reg      <= mosi_buf_overrun;
		dpMOSI_fifo_full_reg      <= dpMOSI_fifo_full;
	end 
    ...
    assign rx_status[SPIS_STS_RX_FIFO_OVERRUN_BIT]     = mosi_buf_overrun_reg;
	...
	`endif /* B_SPI_Slave_ES3_v2_10__V_ALREADY_INCLUDED */
	
	`ifdef B_SPI_Slave_ES2_v2_10__V_ALREADY_INCLUDED
	`else
	`define B_SPI_Slave_ES2_v2_10__V_ALREADY_INCLUDED

	module B_SPI_Slave_ES2_v2_10(
		input  wire reset,     /*System Reset */
		input  wire clock,     /* System Clk - 2x Bit rate */
		input  wire mosi,      /* SPI MOSI input */
		input  wire sclk,      /* SPI SCLK input */
	...
	reg  miso_tx_empty_reg_fin;
	reg dpcounter_one_dbl_reg;
	reg mosi_rx_full_reg;
	reg mosi_rx_full_dbl_reg;
	...
	wire mosi_fifo_load_sync = dpcounter_one_dbl_reg;
    ...
    always@(posedge clock_fin) begin
		...
		miso_tx_empty_reg_fin <= miso_tx_empty_reg;
		mosi_rx_full_reg      <= mosi_rx_full;
		mosi_rx_full_dbl_reg  <= mosi_rx_full_reg;
     end 
...
`endif /* B_SPI_Slave_ES2_v2_10__V_ALREADY_INCLUDED */
`endif /* B_SPI_Slave_v2_10__V_ALREADY_INCLUDED */
