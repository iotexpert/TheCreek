...
cystatus CySpcLoadMultiByte(uint8 array, uint16 address, const uint8* buffer, uint8 size) `=ReentrantKeil("CySpcLoadMultiByte")`;
cystatus CySpcLoadRow(uint8 array, const uint8* buffer, uint16 size)`=ReentrantKeil("CySpcLoadRow")`;
cystatus CySpcReadMultiByte(uint8 array, uint8 ecc, uint16 address, uint8 size)`=ReentrantKeil("CySpcReadMultiByte")`;
cystatus CySpcWriteRow(uint8 array, uint16 address, uint8 tempPolarity, uint8 tempMagnitude) `=ReentrantKeil("CySpcWriteRow")`;
cystatus CySpcProgramRow(uint8 array, uint16 address, uint8 tempPolarity, uint8 tempMagnitude) `=ReentrantKeil("CySpcProgramRow")`;
cystatus CySpcEraseSector(uint8 array, uint16 address)`=ReentrantKeil("CySpcEraseSector")`;

#if(CY_PSOC5A)
    cystatus CySpcGetTemp(uint8 numSamples, uint16 timerPeriod, uint8 clkDivSelect)`=ReentrantKeil("CySpcGetTemp")`;
#else
    cystatus CySpcGetTemp(uint8 numSamples)`=ReentrantKeil("CySpcGetTemp")`;
#endif  /* (CY_PSOC5A) */

cystatus CySpcSetupTs(uint8 seqSelect, uint8 seqFreeze, uint8 clkDivSel, uint8 curvCompEnable) `=ReentrantKeil("CySpcSetupTs")`;
cystatus CySpcDisableTs(void)`=ReentrantKeil("CySpcDisableTs")`;
cystatus CySpcEraseRow(uint8 array, uint16 address, uint8 tempPolarity, uint8 tempMagnitude) `=ReentrantKeil("CySpcEraseRow")`;
