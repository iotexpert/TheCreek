...
void `$INSTANCE_NAME`_Enable(void)
{
    ...
    tmpReg = `$INSTANCE_NAME`_SAR_CSR3_REG;
        ...
        tmpReg &= ~`$INSTANCE_NAME`_SAR_PWR_CTRL_VREF_DIV_BY4;
    #endif /* CY_PSOC5A */
    ...
    /* Vdda/2 reference goes through the buffer */
    #if((`$INSTANCE_NAME`_DEFAULT_RANGE == `$INSTANCE_NAME`__VSSA_TO_VDDA) || \
        (`$INSTANCE_NAME`_DEFAULT_RANGE == `$INSTANCE_NAME`__VNEG_VDDA_2_DIFF))
        tmpReg |= `$INSTANCE_NAME`_SAR_EN_RESVDA_EN;        /* Set bit for VDD/2 mode */
    #else
        tmpReg &= ~`$INSTANCE_NAME`_SAR_EN_RESVDA_EN;
    #endif /* VDDA/2 REFERENCE RANGE */
    ...
    /* Set reference for ADC */
        ...
        #else /* INTREF Bypassed or External */
            `$INSTANCE_NAME`_SAR_CSR6_REG = `$INSTANCE_NAME`_INT_BYPASS_EXT_VREF; /* S2 */
...
void `$INSTANCE_NAME`_Stop(void)
{
    ...
    `$INSTANCE_NAME`_SAR_CSR0_REG &= ~`$INSTANCE_NAME`_SAR_SOF_START_CONV;
        ...
        `$INSTANCE_NAME`_SAR_CSR6_REG &= ~`$INSTANCE_NAME`_INT_BYPASS_EXT_VREF;
        /* Disable reference buffer and reduce the reference power to the minimum */
        `$INSTANCE_NAME`_SAR_CSR3_REG &= ~`$INSTANCE_NAME`_SAR_EN_BUF_VREF_EN; /* VCM buffer is absent in PSOC5A*/
...
*  0 ->    Normal
*  1 ->    Half power
*  2 ->    1/3rd power (1.25 power for PSoC5 LP)
*  3 ->    Quarter power.
*
* Return:
*  None.
...
int8 `$INSTANCE_NAME`_GetResult8( void )
{
        ...
        res = CY_GET_REG16(`$INSTANCE_NAME`_SAR_WRK0_PTR) - `$INSTANCE_NAME`_shift;
