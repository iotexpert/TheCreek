...
namespace ADC_SAR_v1_90
{
    public partial class CySARADCControl : UserControl, ICyParamEditingControl
    {
        ...
        {
            object value = CyDictParser.GetDictKey(m_params.m_dnDict, cbInputRange.Text);
            if (value != null)
                m_params.AdcInputRange = (CyEAdcInputRangeType)value;
        }

        private void SetReference()
        {
            object value = CyDictParser.GetDictKey(m_params.m_dnDict, cbReference.Text);
            if (value != null)
                m_params.AdcReference = (CyEAdcRefType)value;
        }
        ...
        private void cbInputRange_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            UpdateReferenceCombobox(sender, cbInputRange.SelectedItem);
            ValidateReference();
        ...
        private void UpdateReferenceCombobox(object sender, object inputRange)
        {
            ...
            if (((CyEAdcInputRangeType)CyDictParser.GetDictKey(m_params.m_dnDict,
                inputRange.ToString())) == CyEAdcInputRangeType.Vneg_Vdda_Diff)
            {
                if (m_params.m_globalEditMode)
                {
                        ...
                        m_previousReference) || sender == cbInputRange)
                    {
                        ...
                        {
                            RemoveReferenceComboBoxItem(bypassRefItem, previouslySelectedItem,
                                CyEAdcRefType.Int_Ref_Not_Bypassed);
                        }
                    }
                }
                else
                {
                    if (cbReference.Items.Contains(bypassRefItem) &&
                        (referenceSelectedItem != CyEAdcRefType.Int_Ref_Bypass))
                    {
                        RemoveReferenceComboBoxItem(bypassRefItem, previouslySelectedItem,
                            CyEAdcRefType.Int_Ref_Not_Bypassed);
                    }
            ...
            else if (inputRange.ToString().Contains(RANGE_VDAC))
            {
                if (cbReference.Items.Contains(bypassRefItem) == false)
                    cbReference.Items.Add(bypassRefItem);
                if (cbReference.Items.Contains(extRefItem) == true)
                {
        ...
        private void RemoveReferenceComboBoxItem(string bypassRefItem, string previouslySelectedItem,
            CyEAdcRefType newValue)
        {
            cbReference.Items.Remove(bypassRefItem);
            if (previouslySelectedItem == bypassRefItem)
                cbReference.SelectedItem = CyDictParser.GetDictValue(m_params.m_dnDict, newValue);
        }
