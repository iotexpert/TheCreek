...
***************************************/
	...
	#define `$INSTANCE_NAME`_ACK_AND_RECEIVE        (`$INSTANCE_NAME`_CSR = `$INSTANCE_NAME`_CSR_ACK)
	#define `$INSTANCE_NAME`_NAK_AND_RECEIVE        (`$INSTANCE_NAME`_CSR = `$INSTANCE_NAME`_CSR_NAK)

	#define `$INSTANCE_NAME`_TRANSMIT_DATA         (`$INSTANCE_NAME`_CSR  = `$INSTANCE_NAME`_CSR_TRANSMIT)

#endif /* `$INSTANCE_NAME`_MODE_MASTER */

/* Special case: udb needs to ack, ff needs to nak. */
#define `$INSTANCE_NAME`_ACKNAK_AND_TRANSMIT       (`$INSTANCE_NAME`_CSR  = (`$INSTANCE_NAME`_CSR_NAK | `$INSTANCE_NAME`_CSR_TRANSMIT))
...
#define CYBTLDR_RECIVE_MASK     0x7F
#define CYBTLDR_RECIVE_SIZE     (CYBTLDR_RECIVE_MASK + 1)

#define CYBTLDR_TRANSMIT_MASK   0x7F
#define CYBTLDR_TRANSMIT_SIZE   (CYBTLDR_RECIVE_MASK + 1)

#define BTLDR_SIZEOF_READ_BUFFER	CYBTLDR_RECIVE_SIZE
#define BTLDR_SIZEOF_WRITE_BUFFER	CYBTLDR_TRANSMIT_SIZE

#define BTLDR_MAX_PACKET_SIZE		(CYBTLDR_RECIVE_SIZE >> 1) /* Arbitrary for I2C. */
