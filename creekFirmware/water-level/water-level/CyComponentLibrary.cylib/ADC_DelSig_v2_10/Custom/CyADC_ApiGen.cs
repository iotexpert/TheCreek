...
namespace ADC_DelSig_v2_10
{
    partial class CyCustomizer : ICyAPICustomize_v1
    {
        ...
        string[] IdealOddDecGain_ParamName = {"DFLT_IDEAL_ODDDEC_GAIN", "DFLT_IDEAL_ODDDEC_GAIN_CFG2", "DFLT_IDEAL_ODDDEC_GAIN_CFG3", "DFLT_IDEAL_ODDDEC_GAIN_CFG4"};
...
//
// Parameters:  
//  resolution:  Set the ADC resolution 
//  inputRange:  Input range selected.
//  config:  ADC configuration number
//  isPSoC3_ES3:  Flag to identify PSoC3 ES3 or later silicon.
//
// Return: 
//  (void) 
//
...
//  inputMode: ipnut range selcted.
//  config:  configuration selected.
//  isPSOC3_ES3:  silicon selected.
//  vdda: vdda value entered in the system wide resource file. Based on this value
...
void SetRef(uint refMode, uint inputMode, uint config, bool isPSOC3_ES3, float vdda)
{
    ...
    if (inputMode != rc.ADC_IR_VSSA_TO_VDDA)
    {
        switch (refMode)
        {
            ...
            case rc.ADC_EXT_REF_ON_P03:
                DsmRef2Reg[config] = rc.DSM_REF2_S6_EN;
            ...
            case rc.ADC_EXT_REF_ON_P32:
                DsmRef2Reg[config] = rc.DSM_REF2_S2_EN | rc.DSM_REF2_S5_EN | rc.DSM_REF2_S6_EN;
                DsmRef3Reg[config] = rc.DSM_REF3_NO_SW;
                DsmCr17Reg[config] &= ~rc.DSM_EN_BUF_VREF;   // Disable Int Ref Amp 
                break;
                
            case rc.ADC_INT_REF_VDDA_4:
                DsmRef0Reg[config] |= rc.DSM_REFMUX_VDA_4 | rc.DSM_VREF_RES_DIV_EN;
                DsmRef2Reg[config] = rc.DSM_REF2_S3_EN | rc.DSM_REF2_S4_EN | rc.DSM_REF2_S6_EN;
                DsmRef3Reg[config] = rc.DSM_REF3_NO_SW;
                DsmCr17Reg[config] |= rc.DSM_EN_BUF_VREF;  // Enable Int Ref Amp 
                break;
                
            case rc.ADC_INT_REF_VDDA_3:    
                DsmRef0Reg[config] |= rc.DSM_REFMUX_VDA_3 | rc.DSM_VREF_RES_DIV_EN;
                DsmRef2Reg[config] = rc.DSM_REF2_S3_EN | rc.DSM_REF2_S4_EN | rc.DSM_REF2_S6_EN;
                DsmRef3Reg[config] = rc.DSM_REF3_NO_SW;
                DsmCr17Reg[config] |= rc.DSM_EN_BUF_VREF;  // Enable Int Ref Amp 
                break;
    ...
    else  // VSSA to VDDA
    {
        if(isPSOC3_ES3)
        {
            ...
            {
                DsmRef0Reg[config] = (rc.DSM_REFMUX_VDA_3 | rc.DSM_EN_BUF_VREF_INN | rc.DSM_VREF_RES_DIV_EN | rc.DSM_VCMSEL_0_8V);
            }
            else
            {
...
//  refMode:  ADC reference options.
//  refVoltage:  Reference voltage selected in the reference voltage numerical up down menu.
//  config:   ADC configuration number.
//  isPSoC3_ES3:  Flag to identify the silicon version selected is PSoC3 or later
//  vdda:  Vdda value entered in the system wide resource file. Based on this value reference
//         will be selected for Vssa to Vdda input range.
//
// Return: 
//  (void) 
//
...
void SetInputRange(uint inputMode, float refVoltage, uint config, bool isPSOC3_ES3, float vdda)
{
    ...
    {
        if ((vdda < MAX_VDDA3_VALUE) && (inputMode == rc.ADC_IR_VSSA_TO_VDDA) && isPSOC3_ES3)
        {
        ...
        Ideal_OddDec_Gain[config] = (uint)rc.IDEAL_GAIN_CONST;
    }
    else                              // 8 to 15 bits of resolution
    {
        ...
        Ideal_OddDec_Gain[config] = (uint)Math.Truncate(Decimator_Ratio_Gain[resolution] * rc.IDEAL_GAIN_CONST);
    }
    ...
    switch (inputMode)
    {
        ...
        case rc.ADC_IR_VSSA_TO_2VREF: // Single ended, Vssa to 2*Vref
            ...
            {            
                ...
                DecDr1Reg[config] = resSettings.dr1_2VREF_ES2[resolution];
                DecShift1Reg[config] = resSettings.shift1_2VREF_ES2[resolution];

                DecShift2Reg[config] = resSettings.shift2_2VREF_ES2[resolution] - rShiftOffset;
            }
...
void CalcValues(ref Dictionary<string, string> paramDict, bool isPSOC3_ES3, float vdda)
     {
            ...
            for (i = 0; i < rc.MAX_CONFIGS; i++)
            {
                ...
                if (ADC_Input_Range[i] == rc.ADC_IR_VSSA_TO_VDDA)
                {
                    if (vdda < MAX_VDDA3_VALUE)
                    {
                ...
                {
                    if (ADC_Reference[i] == rc.ADC_INT_REF_VDDA_3)
                    {
                        Ref_Voltage[i] = vdda / 3;
                    }
                    else if (ADC_Reference[i] == rc.ADC_INT_REF_VDDA_4)
                    {
                        Ref_Voltage[i] = vdda / 4;
                    }
                }
               ...
               // GVAL reg setting based on Resolution 
                ...
                if (ADC_Resolution[i] > 15)    // 16 to 20 bit Resolution
                {
                    ...
                    if ((vdda < MAX_VDDA3_VALUE) && (ADC_Input_Range[i] == rc.ADC_IR_VSSA_TO_VDDA) && isPSOC3_ES3)
                    {
                        DsmCr5Reg[i] = rc.IpCap1_16_20_Vdda_3;
                        DsmCr6Reg[i] = rc.DacCap_16_20_Vdda_3;
                    }
                    else
                    {
                ...
                else   // 8 to 15 bit resolution
                {
                    ...
                    if ((vdda < MAX_VDDA3_VALUE) && (ADC_Input_Range[i] == rc.ADC_IR_VSSA_TO_VDDA) && isPSOC3_ES3)
                    {
                        DsmCr5Reg[i] = rc.IpCap1_8_15_Vdda_3;
                        DsmCr6Reg[i] = rc.DacCap_8_15_Vdda_3;
                    }
                    else
                    {
                ...
                DsmCr8Reg[i] = rc.DSM_IPCAP2_50_FF | rc.DSM_FCAP2_100_FF; //Common to all resolutions
                DsmCr9Reg[i] = rc.DSM_IPCAP3_50_FF | rc.DSM_FCAP3_300_FF;
                DsmCr10Reg[i] = rc.SumCap12_8_20[ADC_Input_Range[i]];
                DsmCr11Reg[i] = rc.SumCap3Fb_8_20[ADC_Input_Range[i]];
                DsmCr12Reg[i] = rc.SumCapin8_20[ADC_Input_Range[i]];
    ...
    public static class rc
    {
       ...
       public static uint[] MAX_FREQ_8_15_BITS = { 6144000, 6144000, 2200000, 1900000, 6144000, 3700000, 1900000, 
        ...
        public static uint DacCap_16_20_Vdda_3 = (DSM_DACCAP_3200FF | DSM_DACCAP_1600FF | DSM_DACCAP_800FF | DSM_DACCAP_400FF | DSM_DACCAP_192FF | DSM_DACCAP_96FF);
        ...
        public static uint[] IpCap1_16_20 = { 
                            ...
                            (DSM_IPCAP1_800FF | DSM_IPCAP1_400FF | DSM_IPCAP1_192FF),                            // 1100 fF VSSA to VDDA
                            (DSM_IPCAP1_800FF | DSM_IPCAP1_192FF),                                               // 1000 fF VSSA to 6*VREF
        ...
        public static uint[] DacCap_16_20 = { 
                            ...
                            (DSM_DACCAP_3200FF | DSM_DACCAP_1600FF | DSM_DACCAP_800FF | DSM_DACCAP_400FF | DSM_DACCAP_192FF),                           // 4896 fF VSSA to VDDA
                            (DSM_DACCAP_3200FF | DSM_DACCAP_1600FF | DSM_DACCAP_800FF | DSM_DACCAP_400FF),       // 6000 fF VSSA to 6*VREF
        ...
        public const uint ADC_INT_REF_VDDA_3           = 6;
