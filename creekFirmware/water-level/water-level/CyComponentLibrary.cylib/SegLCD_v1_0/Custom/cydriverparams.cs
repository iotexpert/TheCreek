...
namespace SegLCD_v1_0
{
    public partial class CyDriverParams : UserControl
    {
        ...
        public CyDriverParams(LCDParameters parameters)
        {
            InitializeComponent();
        ...
        private void ValidateDoubleValue(object sender, double minVal, double maxVal)
        {
            ...
            {
                errorProvider.SetError((Control)sender, String.Format(LCDParameters.FormatErrorMsg, minVal, maxVal));
            }
            else
            {
        ...
        private void ValidateIntValue(object sender, int minVal, int maxVal)
        {
            ...
            {
                errorProvider.SetError((Control)sender, String.Format(LCDParameters.FormatIntErrorMsg, minVal, maxVal));
            }
            else
            {
                if ((val < minVal) || (val > maxVal))
        ...
        private void numUpDownHiDriveTime_ValueChanged(object sender, EventArgs e)
        {
            if (numUpDownHiDriveTime.Minimum > 0)
                Parameters.HiDriveTime = (byte)(Math.Ceiling((double)numUpDownHiDriveTime.Value / (double)numUpDownHiDriveTime.Minimum));
            else
            {
        ...
        private void numUpDownLowDriveDutyCycle_ValueChanged(object sender, EventArgs e)
        {
            ...
            //Parameters.LowDriveDutyCycleTime = (byte)Math.Ceiling((256 - Parameters.HiDriveTime) * numUpDownLowDriveDutyCycle.Value / 100);
        }
