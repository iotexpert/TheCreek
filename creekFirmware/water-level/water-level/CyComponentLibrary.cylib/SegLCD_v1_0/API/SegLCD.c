...
#include <`$INSTANCE_NAME`_Choop_Clock.h>
...
void `$INSTANCE_NAME`_Stop(void)
{
	...
	`$INSTANCE_NAME``[Choop_Clock]`Disable();
}
...
uint8 `$INSTANCE_NAME`_ReadPixel(uint16 PixelNumber)
{
  ...
  PixelState |= (( `$INSTANCE_NAME`_Buffer[Row * 16 + ByteAddr] >> (7 - BitAddr)) & 1);  
  return (PixelState);
}
