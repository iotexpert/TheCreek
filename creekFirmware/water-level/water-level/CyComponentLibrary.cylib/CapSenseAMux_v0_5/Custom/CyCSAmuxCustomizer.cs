...
namespace CyCustomizer.CapSenseAMux_v0_5
{
        ...
        ICyVerilogCustomize_v1        
    {
            ...
            out string codeSnippet)
        {
            ...
            if (ChannelWidth == DIFFERENTIAL_MUX)
            {
                foreach (string termName in GetOrderedTermNames(termQuery))
                {
                    if (termQuery.GetTermBaseName(termName).Contains(INPUT_B_TERM_BASE_NAME))
                    {
                        string sigSegName = termQuery.GetTermSigSegScalarName(termName);
                        inBSigSegNames.Add(sigSegName);
                    }
                }
        ...
        public CyCustErr CustomizeShapes2(
            ICyInstQuery_v1 instQuery,
            ...
            ICyTerminalEdit_v1 termEdit)
        {
            ...
            foreach (float offset in helper.GetTerminalOffsets())
            {
                ...
                {
                    string termName = INPUT_A_TERM_BASE_NAME + index.ToString();
            ...
            shapeEdit.SetFillColor(shapeTags[shapeTags.Count - 1], Color.LightGray);
