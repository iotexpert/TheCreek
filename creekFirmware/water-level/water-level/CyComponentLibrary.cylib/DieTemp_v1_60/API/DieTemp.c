...
#define GET_TEMPR_TIME                      (1 << (`$INSTANCE_NAME`_NUMBER_OF_SAMPLES + 1)) * \
                                            ...
                                            FRM_EXEC_TIME
#define `$INSTANCE_NAME`_MAX_WAIT           ((GET_TEMPR_TIME) / NANO_TO_MILLI_FACTOR) /* In milli seconds. */
...
*
*******************************************************************************/
cystatus `$INSTANCE_NAME`_GetTempT(int16 * temperature) `=ReentrantKeil($INSTANCE_NAME . "_GetTemp")`
{
    uint32 ticks;
    cystatus status;
...
{
    cystatus status = `$INSTANCE_NAME`_GetTempT(temperature);
    return `$INSTANCE_NAME`_GetTempT(temperature);
}
