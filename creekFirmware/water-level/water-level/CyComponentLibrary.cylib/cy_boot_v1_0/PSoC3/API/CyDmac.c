...
// Functions that should be somewhere....
extern void CyEnterCriticalSection(void);
extern void CyExitCriticalSection(void);





// Where do the following data elements go? TBD.

/* Current Number of free elements in the list. */
static uint8 CyDmaTdCurrentNumber; // change to Count
...
cystatus CyDmaChEnable(uint8 chHandle, uint8 preserveTds)
{
    ...
    if(chHandle < NUMBEROF_CHANNELS)
    {
        DMAC_CH[chHandle].basic_cfg[0] |= (preserveTds) ? 0x21u:0x01u;
        status = CYRET_SUCCESS;
    }
    else
    {
