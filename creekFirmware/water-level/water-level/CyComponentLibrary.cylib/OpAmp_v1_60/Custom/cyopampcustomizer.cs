...
namespace OpAmp_v1_60
{
    ...
    public class CyCustomizer : ICyParamEditHook_v1, ICyDRCProvider_v1
    {
        ...
        const string VerifyPowerLOWModeErrMsg = "DRC CHECK: Low Power Mode is not supported on current version of PSoC3/PSoC5 silicon selected. Please select another default power setting.";
        const string VerifyPowerLPOCModeErrMsg = "DRC CHECK: Low Power Over Compensated Mode is not supported on current PSoC3/PSoC5 silicon selected. Please select another default power setting.";
        ...
        CyCustErr VerifyPowerMode(ICyDRCProviderArgs_v1 drcQuery)
        {
                ...
                if (Convert.ToInt32(powerMode.Value) == LOWPOWER)
                {
                    ...
                    (drcQuery.DeviceQueryV1.SiliconRevision >= PSOC5_ES2 && drcQuery.DeviceQueryV1.IsPSoC5 == true)))
                    {
                        ...
                        return new CyCustErr(VerifyPowerLOWModeErrMsg);
                ...
                if (Convert.ToInt32(powerMode.Value) == LPOC)
                {
                    ...
                    {
                        return new CyCustErr(OpAmp_v1_60.Properties.Resources.VerifyPowerLPOCModeErrMsg);                                                                 
