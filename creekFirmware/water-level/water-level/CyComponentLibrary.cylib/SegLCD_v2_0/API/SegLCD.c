...
*
* Summary:
*  Enables the power to LCD fixed hardware and enables generation of UDB signals.
*
* Parameters:
*  None.
*
...
*  generating frame signal. Allocates required number of TDs and configures them,
...
uint8 `$INSTANCE_NAME`_DmaConfigure(void)
{
    ...
    /* The DMA configuration for Type A and Type B waveforms differs becouse of the their specification.
    * Type A waveforms use only two TDs no matter what is the number of commons and Type B waveforms TDs usage is 
    * dependent on the number of commons.
    */ 
...
*  turns off the power for the entire LCD system so you will 
*
* Parameters:
*  None.
*
...
*
* Theory:
*  Non API function. Users should never use this function directly.
*  To save UDB resources there is only one Control Register
*  used for LCD operation. This Control Register contains Frame bit which is
*  used to generate Frame signal. This is a dynamic signal which is transfered 
*  by the DMA. So all other control bist will be owervriten on every DMA 
*  transaction. That's why all the data that need to be remained in the Control 
*  Register are written to array where predefined values of Frame contain.   
*
*******************************************************************************/
void `$INSTANCE_NAME`_WriteControlReg(uint8 value, uint8 mask)
{
    ...
    for(i = 0; i < `$INSTANCE_NAME`_CONTROL_WRITE_COUNT; i++)
    {
