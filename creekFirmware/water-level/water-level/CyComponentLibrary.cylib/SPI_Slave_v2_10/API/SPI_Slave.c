...
#include "`$INSTANCE_NAME`.h"
    ...
    volatile uint8 `$INSTANCE_NAME`_txBufferFull = 0u;
    ...
    volatile uint8 `$INSTANCE_NAME`_rxBufferFull = 0u;
...
void `$INSTANCE_NAME`_WriteTxData(`$RegSizeReplacementString` txData)
{
        ...
        do
        {
            tmpTxBufferRead = `$INSTANCE_NAME`_txBufferRead - 1u;
        ...
        else
        {
            ...
            if(`$INSTANCE_NAME`_txBufferWrite == `$INSTANCE_NAME`_txBufferRead)
            {
                ...
                if(`$INSTANCE_NAME`_txBufferRead >= `$INSTANCE_NAME`_RXBUFFERSIZE)
                {
                    `$INSTANCE_NAME`_txBufferRead = 0u;
                }
...
uint8 `$INSTANCE_NAME`_GetRxBufferSize(void) `=ReentrantKeil($INSTANCE_NAME . "_GetRxBufferSize")`
{
    uint8 size = 0u;
        ...
        size = ((`$INSTANCE_NAME`_RX_STATUS_REG & `$INSTANCE_NAME`_STS_RX_FIFO_NOT_EMPTY) == 
                 `$INSTANCE_NAME`_STS_RX_FIFO_NOT_EMPTY) ? 1u : 0u;
...
uint8  `$INSTANCE_NAME`_GetTxBufferSize(void) `=ReentrantKeil($INSTANCE_NAME . "_GetTxBufferSize")`
{
    uint8 size = 0u;
        ...
        /* Is the fifo is full. */
        if((size & `$INSTANCE_NAME`_STS_TX_FIFO_EMPTY) == `$INSTANCE_NAME`_STS_TX_FIFO_EMPTY)
        {
            size = 0u;
        }
        else if((size & `$INSTANCE_NAME`_STS_TX_FIFO_NOT_FULL) == `$INSTANCE_NAME`_STS_TX_FIFO_NOT_FULL)
        {
            size = 1u;
        }
        else
...
void `$INSTANCE_NAME`_ClearTxBuffer(void)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        /* Clear TX FIFO */
        `$INSTANCE_NAME`_TX_AUX_CONTROL_DP0_REG |= `$INSTANCE_NAME`_FIFO_CLR;
        `$INSTANCE_NAME`_TX_AUX_CONTROL_DP0_REG &= ~`$INSTANCE_NAME`_FIFO_CLR;
        ...
        `$INSTANCE_NAME`_TX_AUX_CONTROL_DP0_REG |= `$INSTANCE_NAME`_FIFO_CLR;
        `$INSTANCE_NAME`_TX_AUX_CONTROL_DP0_REG &= ~`$INSTANCE_NAME`_FIFO_CLR;
        `$INSTANCE_NAME`_TX_AUX_CONTROL_DP1_REG |= `$INSTANCE_NAME`_FIFO_CLR;
        `$INSTANCE_NAME`_TX_AUX_CONTROL_DP1_REG &= ~`$INSTANCE_NAME`_FIFO_CLR;
...
uint8 `$INSTANCE_NAME`_ReadStatus(void)
{
    uint8 tmpStatus;
        ...
        tmpStatus = `$INSTANCE_NAME`_GET_STATUS_TX(`$INSTANCE_NAME`_swStatusTx) |
                    (`$INSTANCE_NAME`_GET_STATUS_RX(`$INSTANCE_NAME`_swStatusRx) & ~`$INSTANCE_NAME`_STS_RX_FIFO_FULL);
        ...
        tmpStatus = `$INSTANCE_NAME`_TX_STATUS_REG | (`$INSTANCE_NAME`_RX_STATUS_REG &
                                                      ~`$INSTANCE_NAME`_STS_RX_FIFO_FULL);
