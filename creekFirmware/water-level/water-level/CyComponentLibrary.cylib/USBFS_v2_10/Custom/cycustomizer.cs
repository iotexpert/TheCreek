...
//            Debug.Assert(false);
        ...
        public IEnumerable<CyDRCInfo_v1> GetDRCs(ICyDRCProviderArgs_v1 args)
        {
            ...
            #region DRCs for endpoints (for future implemetation)

            // Get all endpoint

            // 1. Do not allow to enter more then 512 bytes for ISO EP when Endpoint Memory Management(EMM) parameter 
            // is not "DMA w/AutomaticMM".

            // 2. Generate error in DRC when MaxPacketSize is greater then 512 and EMM is not "DMA w/AutomaticMM".

            // 3. Generate DRC error when total sum of the MaxPacketSize for all EPs in one Device is greater then 512 
            // when EMM is not "DMA w/AutomaticMM".
            //      a. For "Dynamic Allocation" check the sum for each Alternate settings.
            //      b. For "Static Allocation" select the greater EP MaxPacketSize in concurrent alternate settings.

            // Generate DRC error when total ISO EPs sum is greater then 1100 (this is bandwith limitation).
