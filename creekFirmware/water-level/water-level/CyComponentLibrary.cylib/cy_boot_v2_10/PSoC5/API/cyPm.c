...
void CyPmHibRestore(void) `=ReentrantKeil("CyPmHibRestore")`;

void CyPmHibSlpSaveSet(uint8 powerMode);
void CyPmHibSlpRestore(void)`=ReentrantKeil("CyPmHibSlpRestore")`;
        ...
        */
        ...
        if(CYRET_TIMEOUT == status)
        {
            /* `#START MHZ ECO TIMEOUT` */
                ...
                /* The megahertz crystal is not ready.
...
void CyPmAltAct(uint16 wakeupTime, uint16 wakeupSource)
{
    ...
    if((wakeupTime >= PM_ALT_ACT_TIME_FTW(1u)) && (wakeupTime <= PM_ALT_ACT_TIME_FTW(256u)))
    {
        ...
        #else   /* PSoC3 ES3 and later, PSoC5 ES2 and later */
...
*  to several analog resources to be unreliable when not the device is placed in
*  a low power mode. See the silicon errata "PSoC(R)3: CY8C38 Family Errata Silicon
*  Revision ES2" (http://www.cypress.com/?rID=35179) and App Note AN# 001-63479
*  (Unreliable Analog Routing in PSoC 3 and PSoC 5 Early Silicon) for details.
*
