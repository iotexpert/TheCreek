            ...
            switch (pktCommand)
            {
                ...
                case CYBTLDR_COMMAND_VERIFY:
                    if (pktSize == 3)
                    {
                        ...
                        for (checksum = 0, index = 0; index < CYDEV_FLS_ROW_SIZE; index++)
                        {
                            checksum += CY_GETCODEDATA(rowAddr + index);
                        }

                        #if (CYDEV_ECC_ENABLE == 0)
                        /* Save the ECC area. */
                        rowAddr = CYDEV_ECC_BASE + ((uint32)packetBuffer[CYBTLDR_DATA_ADDR] * (CYDEV_FLS_SECTOR_SIZE / 8)) 
