...
void `$INSTANCE_NAME`_Enable(void) `=ReentrantKeil($INSTANCE_NAME . "_Enable")`
{
    ...
    /* This is only valid if there is an external charpe pump clock */
    #if(`$INSTANCE_NAME`_DEFAULT_CHARGE_PUMP_CLOCK)
...
*
* Global variables:
*  `$INSTANCE_NAME`_conovDone:  Modified when conversion is complete for single
...
*  variable is used for offset callibration purpose.
...
uint16 `$INSTANCE_NAME`_RoundValue(double busClockFreq, double clockFreq) \
                                          `=ReentrantKeil($INSTANCE_NAME . "_round_Value")`
{
     uint16 x;
     double divider;
...
*  and then initializes the registers with the default vaues for that config. 
*  This also performs calibration by wrting GCOR registers with trim values 
*  stored in the flash.
*
* Parameters:  
                                              ...
                                              `=ReentrantKeil($INSTANCE_NAME . "_SelectConfiguration")`
{
    ...
    {   
        ...
        /* Set the proper devider for the internal clock */
        #if(`$INSTANCE_NAME`_DEFAULT_INTERNAL_CLK)
        ...
        /* Compensate the gain */
        `$INSTANCE_NAME`_GainCompensation(inputRange, idealGain, resolution);
...
/*******************************************************************************
* Function Name: `$INSTANCE_NAME`_GainCompensation(uint8, uint16, uint8)
********************************************************************************
*
* Summary:
*  This API calculates the tirm value and then loads this to GCOR register.
*
* Parameters:  
*  inputrange:  input range for which trim value is to be calculated.
*  IdealGain:  Ideal Decimator gain for the selected resolution and conversion 
*              mode.
                                                         ...
                                                         `=ReentrantKeil($INSTANCE_NAME . "_GainCompensation")`
{
    ...
    {
        GcorValue = IdealDecGain;         
    }
    else  /* Compensate for calibration value */
    {
    ...
    if(resolution == 8)
    {
        GcorValue = GcorValue >> 6;
        `$INSTANCE_NAME`_DEC_GVAL_REG = 9; /* Use only 10 bits */
    }
    else if (resolution == 9)
    {
        GcorValue = GcorValue >> 5;
        `$INSTANCE_NAME`_DEC_GVAL_REG = 10; /* Use only 11 bits */
    }
    else if (resolution == 10)
    {
        GcorValue = GcorValue >> 4;
        `$INSTANCE_NAME`_DEC_GVAL_REG = 11; /* Use only 12 bits */
    }
    else
    {
