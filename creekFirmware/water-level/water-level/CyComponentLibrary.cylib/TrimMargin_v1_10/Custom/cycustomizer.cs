    ...
    public partial class CyCustomizer : ICyParamEditHook_v1, ICyAPICustomize_v1, ICyShapeCustomize_v1, ICyDRCProvider_v1
    {
            ...
            IEnumerable<CyAPICustomizer> apis)
        {
            ...
            for (int i = 0; i < parameters.NumConverters; i++)
            {
                ...
                element = Convert.ToUInt16(parameters.VoltagesTable[i].MarginLow * 1000).ToString();
                AddStringElement(ref vMarginLow, element, last);

                element = Convert.ToUInt16(parameters.VoltagesTable[i].MarginHigh * 1000).ToString();
...
//                        );
                ...
                double pwmVMarginLowElement = pwmNomElement + pwmPerMVElement * 
                    ((double)(parameters.VoltagesTable[i].m_nominalVoltage - parameters.VoltagesTable[i].MarginLow));
                ...
                double pwmVMarginHighElement = pwmNomElement - pwmPerMVElement * 
                    ((double)(parameters.VoltagesTable[i].MarginHigh - parameters.VoltagesTable[i].m_nominalVoltage));
