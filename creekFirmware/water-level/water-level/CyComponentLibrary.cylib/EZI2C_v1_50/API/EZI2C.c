...
extern uint8 `$INSTANCE_NAME`_curState;

/* Status byte */
extern uint8 `$INSTANCE_NAME`_curStatus;

/* Pointer to data exposed to I2C Master */
extern uint8 * `$INSTANCE_NAME`_dataPtrS1;
    ...
    extern uint8   `$INSTANCE_NAME`_rwOffsetS1;

    /* Points to next value to be read or written */
    extern uint8   `$INSTANCE_NAME`_rwIndexS1;

    /* Offset where data is read only */
    extern uint8   `$INSTANCE_NAME`_wrProtectS1;

    /* Size of array between 1 and 255 */
    extern uint8   `$INSTANCE_NAME`_bufSizeS1;

#else   /* 16 bit sub-address */

    /* Offset for read and write operations, set at each write sequence */
    extern uint16  `$INSTANCE_NAME`_rwOffsetS1;

    /* Points to next value to be read or written */
    extern uint16  `$INSTANCE_NAME`_rwIndexS1;

    /* Offset where data is read only */
    extern uint16  `$INSTANCE_NAME`_wrProtectS1;

    /* Size of array between 1 and 65535 */
    extern uint16  `$INSTANCE_NAME`_bufSizeS1;
    ...
    /* Pointer to data exposed to I2C Master */
    extern uint8 * `$INSTANCE_NAME`_dataPtrS2;

    /* Software address compare 1 */
    extern uint8   `$INSTANCE_NAME`_addrS1;

    /* Software address compare 2 */
    extern uint8   `$INSTANCE_NAME`_addrS2;

    #if(`$INSTANCE_NAME`_SUBADDR_WIDTH == `$INSTANCE_NAME`_SUBADDR_8BIT)

        /* Offset for read and write operations, set at each write sequence */
        extern uint8   `$INSTANCE_NAME`_rwOffsetS2;

        /* Points to next value to be read or written */
        extern uint8   `$INSTANCE_NAME`_rwIndexS2;

        /* Offset where data is read only */
        extern uint8   `$INSTANCE_NAME`_wrProtectS2;

        /* Size of array between 1 and 255 */
        extern uint8   `$INSTANCE_NAME`_bufSizeS2;

    #else /* 16 bit subaddress */

        /* Offset for read and write operations, set at each write sequence */
        extern uint16  `$INSTANCE_NAME`_rwOffsetS2;

        /* Points to next value to be read or written */
        extern uint16  `$INSTANCE_NAME`_rwIndexS2;

        /* Offset where data is read only */
        extern uint16  `$INSTANCE_NAME`_wrProtectS2;

        /* Size of array between 1 and 65535 */
        extern uint16  `$INSTANCE_NAME`_bufSizeS2;
