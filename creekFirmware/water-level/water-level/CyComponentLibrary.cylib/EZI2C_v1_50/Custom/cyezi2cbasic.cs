...
namespace EZI2C_v1_50
{
    public partial class CyEZI2CBasic : UserControl, ICyParamEditingControl
    {
        ...
        private void m_cbBusSpeed_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                m_errorProvider.SetError((ComboBox)sender, string.Format(
                    Properties.Resources.BusSpeedEP));
                e.Cancel = true;
            }
            else
            {
                m_errorProvider.SetError((ComboBox)sender, "");
            }
        }
        ...
        public bool TextBoxValidating(object sender, CancelEventArgs e)
        {
            ...
            if (m_params.IntValidated(currentTextBox.Text, CyParamNames.I2C_ADDRESS_1) == false)
            {
                ...
                if (sender == m_tbPrimarySlaveAddress)
                    message = Properties.Resources.PrimarySlaveAddressEP;
                else if (sender == m_tbSecondarySlaveAddress)
                    message = Properties.Resources.SecondarySlaveAddressEP;

                m_errorProvider.SetError((TextBox)sender, string.Format(message));
                e.Cancel = true;
                result = false;
            }
            else if (m_tbPrimarySlaveAddress.Text == m_tbSecondarySlaveAddress.Text && m_params.m_i2c_Addresses == 2)
            {
                m_errorProvider.SetError((TextBox)sender, string.Format(
                    Properties.Resources.PrimarySecondaryEqualEP));
            }
            else if (hex1 != hex2)
            {
        ...
        {
            m_errorProvider.SetError(m_tbPrimarySlaveAddress, "");
            m_errorProvider.SetError(m_tbSecondarySlaveAddress, "");
        }
        #endregion
