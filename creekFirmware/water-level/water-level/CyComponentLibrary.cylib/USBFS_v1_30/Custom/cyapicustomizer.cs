...
namespace USBFS_v1_30
{
    public partial class CyCustomizer : ICyAPICustomize_v1
    {
                                                          ...
                                                          IEnumerable<CyAPICustomizer> apis)
        {
            ...
            //Prepare HIDReportTree for parsing (remove comment items)
            for (int i = 0; i < Parameters.HIDReportTree.Nodes[0].Nodes.Count; i++)
                for (int j = 0; j < Parameters.HIDReportTree.Nodes[0].Nodes[i].Nodes.Count; j++)
                {
                    if (CyUSBFSParameters.StringIsComment(
                       ((HIDReportItemDescriptor) Parameters.HIDReportTree.Nodes[0].Nodes[i].Nodes[j].Value).Item.Name))
                    {
    ...
    internal class USBCodeGenerator : UsbCodePrimitives
    {
                ...
                for (int i = 0; i < m_Parameters.HIDReportTree.Nodes[0].Nodes.Count; i++)
                {
                    ...
                    AddDefine("HID_RPT_" + rptIndex + "_SIZE_LSB",
                              String.Format("{0}", HexByteString((byte)
                                               (ReportDescriptorSize(m_Parameters.HIDReportTree.Nodes[0].Nodes[i].Nodes) 
                                                & 0xFF))));
                    AddDefine("HID_RPT_" + rptIndex + "_SIZE_MSB",
                              String.Format("{0}", HexByteString((byte)((ReportDescriptorSize(
                                                   m_Parameters.HIDReportTree.Nodes[0].Nodes[i].Nodes) >> 8) & 0xFF))));
                }
            }
        }
        ...
        private string GenReportTables(DescriptorNode node, string instName, string qualName)
        {
            ...
            for (int i = 0; i < node.Nodes.Count; i++)
            {
                ...
                {
                    qual = qualName + "_INTERFACE" + i;
                }
                else if (node.Nodes[i].Value.bDescriptorType == USBDescriptorType.INTERFACE)
                {
                    qual = qualName + "_ALTERNATE" + i;
                }
                else if (node.Nodes[i].Value.bDescriptorType == USBDescriptorType.HID)
                {
                    qual = qualName + "_" + node.Nodes[i].Value.bDescriptorType;
                }
                else
                {
        ...
        private string GenReportTable(int rptIndex, string instName, string qualName)
        {
            ...
            {
                DescriptorNode ReportTop = m_Parameters.HIDReportTree.Nodes[0].Nodes[rptIndex - 1];
                ...
                code += "/*********************************************************************\r\n";
                code += " HID Report Look Up Table          This table has four entries:\r\n";
                code += "                                         IN Report Table\r\n";
                code += "                                         OUT Report Table\r\n";
                code += "                                         Feature Report Table\r\n";
                code += "                                         HID Report Descriptor\r\n";
                code += " *********************************************************************/\r\n";
                ...
                code += COMMA_NEWLINE;
                code += GenerateLUTEntry(instName + "_HIDREPORT_DESCRIPTOR" + rptIndex + "[0]", 1);
                code += NEWLINE;
                code += GenerateFooter() + ";\r\n";
...
//            code += GenerateDescrByte("iSerialNumber", d.iSerialNumber) + COMMA_NEWLINE;
    ...
    internal class UsbHIDReportDescriptorGen : UsbCodeGen
    {
        ...
        public override string GenFooter(USBDescriptor ud, string instanceName, string qualName)
        {
            string code = "";
            code += "/*********************************************************************/\r\n";
            code += GenerateDescrDWLS("End of the HID Report Descriptor", 0) + "};\r\n";
            code += "/*********************************************************************/\r\n";
            return code;
        }
    }
    ...
    internal class UsbConfigTableGen : UsbCodeGen
    {
        ...
        public override string Generate(DescriptorNode node, string instName, string qualName)
        {
            ...
            for (int i = 0; i < node.Nodes.Count; i++)
            {
                ...
                {
                    /* i,j were exchanged with the express purpose (to have a qual like _INTERFACEx_ALTERNATEx_ instead of
                       _ALTERNATEx_INTERFACEx_)*/
                    string qual = qualName + "_" + node.Nodes[i].Nodes[j].Value.bDescriptorType + i +
                                             "_" + node.Nodes[i].Value.bDescriptorType + j; //+ i.ToString;
    ...
    internal class UsbHIDReportDescriptorGenTree : GenTree
    {
        ...
        public override string Generate(List<DescriptorNode> nodes, string instName, string qualName)
        {
            ...
            if (nodes.Count > 0)
            {
                ...
                {
                    int rptIndex = i + 1;
                    string qualHead = qualName + "_HIDREPORT_DESCRIPTOR" + rptIndex + "[]";
                    code += cg.GenHeader(nodes[i].Value, instName, qualHead);
                   // code += cg.GenBody(nodes[i].Value, instName, qualName);
                    code += prim.GenerateSymbolicByte(" Descriptor Size (Not part of descriptor)",
                                     qualName + "_HID_RPT_" + rptIndex + "_SIZE_LSB" + prim.COMMA_NEWLINE +
                                     qualName + "_HID_RPT_" + rptIndex + "_SIZE_MSB") + prim.COMMA_NEWLINE;
