...
***************************************/
    ...
    `$RxBuffRegSizeReplacementString` `$INSTANCE_NAME`_rxBufferRead = 0u;
...
void  `$INSTANCE_NAME`_WriteControlRegister(uint8 control) `=ReentrantKeil($INSTANCE_NAME . "_WriteControlRegister")`
{
    ...
    *
    *******************************************************************************/
    void `$INSTANCE_NAME`_SetRxAddressMode(uint8 addressMode)  `=ReentrantKeil($INSTANCE_NAME . "_SetRxAddressMode")`
    {
        #if(`$INSTANCE_NAME`_RXHW_ADDRESS_ENABLED)
            #if(`$INSTANCE_NAME`_CONTROL_REG_REMOVED)
    ...
    void `$INSTANCE_NAME`_SendBreak(uint8 retMode)
    {
        ...
        if(`$INSTANCE_NAME`_initVar != 0u)
        {
            ...
            uint8 tmpStat;
                ...
                if( (retMode == `$INSTANCE_NAME`_WAIT_FOR_COMLETE_REINIT) || \
                    (retMode == `$INSTANCE_NAME`_SEND_WAIT_REINIT) )
                {
