...
namespace SegLCD_v1_20
{
    public class CyLCDParameters
    {
        ...
        public byte m_DacDisInitTime = 0;

        private bool m_Gang = false;
        ...
        private void GetParams()
        {
            if (m_inst != null)
            {
                ...
                m_NumCommonLines = Convert.ToByte(m_inst.GetCommittedParam("NumCommonLines").Value);
                m_NumSegmentLines = Convert.ToByte(m_inst.GetCommittedParam("NumSegmentLines").Value);
                m_BiasType = Convert.ToByte(m_inst.GetCommittedParam("BiasType").Value);
                m_WaveformType = Convert.ToByte(m_inst.GetCommittedParam("WaveformType").Value);
                m_FrameRate = Convert.ToByte(m_inst.GetCommittedParam("FrameRate").Value);
                m_BiasVoltage = Convert.ToByte(m_inst.GetCommittedParam("BiasVoltage").Value);

                m_DriverPowerMode = Convert.ToByte(m_inst.GetCommittedParam("DriverPowerMode").Value);
                m_HiDriveTime = Convert.ToByte(m_inst.GetCommittedParam("HiDriveTime").Value);
                m_LowDriveMode = Convert.ToByte(m_inst.GetCommittedParam("LowDriveMode").Value);
                m_LowDriveInitTime = Convert.ToByte(m_inst.GetCommittedParam("LowDriveInitTime").Value);

                m_ClockFrequency = Convert.ToUInt32(m_inst.GetCommittedParam("ClockFrequency").Value);
                m_Gang = Convert.ToBoolean(m_inst.GetCommittedParam("Gang").Value);
                m_DebugMode = Convert.ToBoolean(m_inst.GetCommittedParam("DebugMode").Value);
                m_DisabledCommons = Convert.ToString(m_inst.GetCommittedParam("DisabledCommons").Value);
                DeserializeHelpers(m_inst.GetCommittedParam("Helpers").Value);
            }
        }

        /// <summary>
        /// Deserializes the list of Helper functions from the string stored in Parameters
        /// </summary>
        ...
        public void DeserializeHelpers(string _SHelpers)
        {
                    ...
                    for (int i = 0; i < m_HelpersConfig.Count; i++)
                    {
                        switch (m_HelpersConfig[i].Kind)
                        {
                            ...
                            case CyHelperKind.Segment14:
                                ...
                                {
                                    if (
                                        !m_SymbolIndexes_14SEG.Contains(m_HelpersConfig[i].HelpSegInfo[j].GlobalDigitNum))
                            ...
                            case CyHelperKind.Segment16:
                                ...
                                {
                                    if (
                                        !m_SymbolIndexes_16SEG.Contains(m_HelpersConfig[i].HelpSegInfo[j].GlobalDigitNum))
                                ...
                                for (int j = 0; j < m_HelpersConfig[i].HelpSegInfo.Count; j++)
                                {
                                    if (
                                        !m_SymbolIndexes_MATRIX.Contains(
                                             m_HelpersConfig[i].HelpSegInfo[j].GlobalDigitNum))
                                        m_SymbolIndexes_MATRIX.Add(m_HelpersConfig[i].HelpSegInfo[j].GlobalDigitNum);
                                }
                                break;
                            case CyHelperKind.Empty:
                                // For compability, add pixel's definitions if there are less than needed.
                                    ...
                                    for (int j2 = 0; j2 < NumCommonLines; j2++)
                                    {
                                        if (j1*NumCommonLines + j2 >= m_HelpersConfig[0].HelpSegInfo.Count)
                                        {
                                            m_HelpersConfig[0].HelpSegInfo.Add(new CyHelperSegmentInfo(
                                                                               "PIX" + (j1*NumCommonLines + j2), j2,
                                                                               j1, -1, -1, -1, CyHelperKind.Empty, -1));
                                        }
                                    }
                                break;
                            default:
            ...
            {
                MessageBox.Show("Error in Helpers parameter", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
            ...
            if (m_inst != null)
            {
                switch (paramName)
                {
                    ...
                    case "NumSegmentLines":
                        ...
                        break;
                    case "BiasType":
                        m_inst.SetParamExpr("BiasType", BiasType.ToString());
                        break;
                    case "WaveformType":
                    ...
                    case "FrameRate":
                        ...
                        break;
                    case "BiasVoltage":
                        m_inst.SetParamExpr("BiasVoltage", BiasVoltage.ToString());
                        break;
                    ...
                    case "DacDisInitTime":
                        ...
                        break;
                    case "Gang":
                        m_inst.SetParamExpr("Gang", Gang.ToString().ToLower());
                        break;
                    case "DebugMode":
    ...
    {
        [XmlAttribute("Display")] public int DisplayNum;
        [XmlAttribute("Digit")] public int DigitNum;
        [XmlAttribute("Helper")] public CyHelperKind HelperType;
        [XmlAttribute("RelPos")] public int RelativePos;
    ...
    public class HelperInfo
    {
        ...
        [XmlElementAttribute("Color")] public int m_HelperColor;
        ...
        [XmlArray("HelpSegInfoArray")] [XmlArrayItem("HelperSegmentInfo")] public List<CyHelperSegmentInfo> HelpSegInfo;
    ...
    {
        private readonly List<Color> clArxiv = new List<Color>();
