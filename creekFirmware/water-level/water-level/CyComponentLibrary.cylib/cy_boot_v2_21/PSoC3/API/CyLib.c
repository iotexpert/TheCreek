...
*
*******************************************************************************/
uint8 CyXTAL_32KHZ_ReadStatus() `=ReentrantKeil("CyXTAL_32KHZ_ReadStatus")`
{
    uint8 status;
...
cyisraddress CyIntSetVector(uint8 number, cyisraddress address) `=ReentrantKeil("CyIntSetVector")`
{
    ...
    oldIsr = (cyisraddress) CY_GET_REG16(&CYINT_VECT_TABLE[number]);
...
{
    return (cyisraddress) CY_GET_REG16(&CYINT_VECT_TABLE[number]);
}
