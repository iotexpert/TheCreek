...
/* These macros are used to validate the sector number. 
...
cystatus CySpcReadData(uint8 * buffer, uint8 size) `=ReentrantKeil("CySpcReadData")`;
void CySpcActivePower(uint8 state) `=ReentrantKeil("CySpcActivePower")`;
void CySpcStandbyPower(uint8 state) `=ReentrantKeil("CySpcStandbyPower")`;
