...
/* PSoC3 ES3 or early */
#define `$INSTANCE_NAME`_PSOC3_UPTO_ES3 ((CYDEV_CHIP_MEMBER_USED == CYDEV_CHIP_MEMBER_3A) && \
...
void   `$INSTANCE_NAME`_Resume(void) `=ReentrantKeil($INSTANCE_NAME . "_Resume")`;
    ...
    cystatus `$INSTANCE_NAME`_CyBtldrCommWrite(uint8 *pData, uint16 size, uint16 *count, uint8 timeOut);
...
#define `$INSTANCE_NAME`_bEPHalt                    `$INSTANCE_NAME`_epHalt
...
#else  /* `$INSTANCE_NAME`_PSOC3_ES3 || `$INSTANCE_NAME`_PSOC5_ES2 */  
...
#if (`$INSTANCE_NAME`_MON_VBUS == 1u)
    #define `$INSTANCE_NAME`_VBUS_DR        `$INSTANCE_NAME`_VBUS_DR_PTR
    #define `$INSTANCE_NAME`_VBUS_PS        `$INSTANCE_NAME`_VBUS_PS_PTR
#endif /* End `$INSTANCE_NAME`_MON_VBUS */
