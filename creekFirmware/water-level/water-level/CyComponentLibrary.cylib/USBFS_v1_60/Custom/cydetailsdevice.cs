...
namespace USBFS_v1_60
{
    public partial class CyDetailsDevice : UserControl
    {
        ...
        {
            byte res = 0;
            if (rbStaticAllocation == sender) res = CyUSBFSParameters.MEMORY_ALLOC_STATIC;
            if (rbDynamicAllocation == sender) res = CyUSBFSParameters.MEMORY_ALLOC_DYNAMIC;
            if (!m_internalChanges)
            {
                if (((RadioButton)sender).Checked) m_descriptor.bMemoryAlloc = res;
        ...
        private void rbMemoryMgmt_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (!m_internalChanges)
            {
                if (((RadioButton) sender).Checked) m_descriptor.bMemoryMgmt = res;
                m_parameters.ParamDeviceTreeChanged = true;
            }
        }
        ...
        private void InitFields()
        {
            ...
            // Memory management
            switch (m_descriptor.bMemoryMgmt)
            {
                case 0:
            ...
            // Memory allocation
            switch (m_descriptor.bMemoryAlloc)
            {
                case CyUSBFSParameters.MEMORY_ALLOC_STATIC:
                    rbStaticAllocation.Checked = true;
                    break;
                case CyUSBFSParameters.MEMORY_ALLOC_DYNAMIC:
