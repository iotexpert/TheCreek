...
namespace USBFS_v1_60
{
    ...
    internal class CyUSBCodeGenerator : CyUsbCodePrimitives
    {
        ...
        private void GenerateDevices()
        {
            ...
            for (int i = 0; i < m_parameters.m_deviceTree.m_nodes[0].m_nodes.Count; i++)
            {
                if (((CyDeviceDescriptor)m_parameters.m_deviceTree.m_nodes[0].m_nodes[i].m_value).bMemoryAlloc == 
                    CyUSBFSParameters.MEMORY_ALLOC_DYNAMIC)
                {
        ...
        private string GenReportTable(int rptIndex, string instName, string qualName)
        {
            ...
            if ((rptIndex > 0) && (rptIndex <= m_parameters.m_hidReportTree.m_nodes[0].m_nodes.Count))
            {
                ...
                if (ReportCount(ReportTop, CyUSBFSParameters.RPTITEM_INPUT) > 0)
                {
                    ...
                    {
                        code.Append(GenerateTDEntry(NULL_PTR, qualName + "_IN_BUF[0]", NULL_PTR)).Append(COMMA_NEWLINE);
                    }
