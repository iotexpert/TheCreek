...
namespace FanController_v2_20
{
    public partial class CyFansTab : CyEditingWrapperControl
    {
        ...
        private void m_nudNumFans_TextChanged(object sender, EventArgs e)
        {
            ...
            ValidateNudNumFans();
            bool valid = ep_Errors.GetError(m_nudNumFans) == "";
            if (valid)
            {
        ...
        private void ValidateNudNumFans()
        {
            bool error = false;
            string message = "";
            bool invalid = false;

            int val = CyParameters.GetNumericUpDownText(m_nudNumFans, out error, ref message);
        ...
        private void m_nud_Validating(object sender, CancelEventArgs e)
        {
            ...
            int val = CyParameters.GetNumericUpDownText(sender, out error, ref message);
        ...
        public override IEnumerable<CyCustErr> GetErrors()
        {
            ...
            // Check numeric up down controls for empty values
            bool hasEmpty = false;
            if (m_nudNumFans.Text == "")
            {
                hasEmpty = true;
            }
            else if (ep_Errors.GetError(m_nudNumFans) != "")
            {
                errs.Add(new CyCustErr(ep_Errors.GetError(m_nudNumFans)));
            }

            for (int i = 0; i < m_nodsArray.Count; i++)
                for (int j = 0; j < m_nodsArray[i].Length; j++)
                {
                    if (m_nodsArray[i][j].Visible && (m_nodsArray[i][j].Text == ""))
                    {
                        hasEmpty = true;
                    }
                }
            if (hasEmpty)
            {
                errs.Add(new CyCustErr(Resources.ErrorEmptyFieldsOnFans));
            }
            return errs;
        }
    }
