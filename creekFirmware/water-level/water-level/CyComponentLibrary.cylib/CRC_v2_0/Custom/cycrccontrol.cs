...
namespace CRC_v2_0
{
    public partial class CyCRCControl : UserControl
    {
        ...
        public void UpdateControls()
        {
            ...
            {
                MessageBox.Show("Problem with parameters initialization.", "Error", MessageBoxButtons.OK,
        ...
        private void textBoxN_TextChanged(object sender, EventArgs e)
        {
            // ErrorProvider
            UInt16 res;
            const string ERROR_N_RANGE = "N should be in range [1, 64]";
            if (UInt16.TryParse(textBoxN.Text, out res))
            {
                ...
                {
                    errorProvider.SetError(textBoxN, ERROR_N_RANGE);
                }
            }
            else if (textBoxN.Text == "")
            {
                    ...
                    if (textBoxResult.Text != "")
                    {
                        ...
                        if (polyVal >= ((ulong)1 << 63))
                        {
                            ...
                            s = Convert.ToString(Convert.ToInt64(s.Substring(0, 4), 16), 2) +
                                Convert.ToString(Convert.ToInt64(s.Substring(4), 16), 2);
                        }
                        else
                        {
        ...
        private void textBoxSeed_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    errorProvider.SetError(textBoxSeed, "");
                }
                else
                {
                    errorProvider.SetError(textBoxSeed, "The maximum valid Seed value for N=" + 
                    m_Parameters.Resolution + " is 0x" + GetMaxSeedValue().ToString("X"));
                }
            }
            catch
            {
                errorProvider.SetError(textBoxSeed, "Invalid Seed value");
            }
        }
