...
void `$INSTANCE_NAME`_Start(void)
{
    ...
    if(`$INSTANCE_NAME`_initvar == 0)
    {
        ...
        `$INSTANCE_NAME`_InternalInterrupt_Start();
        `$INSTANCE_NAME`_InternalInterrupt_SetVector(`$INSTANCE_NAME`_ISR);        
        #endif
...
void `$INSTANCE_NAME`_Stop(void)
{
	#if(`$INSTANCE_NAME`_InternalInterruptEnabled)
        /* Initialize the Interrupt Component */
        `$INSTANCE_NAME`_InternalInterrupt_Stop();
    #endif
}
...
{
    #if(`$INSTANCE_NAME`_InternalInterruptEnabled)
	`$INSTANCE_NAME`_InternalInterrupt_Enable();
    #endif
}
...
{
    #if(`$INSTANCE_NAME`_InternalInterruptEnabled)
	`$INSTANCE_NAME`_InternalInterrupt_Disable();
    #endif
}
...
void `$INSTANCE_NAME`_WriteByte(`$RegSizeReplacementString`  txDataByte)
{
	...
	if((`$INSTANCE_NAME`_TXBUFFERREAD == `$INSTANCE_NAME`_TXBUFFERWRITE) && !(`$INSTANCE_NAME`_STATUS & `$INSTANCE_NAME`_STS_TX_BUF_FULL))
	{
	...
	while((`$INSTANCE_NAME`_STATUS & `$INSTANCE_NAME`_STS_TX_BUF_FULL) != 0);
...
uint8 `$INSTANCE_NAME`_GetRxBufferSize(void)
{
    ...
    size = (`$INSTANCE_NAME`_STATUS & `$INSTANCE_NAME`_STS_RX_BUF_EMPTY) ? 0:1;
...
uint8  `$INSTANCE_NAME`_GetTxBufferSize(void)
{
    ...
    /* Is the fifo is full. */
    if(size & `$INSTANCE_NAME`_STS_TX_BUF_FULL)
    {
        size = 4;
    }
    else if(size & `$INSTANCE_NAME`_STS_TX_BUF_NOT_FULL)
    {
        size = 0;
    }
    else
...
{
    while((`$INSTANCE_NAME`_ReadStatus() & `$INSTANCE_NAME`_STS_RX_BUF_EMPTY) == 0)
