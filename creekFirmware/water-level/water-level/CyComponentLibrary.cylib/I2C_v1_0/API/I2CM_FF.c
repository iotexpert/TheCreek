...
*
* Description: I2C Master Specification
*
*
* NOTE:
* 
...
uint8 `$INSTANCE_NAME`_MasterSendStart(uint8 slaveAddress, uint8 R_nW)
{
    ...
    {
        /* Do nothing until I2C bus is not busy  */
        for(timeOut = 300, errStatus = `$INSTANCE_NAME`_MSTR_BUS_TIMEOUT; timeOut; timeOut--)
        {
            ...
            //:TODO  We should get something better, CyuSecDelay().
            CyDelay(1);
        }
        ...
        `$INSTANCE_NAME`_MCSR = `$INSTANCE_NAME`_MCSR_START_GEN;  /* Generate a start */
...
uint8 `$INSTANCE_NAME`_MasterSendStop(void)
{
    ...
    if((`$INSTANCE_NAME`_MCSR & `$INSTANCE_NAME`_MCSR_MSTR_MODE) != 0)
    {
        `$INSTANCE_NAME`_CSR = `$INSTANCE_NAME`_CSR_GEN_STOP;
                                       ...
                                       uint8 cnt, uint8 mode)
{
    ...
    if((`$INSTANCE_NAME`_State == `$INSTANCE_NAME`_SM_IDLE) ||(`$INSTANCE_NAME`_State == `$INSTANCE_NAME`_SM_MSTR_HALT))
    {
        ...
        if(errStatus == `$INSTANCE_NAME`_MSTR_NO_ERROR)
        {
            ...
            `$INSTANCE_NAME`_I2C_IRQ_Enable( );   /* IRQ must be enabled for this to work */
