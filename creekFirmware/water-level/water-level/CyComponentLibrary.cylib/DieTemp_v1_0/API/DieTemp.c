...
#define NUM_SAMPLES                         (1 << (`$INSTANCE_NAME`_NUMBER_OF_SAMPLES))
...
#define GET_TEMP_TIME                       (1 << (NUM_SAMPLES + 1)) * \
                                            ...
                                            FRM_EXEC_TIME
#define `$INSTANCE_NAME`_MAX_WAIT           ((GET_TEMP_TIME  ) / CY_SYS_ns_PER_TICK) /* In system ticks. */
...
*   CYRET_LOCKED if the spc was bussy.
*
*
*******************************************************************************/
...
*
*******************************************************************************/
cystatus `$INSTANCE_NAME`_Query(uint16 * temperature)
{
    uint8 temp[2];
    cystatus status;
    ...
    if(!(*SPC_STATUS & SPC_SPC_IDLE))
    {
        ...
        if(CySpcReadStatus == CYRET_SUCCESS)
        {
            ...
            if(CySpcReadData(temp, 2) == 2)
            {
                /* Set magnitude */
                if(temp[0])
                    *temperature = (unsigned short) temp[1];
                else
                    *temperature = (unsigned short) - temp[1];
...
*
*******************************************************************************/
cystatus `$INSTANCE_NAME`_GetTemp(uint16 * temperature)
{
    uint8 ticks;
    cystatus status;
