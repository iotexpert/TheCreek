    ...
    public partial class CyVoltagesTab : CyEditingWrapperControl
    {
        ...
        private bool RangeCheck(int rowIndex, int colIndex)
        {
            ...
            if (colIndex == colMarginLow.Index)
            {
                ...
                currCellValue = m_params.VoltagesTable[rowIndex].m_marginLow;
            }
            if (colIndex == colMarginHigh.Index)
            {
                message = Resources.MarginHighVoltageError;
                currCellValue = m_params.VoltagesTable[rowIndex].m_marginHigh;
            }

            if (m_editableCols.Contains(colIndex) && dgvVoltages.Columns[colIndex].ReadOnly == false)
            {
                message = CyParameters.IsValueInRange(currCellValue, dgvVoltages[colIndex, rowIndex].Value, min, max, message);
            }
        ...
        private bool SanityCheck(int rowIndex)
        {
            ...
            try
            {
                ...
                double? marginLow = m_params.VoltagesTable[rowIndex].m_marginLow;
                double? marginHigh = m_params.VoltagesTable[rowIndex].m_marginHigh;
                ...
                dgvVoltages.Rows[rowIndex].Cells[colNominalOutputVoltage.Index].ErrorText =
                    CyParameters.ValueNotGreaterThan(nominalVoltage, marginHigh, Resources.NominalOutputVoltageSanityCheck);
                //VMarginHi[x] <= VMax[x]
        ...
        public void UpdateUIFromTable()
        {
            ...
            for (int i = 0; i < m_dataGridRowCount; i++)
            {
                dgvVoltages.Rows.Add(new object[] {
                    ...
                    m_params.VoltagesTable[i].m_marginLow,
                    m_params.VoltagesTable[i].m_marginHigh,
                });
            }
        ...
        public void UpdateTableFromUI()
        {
            for (int i = 0; i < m_dataGridRowCount; i++)
            {
                ...
                m_params.VoltagesTable[i].m_marginLow = CyParameters.ParseNullableDouble(
                            CyParameters.CellToString(dgvVoltages.Rows[i].Cells[colMarginLow.Name].Value));

                m_params.VoltagesTable[i].m_marginHigh = CyParameters.ParseNullableDouble(
                            CyParameters.CellToString(dgvVoltages.Rows[i].Cells[colMarginHigh.Name].Value));
            }
        }
