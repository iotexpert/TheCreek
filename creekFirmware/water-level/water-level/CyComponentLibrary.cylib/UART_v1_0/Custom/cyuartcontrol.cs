...
namespace CyCustomizer.UART_v1_0
{
    public partial class cyuartcontrol : UserControl
    {
        ...
        public cyuartcontrol(ICyInstEdit_v1 inst, ICyTerminalQuery_v1 termquery, cyuartadvancedcontrol control_adv)
        {
            m_Component = inst;
        ...
        public void UpdateFormFromParams(ICyInstEdit_v1 inst)
        {
            ...
            foreach (string str in BaudRateEnums)
            {
                if (!baudfound)
                {
                    string paramcompare = m_Component.ResolveEnumIdToDisplay("BaudRate", prms.BaudRate.Expr);
                    if (paramcompare == str)
                    {
            ...
            if (!baudfound)
            {
                m_cbBitsPerSecond.Text = prms.BaudRate.Expr;
            }
            ...
            foreach (string str in DataBitsEnums)
            {
                if (!DBfound)
                {
                    string paramcompare = m_Component.ResolveEnumIdToDisplay("NumDataBits", prms.NumDataBits.Expr);
                    if (paramcompare  == str)
                    {
            ...
            if (!DBfound)
            {
                m_cbDataBits.Text = prms.NumDataBits.Expr;
            }
            //Set the Parity Combo Box
            IEnumerable<string> ParityEnums = inst.GetPossibleEnumValues("ParityType");
            ...
            foreach (string str in ParityEnums)
            {
                if (!Parityfound)
                {
                    string paramcompare = m_Component.ResolveEnumIdToDisplay("ParityType", prms.ParityType.Expr);
                    if (str == paramcompare)
                    {
            ...
            if (!Parityfound)
            {
                m_cbParity.Text = prms.ParityType.Expr;
            }
            //Set the Stop Bits Combo Box
            IEnumerable<string> StopBitsEnums = inst.GetPossibleEnumValues("NumStopBits");
            bool SBfound = false;
            foreach (string str in StopBitsEnums)
            {
                if (!SBfound)
                {
                    string paramcompare = m_Component.ResolveEnumIdToDisplay("NumStopBits", prms.NumStopBits.Expr);
                    if (paramcompare == str)
                    {
            ...
            if (!SBfound)
            {
                m_cbStopBits.Text = prms.NumStopBits.Expr;
            }
            ...
            foreach (string str in FlowControlEnums)
            {
                if (!FCfound)
                {
                    string paramcompare = m_Component.ResolveEnumIdToDisplay("FlowControl", prms.FlowControl.Expr);
                    if (paramcompare == str)
                    {
            ...
            if (!FCfound)
            {
                m_cbFlowControl.Text = prms.FlowControl.Expr;
            }
        }
        ...
        private void m_rbRXOnly_Click(object sender, EventArgs e)
        {
            if (!m_rbRXOnly.Checked)
            {
                ...
                SetAParameter("TXEnable", "false");
                SetAParameter("RXEnable", "true");
        ...
        private void m_rbTXOnly_Click(object sender, EventArgs e)
        {
            if (!m_rbTXOnly.Checked)
            {
                ...
                m_rbRXTX.Checked = false;
                m_rbTXOnly.Checked = true;
                SetAParameter("TXEnable", "true");
                SetAParameter("RXEnable", "false");
                m_control_adv.UpdateFormFromParams(m_Component);
            }
        }
