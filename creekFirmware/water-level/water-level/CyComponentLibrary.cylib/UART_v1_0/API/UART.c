...
void `$INSTANCE_NAME`_Start(void)
{
    if(`$INSTANCE_NAME`_initvar == 0)
	{
		`$INSTANCE_NAME`_initvar = 1;
            ...
            //Write Value to RX Bit counter
            `$INSTANCE_NAME`_RXBITCTR_PERIOD = `$INSTANCE_NAME`_RXBITCTR_INIT;
            ...
            /*Write Counter Value for TX Bit Clk Generator / Bit Counter and Enable */
            `$INSTANCE_NAME`_TXBITCTR_PERIOD = `$INSTANCE_NAME`_TXBITCTR_INIT;
...
{
	/* MSB contains Status and LSB contains RX data. */
...
/*******************************************************************************
* FUNCTION NAME: void `$INSTANCE_NAME`_PutString( uint16* ramString )
*
* Summary:
* Write a Sequence of bytes on the Transmit line. Data comes from RAM.
*
* Parameters:
* ramString: uint16 pointer to character string of Data to Send
*
* Return:
* -None-
*
...
/*******************************************************************************
* FUNCTION NAME: void `$INSTANCE_NAME`_PutString( uint8* romString )
*
* Summary:
...
/*******************************************************************************
* FUNCTION NAME: void `$INSTANCE_NAME`_PutArray( uint8* romString, int8 byteCount )
*
* Summary:
* Write a Sequence of bytes on the Transmit line. Data comes from ROM.
*
...
{
	//TODO: Hardware doesn't support this yet.
