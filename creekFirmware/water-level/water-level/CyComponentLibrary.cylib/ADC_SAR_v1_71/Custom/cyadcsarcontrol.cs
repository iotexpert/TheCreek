...
namespace ADC_SAR_v1_71
{
    public partial class CySARADCControl : UserControl, ICyParamEditingControl
    {
        ...
        {
            return (m_params.AdcSampleRate >= GetMinSampleRate() && m_params.AdcSampleRate <= GetMaxSampleRate());
        }
        ...
        public uint GetExternalSampleRate()
        {
            if (m_params.m_externalClock >= 0)
                return (uint)Math.Round((m_params.m_externalClock * 1000) / (GetResolution()
        ...
        {
            return (uint)Math.Round((double)CyParamRange.CLOCK_FREQ_MAX_HZ / (double)(GetResolution() +
                m_params.SamplePrecharge));
        }

        public uint GetMinSampleRate()
        {
            return (uint)Math.Round((double)CyParamRange.CLOCK_FREQ_MIN_HZ / (double)(GetResolution() +
                m_params.SamplePrecharge));
        }
