/*******************************************************************************
* File Name:  B_SPI_Slave_v2_0.v
* Version `$CY_MAJOR_VERSION`.`$CY_MINOR_VERSION`
*
* Description:
*  This file provides a base level model of the SPI Slave componnent
*
* Note:
*  None
********************************************************************************
...
*  |  Bit  |   7    |   6    |   5    |   4    |   3    |   2    |   1    |   0    |
*  +=======+--------+--------+--------+--------+--------+--------+--------+--------+
*  | Desc |interrupt|bt_cplt |unused  |unused  |unused  |tx_empty |tx_n_fll|spi_done|
*  +=======+--------+--------+--------+--------+--------+--------+--------+--------+
...
*  |  Bit  |   7    |   6    |   5    |   4    |   3    |   2    |   1    |   0    |
*  +=======+--------+--------+--------+--------+--------+--------+--------+--------+
*  | Desc |interrupt|rx_full |rx_overr|rx_emp  |rx_n_emp| unused |unused  | unused |
*  +=======+--------+--------+--------+--------+--------+--------+--------+--------+
...
*                  1 = TX not full
*
*    tx_f_full =>  0 = TX_FIFO not full
*                  1 = TX FIFO full
*
*    rx_n_e    =>  0 =  RX FIFO not full
*                  1 =  RX FIFO full
*
*    rx_f_e    =>  0 = RX FIFO empty
*                  1 = RX FIFO not empty
*
...
********************************************************************************/
`include "cypress.v"
`ifdef B_SPI_Slave_v2_0_V_ALREADY_INCLUDED
`else
`define B_SPI_Slave_v2_0__V_ALREADY_INCLUDED

module B_SPI_Slave_v2_0(
...
***************************************************************************/    
    ...
    localparam PSOC3_ES2  = (`CYDEV_CHIP_DIE_EXPECT == `CYDEV_CHIP_DIE_LEOPARD &&  
                             `CYDEV_CHIP_REV_EXPECT <= `CYDEV_CHIP_REV_LEOPARD_ES2); 
    /* PSoC5 ES1 or earlier */                         
    localparam PSOC5_ES1  = (`CYDEV_CHIP_DIE_EXPECT == `CYDEV_CHIP_DIE_PANTHER &&  
    ...
    begin:es2
        ...
        B_SPI_Slave_ES2 #(.ModePOL(ModePOL), .ModeCPHA(ModeCPHA),.ShiftDir(ShiftDir),
        ...
        B_SPI_Slave_ES3 #(.ModePOL(ModePOL), .ModeCPHA(ModeCPHA),.ShiftDir(ShiftDir),
                          .NumberOfDataBits(NumberOfDataBits),.BidirectMode(BidirectMode))
        SPISlave(
            .reset(reset),
            .clock(clock),
    ...
    localparam [2:0] dp16MsbVal   = NumberOfDataBits[3:0] - 4'd9;
    /* Index for Bidirectional Mode Control Register bit */
    ...
    /* TX FIFO loads data to shifter on negedge of SCLK */
    /* Received data is stored into the RX FIFO on the posedge of SCLK */
    localparam f0_edge = `SC_FIFO_CLK0_POS;
    ...
    localparam dp8_config = {
            ...
            3'h00, `SC_FIFO_SYNC__ADD, 4'h00,f1_edge, f0_edge,
            `SC_FIFO_CLK__DP,`SC_FIFO_CAP_AX,`SC_FIFO__EDGE,
            `SC_FIFO__SYNC,`SC_EXTCRC_DSBL,`SC_WRK16CAT_DSBL  /*CFG17-16:  */
        };
    ...
    reg  dpcounter_one_fin;
    reg  miso_tx_empty_reg;
    reg  miso_tx_empty_reg_fin;
    
    wire tx_load           = (ModeCPHA == 1'b0) ? (load & (~dpMISO_fifo_empty)) : dpcounter_zero;
    wire dpcounter_one_inv = ~dpcounter_one_reg;    
	wire byte_complete     = dpcounter_one_fin & dpcounter_one_inv;
    
    wire prc_clk_src = (((ModeCPHA == 0) && (ModePOL == 0)) || ((ModeCPHA == 1) && (ModePOL == 1))) ? sclk : ~sclk;
	wire cnt_clk_src = (((ModeCPHA == 0) && (ModePOL == 1)) || ((ModeCPHA == 1) && (ModePOL == 0))) ? sclk : ~sclk;
    ...
    always@(posedge clock_fin) begin
        ...
        dpcounter_one_fin     <= dpcounter_one_reg;
        miso_tx_empty_reg     <= dpMISO_fifo_empty;
        miso_tx_empty_reg_fin <= miso_tx_empty_reg;
     end 
    ...
    assign tx_status[SPIS_STS_TX_BUFFER_EMPTY_BIT]     = dpMISO_fifo_empty;
    ...
    assign rx_status[SPIS_STS_RX_FIFO_OVERRUN_BIT]     = mosi_buf_overrun;
	assign rx_status[SPIS_STS_RX_FIFO_FULL_BIT]        = dpMOSI_fifo_full;
    ...
    generate
        if(BidirectMode) begin: SyncCtl
            ...
            assign mosi_fin  = (tx_enable == 1'b1) ? miso_from_dp : mosi;
    ...
    cy_psoc3_count7 #(.cy_period(NumberOfDataBits-7'h1),.cy_route_ld(1),.cy_route_en(1))
    BitCounter(
        /* input          */ .clock(cnt_clock),
        /* input          */ .reset(reset),
        /* input          */ .load(1'b0),
        /* input          */ .enable(1'b1),
    ...
    assign dpcounter_zero = (count == 4'h00);
    assign dpcounter_one  = (count == 4'h01);
    ...
    generate
        ...
        if(NumberOfDataBits <= SR8) begin: sR8
            ...
            cy_psoc3_dp16 #(.cy_dpconfig_a(dp16_config_msb), .cy_dpconfig_b(dp16_config_lsb))
            Dp(
                /*  input         */  .clk(dp_clock),
                /* input          */  .reset(reset), 
                /*  input [02:00] */  .cs_addr({inv_ss, 1'b0, tx_load}),
...
module B_SPI_Slave_ES2(
    ...
    wire [7:0] control;
    wire clock_fin;
    wire dpcounter_zero = (count == 4'd0);
	wire dpcounter_one  = (count == 4'h01);
	wire mosi_to_dp_fin;
    reg  dpcounter_one_reg;
    reg  dpcounter_one_fin;
    reg  miso_tx_empty_reg;
    ...
    wire dpcounter_one_inv = ~dpcounter_one_reg;    
	wire byte_complete     = dpcounter_one_fin & dpcounter_one_inv;
    wire ff0_load          = dpcounter_zero    & (~miso_tx_empty);	
    ...
    if(BidirectMode) begin : AsyncCtl
        ...
        assign tx_enable      = control[CTRL_TX_ENABLE];
        assign mosi_to_dp_fin = (tx_enable == 1'b1) ? miso : mosi_to_dp;
    ...
    generate
        ...
        begin : sR16
            cy_psoc3_dp16 #(.cy_dpconfig_a(dp16_MISO_msb_cfg), .cy_dpconfig_b(dp16_MISO_lsb_cfg))
            DpMISO(
                /*  input           */ .clk(misoclk_fin),
                /*  input   [02:00] */ .cs_addr({ss,1'b0,ff0_load_fin}),
                /*  input           */ .route_si(1'b0),
            ...
            cy_psoc3_dp16 #(.cy_dpconfig_a(dp16_MOSI_msb_cfg), .cy_dpconfig_b(dp16_MOSI_lsb_cfg))
            DpMOSI(
                /*  input           */ .clk(mosiclk_fin),
                /*  input   [02:00] */ .cs_addr({ss, 2'b0}),
                /*  input           */ .route_si(mosi_to_dp_fin),
    ...
    always@(posedge clock_fin) begin
        dpcounter_one_reg <= dpcounter_one;
        dpcounter_one_fin <= dpcounter_one_reg;
        miso_tx_empty_reg <= miso_tx_empty;
     end 
    ...
    RxStsReg(
        ...
        /* ES3 status register connections */
    assign tx_status[SPIS_STS_SPI_DONE_BIT]            = (miso_tx_empty_reg & byte_complete);
    assign tx_status[SPIS_STS_TX_BUFFER_EMPTY_BIT]     = miso_tx_empty;
    ...
    assign rx_status[SPIS_STS_RX_FIFO_OVERRUN_BIT]     = (mosi_rx_full & mosi_fifo_load);
	assign rx_status[SPIS_STS_RX_FIFO_FULL_BIT]        = mosi_rx_full;
...
`endif /* B_SPI_Slave_v2_0_V_ALREADY_INCLUDED */
