...
{   
    ...
    `$INSTANCE_NAME`_LSB_AUX_CONTROL_REG |= `$INSTANCE_NAME`_F0_LVL;
...
*
* Summary:
*  Initiates a write transaction on the `$BitWidthReplacementString`-bit parallel interface.
...
{
    while((`$INSTANCE_NAME`_STATUS_REG & `$INSTANCE_NAME`_FIFO_HALF_EMPTY) == 0u)
    {
            /* Command queue is full */
    }   
    `$INSTANCE_NAME`_FIFO_LSB_REG = d_c;
    `$INSTANCE_NAME`_FIFO_LSB_REG = LO8(wrData);
    #if (`$INSTANCE_NAME`_BUS_WIDTH == 16u)
        `$INSTANCE_NAME`_FIFO_MSB_REG = HI8(wrData);
                                                  ...
                                                  `=ReentrantKeil($INSTANCE_NAME . "_Read`$BitWidthReplacementString`")`
{
    ...
    `$INSTANCE_NAME`_FIFO_LSB_REG = d_c | `$INSTANCE_NAME`_READ_COMMAND;
