...
namespace ADC_DelSig_v1_30
{
    public partial class CyADC_DelSigControl1 : UserControl
    {
        ...
        const string DISABLE_BUFFER = "Disable Buffer";
        ...
        private void CheckFreq()
        {
            ...
            gain = int.Parse(m_Component.GetCommittedParam(INPUT_BUFFER_NAME).Value);
            if(resolution > 11)
            {
                if(resolution > 15)
                {
                    ...
                    if(m_cbInputBufferGain.Text == DISABLE_BUFFER) //Unbuffered Input
