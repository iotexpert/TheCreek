...
*  the component with the following exceptions: it does not clear data from 
*  the mailboxes.
...
*
* Summary:   
*  This function activates the hardware and begins component operation. It is 
*  not necessary to call `$INSTANCE_NAME`_Enable() because the 
*  `$INSTANCE_NAME`_Start() routine calls this function, which is the preferred
...
*
* Summary:
*  This function sets the initVar variable, calls the CAN_Init() function, and 
*  then calls the CAN_Enable() function. This function puts the CAN component 
*  into run mode and starts the counter if polling mailboxes are available.
*
* Parameters:
*  None. 
*
* Return:
*  Indication whether the register is written and verified.  
*
* Global variables:
...
*
* Summary:
*  This function puts CAN Component into Stop mode and stops the counter if 
...
*
* Summary:
*  This function sets PreScaler that generates  the time quantum which defines
...
*
* Summary:
*  This function sets the Edge Mode. Modes are 'R' to 'D'(Reccessive to Dominant)
...
*
* Summary:
*  This function sets the Operation Mode. Operations Modes are Active of Listen
...
*
* Summary:
*  This function writes only CAN receive registers.
*
* Parameters:
*  regAddr: Pointer to the CAN receive register;
*  config:  Value that will be written in the register.
*
* Return:
*  Indication whether the register is written and verified.
*
...
*
* Summary:
*  This function configures all receive registers for particular mailbox. The 
*  mailbox number contains the CAN_RX_CFG structure.
*
* Parameters:
*  rxConfig: Pointer to the structure that contain all required values to configure
*  all receive registers for particular mailbox.
*
* Return:
*  Indication if a particular receive register configuration has been accepted 
*  or rejected.
*
*******************************************************************************/
...
*  Mailbox number contains the `$INSTANCE_NAME`_TX_CFG structure.
*
* Parameters:
*  txConfig: Pointer to the structure that contain all required values to 
*  configure all transmit registers for particular mailbox.
*
* Return:
*  Indication if a particular transmit register configuration has been accepted
*  or rejected.
*
*******************************************************************************/
