...
#else
    #if (`$INSTANCE_NAME`_EXTERNAL_LO_CLK_FREQ != -1)
        /* External LO frequency when clock is used as the LO source */
        #define `$INSTANCE_NAME`_LO_CLOCK_FREQ   (`$EXT_LO_CLK_FREQ`)  
    #else
        /* External LO frequency when other than Clock is used as the LO source */
        #define `$INSTANCE_NAME`_LO_CLOCK_FREQ   (`$LO_clock_freq`)
    #endif
#endif
...
/* Below defines are to retain backward compatibility. These are depricated and should not be used
in future designs */
...
#define `$INSTANCE_NAME`_CLK                    `$INSTANCE_NAME`_CLK_REG     /* SC Block clk control */
#define `$INSTANCE_NAME`_BSTCLK                 `$INSTANCE_NAME`_BSTCLK_REG  /* SC Boost clk Control */
#define `$INSTANCE_NAME`_PM_ACT_CFG             `$INSTANCE_NAME`_PM_ACT_CFG_REG  /* Power manager */
#define `$INSTANCE_NAME`_PM_STBY_CFG            `$INSTANCE_NAME`_PM_STBY_CFG_REG  /* Power manager */
