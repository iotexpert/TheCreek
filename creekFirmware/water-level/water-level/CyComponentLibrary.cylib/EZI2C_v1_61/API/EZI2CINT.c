...
volatile uint8   `$INSTANCE_NAME`_curState;

/* Status byte */
volatile uint8   `$INSTANCE_NAME`_curStatus;
...
volatile uint8 * `$INSTANCE_NAME`_dataPtrS1;
    ...
    volatile uint8   `$INSTANCE_NAME`_rwOffsetS1;

    /* Points to next value to be read or written */
    volatile uint8   `$INSTANCE_NAME`_rwIndexS1;

    /* Offset where data is read only */
    volatile uint8   `$INSTANCE_NAME`_wrProtectS1;

    /* Size of array between 1 and 255 */
    volatile uint8   `$INSTANCE_NAME`_bufSizeS1;

#else   /* 16 bit sub-address */

    /* Offset for read and write operations, set at each write sequence */
    volatile uint16  `$INSTANCE_NAME`_rwOffsetS1;

    /* Points to next value to be read or written */
    volatile uint16  `$INSTANCE_NAME`_rwIndexS1;
...
/* If two slave addresses, creat second set of varaibles  */
    ...
    /* Select 8 or 16 bit secondary addresses */
        ...
        /* Offset for read and write operations, set at each write sequence */
        volatile uint8   `$INSTANCE_NAME`_rwOffsetS2;

        /* Points to next value to be read or written */
        volatile uint8   `$INSTANCE_NAME`_rwIndexS2;

        /* Offset where data is read only */
        volatile uint8   `$INSTANCE_NAME`_wrProtectS2;

        /* Size of array between 1 and 255 */
        volatile uint8   `$INSTANCE_NAME`_bufSizeS2;

    #else   /* 16 bit sub-address */

        /* Offset for read and write operations, set at each write sequence */
        volatile uint16  `$INSTANCE_NAME`_rwOffsetS2;

        /* Points to next value to be read or written */
        volatile uint16  `$INSTANCE_NAME`_rwIndexS2;
...
CY_ISR(`$INSTANCE_NAME`_ISR)
{
    ...
    */
    ...
    }   /* (tmpCsr & `$INSTANCE_NAME`_CSR_ADDRESS) */
    ...
    #endif /* ((CY_PSOC3_ES2) && (`$INSTANCE_NAME`_isr__ES2_PATCH)) */
}
