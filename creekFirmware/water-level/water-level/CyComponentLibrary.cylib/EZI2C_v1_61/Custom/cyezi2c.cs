...
namespace EZI2C_v1_61
{
    ...
    public class CyCustomizer : ICyParamEditHook_v1, ICyDRCProvider_v1
    {
        ...
        List<CyCustErr> VerifyRevision(ICyInstQuery_v1 instQuery)
        {
            ...
            if (enableWakeup && IsPSoC3ES3(instQuery.DeviceQuery) == false)
            {
                list.Add(new CyCustErr(Resources.DRCRevisionError));
            }
            if (busPort != CyEI2CBusPort.Any && IsPSoC3ES3(instQuery.DeviceQuery) == false)
            {
                list.Add(new CyCustErr(Resources.PinConnectionsPSoC5AndPSoC3ES2EP));
            }
            if (list.Count == 0)
        ...
        {
            if (IsPSoC3ES3(instQuery.DeviceQuery))
            {
        ...
        private bool IsPSoC3ES3(ICyDeviceQuery_v1 deviceQuery)
        {
