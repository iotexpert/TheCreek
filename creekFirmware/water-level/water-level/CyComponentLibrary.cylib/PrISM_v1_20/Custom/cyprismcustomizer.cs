...
namespace PrISM_v1_20
{
    ...
    public partial class CyCustomizer : ICyParamEditHook_v1
    {
        ...
        { 
            ...
            CyPRISMParameters m_Parameters = new CyPRISMParameters(edit);
            CyConfigurationEditingControl editControl = new CyConfigurationEditingControl(m_Parameters);
            CyParamExprDelegate ParamCommitted = delegate(ICyParamEditor custEditor, CyCompDevParam param)
            {
                m_Parameters.GetParams(param);
            };            
            
            editor.AddCustomPage("General", editControl,ParamCommitted, "General");            
    ...
    public class CyConfigurationEditingControl : ICyParamEditingControl
    {
        ...
        public IEnumerable<CyCustErr> GetErrors()
        {
            ...
            ulong maxSeed = (((ulong)1 << m_control.m_Parameters.Resolution) - 1);
            if (m_control.m_Parameters.SeedValue > maxSeed)
            {
                error1 = new CyCustErr("The maximum valid Seed value for Resolution=" + 
                    m_control.m_Parameters.Resolution.ToString() + " is 0x" + maxSeed.ToString("x"));
