...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_2__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_2__INTC_NUMBER];
}
...
void isr_2_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_2__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_2_INTC_PRIOR = (*isr_2_INTC_PRIOR & (uint32)(~isr_2__INTC_PRIOR_MASK)) |
...
uint8 isr_2_GetPriority(void)
{
    ...
    priority = (*isr_2_INTC_PRIOR & isr_2__INTC_PRIOR_MASK) >> priorityOffset;
